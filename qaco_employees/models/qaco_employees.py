...
class HrEmployee(models.Model):
    ...
    hiring_partner_id = fields.Many2one(
        "res.partner", string="Hiring Partner", tracking=True
    )
    referring_person_id = fields.Many2one(
        "res.partner", string="Referring Person", tracking=True
    )
    technical_supervisor_id = fields.Many2one(
        "res.partner", string="Technical Supervisor", tracking=True
    )
    previous_firm_id = fields.Many2one(
        "res.partner", string="Previous Firm", tracking=True
    )
    joining_reason_noc = fields.Text(string="Reason of NOC (Joining)", tracking=True)
    leaves_already_availed = fields.Integer(
        string="No of Leaves Already Availed", tracking=True
    )
    ...
    leaves_availed_in_qaco = fields.Integer(
        string="No of Leaves Availed in QACO", tracking=True
    )
    ...
    noc_approving_partner_id = fields.Many2one(
        "res.partner", string="NOC Approving Partner Name", tracking=True
    )
    family_reference_ids = fields.One2many(
        "employee.family.reference",
        "employee_id",
        string="Family/References",
        tracking=True,
    )
    ...
    partner_id = fields.Many2one(
        "hr.employee", string="Employee Partner", tracking=True
    )  # New field for Partner
    was_unallocated = fields.Boolean(
        string="Previously Unallocated", default=False, tracking=True
    )
    ...
    experience_ids = fields.One2many(
        "employee.experience", "employee_id", string="Experience", tracking=True
    )
    ...
    transfer_ids = fields.One2many(
        "hr.employee.transfer", "employee_id", string="Transfers History"
    )
    ...
    @api.depends(
        "transfer_ids",
        "transfer_ids.state",
        "transfer_ids.transfer_date_from",
        "transfer_ids.transfer_date_to",
        "transfer_ids.to_client_id",
    )
    def _compute_latest_deputation_client(self):
        ...
        for employee in self:
            ...
            if not tr:
                ...
                employee.latest_deputation_client_id = (
                    unallocated_client.id if unallocated_client else False
                )
    ...
    def _compute_on_leave_today(self):
        ...
        leave_map = {leave.employee_id.id: True for leave in leaves}
        for emp in self:
