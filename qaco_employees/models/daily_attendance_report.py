...
class DailyAttendanceReport(models.Model):
    ...
    def generate_daily_attendance_report(self):
        tz = self.env['ir.config_parameter'].sudo().get_param(
            'qaco_employees.report_tz', 'Asia/Karachi'
        )
        ...
        for emp in employees:
            ...
            if checkins:
                avg_seconds = sum(t.hour * 3600 + t.minute * 60 for t in checkins) / len(checkins)
            ...
            if not today_checkin:
                if leaves.filtered(lambda leave: leave.employee_id.id == emp.id):
                    ...
                    manager_summary[emp.parent_id.name if emp.parent_id else "N/A"]['unauthorized'] += 1
                checkin_raw = None
            else:
            ...
            record = {
                'name': emp.name,
                'client': emp.latest_deputation_client_id.name if emp.latest_deputation_client_id else "UNALLOCATED",
        ...
        for manager, group in grouped_by_manager.items():
            ...
            final_report.append({'is_manager_header': True, 'manager': manager, 'count': len(group)})
            final_report.extend(group)

        total_employees = len(employees)
        present_employees = sum(1 for r in final_report if isinstance(r.get('checkin_raw'), time))
        approved_leaves = sum(1 for r in final_report if r.get('today_checkin') == 'Approved Leave')
        unauthorized_leaves = sum(1 for r in final_report if r.get('today_checkin') == 'Unauthorized Leave')
        late_checkins = sum(1 for r in final_report if isinstance(r.get('checkin_raw'), time) and r['checkin_raw'] > time(10, 0))

        report_date = today.strftime("%d %B %Y")

        # sort manager_summary by late check-ins and then unauthorized leaves
        sorted_summary = sorted(manager_summary.items(), key=lambda x: (-x[1]['late'], -x[1]['unauthorized']))
        ...
        for manager, data in sorted_summary:
            total_managed = len(grouped_by_manager[manager]) if manager in grouped_by_manager else 0
            html_body += f"""
    ...
    def send_email(self, body_html, total_employees, present_employees, approved_leaves, unauthorized_leaves,
                   late_checkins, report_date):
        try:
            ...
            email_list = list(set(emp.work_email for emp in recipient_employees if emp.work_email))
