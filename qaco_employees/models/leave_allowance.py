...
class LeaveAllowance(models.Model):
    ...
    state = fields.Selection(
        [("draft", "To Approve"), ("approved", "Approved")],
        string="Status",
        default="draft",
        tracking=True,
    )
    ...
    def action_reset_to_draft(self):
        if not (
            ...
            raise AccessError(
                "Only HR managers or administrators can reset allowances."
            )
        ...
        for rec in self:
            ...
            if summary_dates:
                ...
                for summary in future_summaries:
                    summary.opening_leaves = (
                        previous.closing_leaves if previous else 0.0
                    )
                    summary.with_context(force_recompute=True)._compute_allowed_leaves()
