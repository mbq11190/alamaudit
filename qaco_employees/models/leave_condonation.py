...
class LeaveCondonation(models.Model):
    ...
    line_ids = fields.One2many(
        'leave.condonation.line', 'condonation_id', string='Payroll Deduction Lines', readonly=True
    )
    ...
    def _compute_total_leave_deduction(self):
        for rec in self:
            rec.total_leave_deduction = sum(rec.line_ids.mapped('leave_deduction_amount'))
    ...
    def default_get(self, fields_list):
        ...
        if 'approval_ids' in fields_list:
            mbq_user = self.env['res.users'].sudo().search([
                '|', ('login', '=', 'mbqasim'), ('name', 'ilike', 'Muhammad Bin Qasim')
            ], limit=1)
            
            if mbq_user:
                res.setdefault('approval_ids', [(0, 0, {
    ...
    def _compute_show_approval_buttons(self):
        ...
        for rec in self:
            ...
            if rec.state == 'submitted':
                if rec._is_admin_user() or (mbq_user and self.env.user.id == mbq_user.id):
    ...
    def _check_employee_change(self):
        for rec in self:
            if not rec._is_admin_user() and rec.employee_id != rec.env.user.employee_id:
                raise UserError(_('You can only create leave condonation for yourself.'))
    ...
    def _ensure_mbq_approver(self):
        ...
        for rec in self:
            ...
            if not mbq_user:
                raise UserError(_('Muhammad Bin Qasim user not found. Please contact administrator.'))
    ...
    def action_submit(self):
        for rec in self:
            if not rec.line_ids:
                raise ValidationError(_('No payroll deduction lines found. Please select a month with leave deductions.'))
            if not rec.reason or not rec.reason.strip():
                raise ValidationError(_('Please provide a reason for leave condonation.'))
            rec.state = 'submitted'
        ...
        if self._is_admin_user() or (mbq_user and self.env.user.id == mbq_user.id):
            ...
            if self.employee_id.user_id and self.employee_id.user_id.partner_id:
                self.env['mail.mail'].sudo().create({
                    'subject': _('Your Leave Condonation Request has been Approved'),
                    'body_html': _(
                        '<p>Dear %s,</p>'
            ...
            raise UserError(_('Only Muhammad Bin Qasim or administrators can approve leave condonations.'))

    def action_reject(self):
        for rec in self:
            rec.state = 'rejected'
            rec.message_post(body=_('Leave condonation rejected by %s') % (self.env.user.name))
            # Notify employee
            if rec.employee_id.user_id and rec.employee_id.user_id.partner_id:
                self.env['mail.mail'].sudo().create({
                    'subject': _('Your Leave Condonation Request has been Rejected'),
                    'body_html': _(
                        '<p>Dear %s,</p>'
    ...
    def action_revert(self):
        for rec in self:
            ...
            rec.message_post(body=_('Leave condonation reverted by %s') % (self.env.user.name))
...
class LeaveCondonationLine(models.Model):
    ...
    payroll_line_id = fields.Many2one(
        'qaco.payroll.line',
        string='Payroll Line Reference',
        readonly=True
    )
    employee_id = fields.Many2one('hr.employee', string='Employee', readonly=True)
    department_id = fields.Many2one('hr.department', string='Department', readonly=True)
    designation_id = fields.Many2one('employee.designation', string='Designation', readonly=True)
...
class LeaveCondonationApproval(models.Model):
    ...
    condonation_id = fields.Many2one('leave.condonation', string='Leave Condonation', ondelete='cascade')
    validating_users_id = fields.Many2one('res.users', string='Approver', domain="[('share','=',False)]")
    is_validation_status = fields.Boolean(string='Approved', readonly=True, default=False, tracking=True)
    ...
    approve_token = fields.Char(readonly=True, copy=False, default=lambda self: str(uuid.uuid4()))
