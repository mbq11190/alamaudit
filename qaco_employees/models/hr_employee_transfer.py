...
class HrEmployeeTransfer(models.Model):
    ...
    sequence = fields.Char(string="Reference", required=True, copy=False, readonly=True, default='/')
    active = fields.Boolean(string="Active", default=True)

    employee_id = fields.Many2one("hr.employee", string="Employee", required=True)
    manager_id = fields.Many2one("res.users", string="Manager", compute="_compute_manager", store=True, readonly=True)
    from_client_id = fields.Many2one("res.partner", string="From Client", readonly=True, store=True)
    to_client_id = fields.Many2one("res.partner", string="To Client", required=True)
    transfer_date_from = fields.Date(string="Transfer Date From", required=True, default=fields.Date.today)
    ...
    previous_transfers_count = fields.Integer(
        string="Previous Transfers",
        compute="_compute_previous_transfers_count"
    )
    ...
    def _check_transfer_date_gap(self):
        ...
        for record in self:
            if record.transfer_date_from and record.transfer_date_to:
                designation_name = (record.employee_id.designation_id.name or '').strip().lower()
                allowed_months = 6 if designation_name in ['assistant manager', 'manager'] else 2
                max_allowed_date = record.transfer_date_from + relativedelta(months=allowed_months)
    ...
    def action_set_unallocated(self):
        ...
        for record in self:
            if record.state != 'approved':
                raise UserError("You can only mark employees as 'Unallocated' after approval.")
    ...
    def _compute_manager(self):
        ...
        for record in self:
            if record.employee_id and record.employee_id.parent_id:
                record.manager_id = record.employee_id.parent_id.user_id  # Assign the manager
            else:
            ...
            if 'employee_id' in vals and 'from_client_id' not in vals:
                ...
                if employee.was_unallocated:
                    ...
                    latest_approved = self.env['hr.employee.transfer'].search([
                        ...
                        ('state', '=', 'approved')
                    ], order='transfer_date_to desc', limit=1)
    ...
    def action_submit(self):
        ...
        for record in self:
            if not record.manager_id:
                raise UserError("You cannot submit this transfer request without selecting a Manager.")
    ...
    def action_approve(self):
        for record in self:
            if (
                    self.env.user != record.manager_id
            ...
            employee.write({
                'latest_deputation_client_id': to_client.id,
                'was_unallocated': False
            })
            ...
            if work_from == 'client':
                to_geofence = geofence_model.search([('name', '=', to_client.name)], limit=1)
                if not to_geofence:
            ...
            if from_client:
                from_geofence = geofence_model.search([('name', '=', from_client.name)], limit=1)
                if from_geofence:
    ...
    def action_reject(self):
        ...
        for record in self:
            if self.env.user != record.manager_id:
                raise UserError("Access Denied: Only the assigned manager can reject this request.")
    ...
    def _compute_previous_transfers_count(self):
        counts = self.env["hr.employee.transfer"].read_group(
            [('employee_id', 'in', self.mapped('employee_id').ids), ('id', 'not in', self.ids)],
    ...
    def send_pending_transfer_reminder(self):
        ...
        for transfer in pending_transfers:
            ...
            if transfer.employee_id.parent_id and transfer.employee_id.parent_id.user_id:
            ...
            recipients = list(set(filter(None, recipients)))  # Remove duplicates & empty
    ...
    def send_email_to_employee(self):
        ...
        for record in self:
            ...
            approving_manager = record.manager_id.name if record.manager_id else "HR Department"

            cc_emails = ",".join(
                rec.employee_id.work_email
                for rec in self.env['hr.transfer.cc.recipient'].search([('active', '=', True)])
    ...
    def reconcile_geofences(self):
        ...
        client_map = {}
        employees = employee_model.search([('latest_deputation_client_id', '!=', False)])
    ...
    ], string="Work From", default='firm', required=True)
    ...
    def write(self, vals):
        if 'active' in vals and vals['active'] is False and \
           not self.user_has_groups('qaco_employees.group_qaco_employee_administrator'):
