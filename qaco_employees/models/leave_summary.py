...
class LeaveSummary(models.Model):
    ...
    event_date = fields.Date(string="Event Date", required=True, default=fields.Date.context_today)
    sequence = fields.Char(default='/', readonly=True)

    opening_leaves = fields.Float(string='Opening Leaves')

    leave_adjustment = fields.Float(
        string='Adjustment',
        store=True
    )
    ...
    closing_leaves = fields.Float(string='Closing Leaves', compute='_compute_closing_leaves', store=True)
    allowed_leaves = fields.Float(string='Allowed Leaves', compute='_compute_allowed_leaves', store=True)
    remaining_leaves = fields.Float(string='Remaining Leaves', compute='_compute_remaining_leaves', store=True)
    approved_leaves = fields.Float(string="Approved Leaves")
    absent_days = fields.Float(string="Absences", compute="_compute_absent_days", store=True)
    ...
    _sql_constraints = [
        ('unique_employee_event', 'unique(employee_id, event_date, is_monthly_summary)',
         'Employee already has a summary for this date!'),
        ('unique_adjustment_ref', 'unique(adjustment_ref_id)', 'Each adjustment can create only one summary!'),
        ('unique_allowance_ref', 'unique(allowance_ref_id)', 'Each allowance can create only one summary!'),
    ]
    ...
    def _compute_approved_leaves(self):
        for record in self:
            ...
            existing_events = self.env['leave.summary'].sudo().search([
            ...
            for leave in leaves:
                ...
                leave_dates = set(start + timedelta(days=i) for i in range((end - start).days + 1))
    ...
    def _compute_absent_days(self):
        ...
        for record in self:
            ...
            attendance_days = set(
                att.check_in.date() for att in Attendance.search([
                    ('employee_id', '=', record.employee_id.id),
                    ('check_in', '>=', start_date),
            ...
            working_days = {
                ...
                if (start_date + timedelta(days=i)).weekday() < 5 and
                   (start_date + timedelta(days=i)) not in holiday_dates
            }
    ...
    def write(self, vals):
        ...
        for record in self:
            if (
                    not record.is_monthly_summary and
                    record.approved_leaves == 0 and
                    not record.absent_days and
                    not vals.get('skip_cascade')
            ):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('sequence', '/') == '/':
                vals['sequence'] = self.env['ir.sequence'].next_by_code('leave.summary.seq') or '/'
    ...
    def unlink(self):
        ...
        for record in self:
            if record.employee_id and record.event_date:
                affected_employees.setdefault(record.employee_id.id, []).append(record.event_date)

        res = super().unlink()

        for emp_id, event_dates in affected_employees.items():
            min_date = min(event_dates)
            future_summaries = self.env['leave.summary'].sudo().search([
                ('employee_id', '=', emp_id),
                ('event_date', '>', min_date)
            ], order='event_date')

            previous = self.env['leave.summary'].sudo().search([
                ('employee_id', '=', emp_id),
                ('event_date', '<', min_date)
    ...
    def _set_opening_based_on_previous(self):
        for record in self:
            previous = self.sudo().search([
                ...
                ('event_date', '=', record.event_date),
                ('id', '<', record.id)
            ], order='event_date desc, id desc', limit=1)
    ...
    def _send_monthly_leave_summary_emails(self):
        ...
        last_month_summaries = self.env['leave.summary'].sudo().search([
            ('event_date', '>=', month_start),
            ('event_date', '<=', month_end),
        ])
        ...
        for employee in employees:
            ...
            summaries = self.env['leave.summary'].sudo().search([
    ...
    def create_leave_summary_event(self):
        ...
        for leave in self:
            ...
            if not leave_start or not leave_end:
                _logger.warning("❗ Leave missing dates: %s (From=%s To=%s)", leave.name, leave_start, leave_end)
                continue
            ...
            if leave_days <= 0:
                _logger.warning("❌ Invalid leave duration: %s (%d days)", leave.name, leave_days)
                continue

            _logger.info("🔁 Processing leave for %s | %s → %s (%d days)",
                         employee.name, leave_start, leave_end, leave_days)

            # Skip duplicate summary
            if self.env['leave.summary'].sudo().search([
                ('employee_id', '=', employee.id),
                ('event_date', '=', leave_start),
                ('approved_leaves', '=', leave_days),
                ('is_monthly_summary', '=', False),
            ], limit=1):
                _logger.debug("⚠️ Summary already exists for %s on %s", employee.name, leave_start)
                continue

            # Create event summary
            summary = self.env['leave.summary'].sudo().create({
                'employee_id': employee.id,
                'event_date': leave_start,
                'approved_leaves': leave_days,
                'is_monthly_summary': False,
            ...
            _logger.info("✅ Created leave.summary for %s (%d days)", employee.name, leave_days)

            # Update monthly summaries (if any overlap)
            affected_months = self.env['leave.summary'].sudo().search([
                ('employee_id', '=', employee.id),
                ('is_monthly_summary', '=', True),
            ...
            for summary in affected_months:
                _logger.info("🔄 Updating monthly summary: %s for %s", summary.event_date, employee.name)
                summary._compute_absent_days()
                summary._compute_closing_leaves()
                summary._compute_remaining_leaves()
                ...
                summary.message_post(
                    body=(f"🟡 Auto-adjusted due to backdated leave "
