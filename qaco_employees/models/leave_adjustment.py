...
class LeaveAdjustment(models.Model):
    ...
    adjustment_ref_id = fields.Many2one(
        'leave.adjustment', string="Adjustment Ref"
    )
    allowance_ref_id = fields.Many2one(
        'leave.allowance', string="Allowance Ref"
    )
    ...
    previous_adjustments_ids = fields.One2many(
        'leave.adjustment', compute='_compute_previous_adjustments', string='Previous Adjustments'
    )
    ...
    def default_get(self, fields_list):
        ...
        if employee:
            ...
            if manager and manager.user_id:
                lines.append({'validating_users_id': manager.user_id.id,
                              'is_manager': True,
                              'is_partner': partner and partner.user_id == manager.user_id})
            if partner and partner.user_id and partner.user_id != (manager and manager.user_id):
                lines.append({'validating_users_id': partner.user_id.id,
                              'is_partner': True,
                              'is_manager': False})
            if lines and 'approval_ids' in fields_list:
                res.setdefault('approval_ids', [(0, 0, l) for l in lines])
        return res
    ...
    def _compute_previous_adjustments(self):
        ...
        for rec in self:
            if rec.employee_id:
                ...
                ], order='adjustment_date desc', limit=20)
    ...
    def _compute_show_approval_buttons(self):
        for rec in self:
            ...
            if rec.state == 'submitted':
                if (
                    rec.env.user.has_group('base.group_system')
                    ...
                    for line in rec.approval_ids.filtered(
                        lambda l: l.validating_users_id == rec.env.user
                    ):
        ...
        for rec in self:
            ...
            if partner and partner.user_id and partner.user_id != (manager and manager.user_id):
                lines.append({'validating_users_id': partner.user_id.id,
                              'is_partner': True,
                              'is_manager': False})
            rec.approval_ids = [(5, 0, 0)] + [(0, 0, l) for l in lines]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if vals.get('sequence', '/') == '/':
                vals['sequence'] = (
                    self.env['ir.sequence'].next_by_code('leave.adjustment.seq')
                    or '/'
                )
    ...
    def write(self, vals):
        if not self._is_admin_user() and 'employee_id' in vals:
            if vals['employee_id'] != self.env.user.employee_id.id:
                raise UserError(
                    _('You can only set the employee to yourself.')
                )
    ...
    def get_approval_url_for(self, user_id):
        ...
        validator = (self.approval_ids.filtered(
            lambda v: v.validating_users_id.id == user_id
        )[0] if self.approval_ids else False)
    ...
    def action_submit(self):
        ...
        for rec in self:
            ...
            adjustment_label = 'Add Leaves' if rec.adjustment_type == 'positive' else 'Reduce Leaves'
    ...
    def action_approve(self):
        ...
        if validator and not validator.is_validation_status:
            ...
            approved_count = len(self.approval_ids.filtered(lambda l: l.is_validation_status))
        ...
        if not pending:
            ...
            final_approver = validator.validating_users_id.name if validator else self.env.user.name
            approval_time = fields.Datetime.now().strftime('%d-%b-%Y %H:%M:%S')
            body = f"""
                <div style="padding: 10px; border-left: 4px solid #4CAF50; background-color: #c8e6c9; margin: 10px 0;">
    ...
    def _send_submit_email(self, target_validators=None, log_chatter=True, transfer_notice=None):
        ...
        for rec in self:
            ...
            subject = f"[Action Required] Approve Leave Adjustment {rec.employee_id.name}"
            if transfer_notice:
            ...
            if target_validators:
                if isinstance(target_validators, models.BaseModel):
                    if target_validators._name == 'leave.adjustment.approval':
                        ...
                        pending = pending.filtered(lambda a: a.validating_users_id.id in user_ids)
                else:
                    user_ids = set(target_validators if isinstance(target_validators, (list, tuple, set)) else [target_validators])
                    pending = pending.filtered(lambda a: a.validating_users_id.id in user_ids)
            ...
            for approver in pending:
                ...
                email_body = self.env['ir.qweb']._render(
                    template_xmlid,
                    {
                        ...
                        'adjustment_type': dict(rec._fields['adjustment_type'].selection).get(rec.adjustment_type, ''),
                        ...
                        'transfer_notice': Markup(transfer_notice) if transfer_notice else '',
                    },
    ...
    def _generate_leave_summary_table_html(self):
        ...
        recent_history = self.leave_history_ids.sorted(key=lambda r: r.event_date, reverse=True)[:10]
        ...
        cell_style = "padding:10px;border:1px solid #dee2e6;text-align:left;color:#495057;"
    ...
    def _generate_previous_adjustments_table_html(self):
        ...
        # Limit to last 10 records for email brevity
        recent_adjustments = self.previous_adjustments_ids.sorted(key=lambda r: r.adjustment_date, reverse=True)[:10]
        ...
        for idx, record in enumerate(recent_adjustments, start=1):
            adjustment_type = dict(record._fields['adjustment_type'].selection).get(record.adjustment_type, '')
