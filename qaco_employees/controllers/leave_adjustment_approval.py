...
class LeaveAdjustmentApprovalController(http.Controller):
    ...
    @http.route(['/leave_adjustment/approve/<string:token>'], type='http', auth='public', website=True)
    def approve_adjustment(self, token):
        validator = request.env['leave.adjustment.approval'].sudo().search([('approve_token', '=', token)], limit=1)
        ...
        return request.render('qaco_employees.adjustment_approval_success', {'adjustment': adjustment})

    @http.route(['/leave_adjustment/refuse/<string:token>'], type='http', auth='public', website=True, methods=['GET', 'POST'], csrf=False)
    def refuse_adjustment(self, token, **post):
        ...
        if request.httprequest.method == 'GET':
            return request.render('qaco_employees.adjustment_refuse_form', {'token': token, 'adjustment': adjustment})
        
        # Process refusal on POST request
        rejection_reason = post.get('rejection_reason', '').strip()
        
        if not rejection_reason:
            return request.render('qaco_employees.adjustment_refuse_form', {
                'token': token,
        ...
        return request.render('qaco_employees.adjustment_rejected', {'adjustment': adjustment, 'rejection_reason': rejection_reason})

    @http.route(['/leave_adjustment/revert/<string:token>'], type='http', auth='public', website=True, methods=['GET', 'POST'], csrf=False)
    def revert_adjustment(self, token, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            adjustment.message_post(body=f"Adjustment reverted by {validator.validating_users_id.name}{remark_txt}", subject='Adjustment Reverted')

            if adjustment.employee_id.work_email:
                body = ("<p>Your leave adjustment has been reverted.</p>" +
                ...
                mail.send()
                sanitized = tools.html_sanitize(body, strip_style=True, strip_classes=True)
            ...
            for user in approver_users:
                if user and user != validator.validating_users_id and user.partner_id.email:
                    body = ("<p>The adjustment has been reverted.</p>" +
            ...
            return request.render('qaco_employees.adjustment_reverted', {'adjustment': adjustment})

        return request.render('qaco_employees.adjustment_revert_form', {'token': token, 'adjustment': adjustment})

    @http.route(['/leave_adjustment/transfer/<string:token>'], type='http', auth='public', website=True, methods=['GET', 'POST'], csrf=False)
    def transfer_adjustment(self, token, **post):
        ...
        if request.httprequest.method == 'GET':
            ...
            return request.render('qaco_employees.adjustment_transfer_form', {
                'token': token,
                'adjustment': adjustment,
                'users': users,
    ...
    # Transfer the approval
        ...
        if transfer_reason:
            notice_chunks.append(f"<p><strong>Transfer reason:</strong> {escape(transfer_reason)}</p>")
        transfer_notice = ''.join(notice_chunks)

        adjustment._send_submit_email(target_validators=validator, log_chatter=False, transfer_notice=transfer_notice)
        
        return request.render('qaco_employees.adjustment_transfer_success', {
            'adjustment': adjustment,
            'old_user': old_user,
            'new_user': new_user
        })
