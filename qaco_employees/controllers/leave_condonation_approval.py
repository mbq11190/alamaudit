...
class LeaveCondonationApprovalController(http.Controller):
    ...
    @http.route(
        ["/leave_condonation/approve/<string:token>"],
        type="http",
        auth="public",
        website=True,
    )
    def approve_condonation(self, token):
        validator = (
            request.env["leave.condonation.approval"]
            .sudo()
        ...
        return request.render(
            "qaco_employees.condonation_approval_success", {"record": record}
        )

    @http.route(
        ["/leave_condonation/refuse/<string:token>"],
        type="http",
        auth="public",
        website=True,
    )
    def refuse_condonation(self, token):
        ...
        record.message_post(
            body=f"Leave condonation rejected by {validator.validating_users_id.name}"
        )
        return request.render("qaco_employees.condonation_rejected", {"record": record})

    @http.route(
        ["/leave_condonation/revert/<string:token>"],
        type="http",
        auth="public",
        website=True,
        methods=["GET", "POST"],
    ...
    def revert_condonation(self, token, **post):
        ...
        if request.httprequest.method == "POST":
            ...
            record.message_post(
                body=f"Condonation reverted by {validator.validating_users_id.name}{remark_txt}",
                subject="Condonation Reverted",
            )

            if record.employee_id.work_email:
                body = (
                    "<p>Your leave condonation has been reverted.</p>"
                    + (
            ...
            for user in approver_users:
                if (
                    ...
                    body = (
                        "<p>The leave condonation has been reverted.</p>"
            ...
            return request.render(
                "qaco_employees.condonation_reverted", {"record": record}
            )

        return request.render(
            "qaco_employees.condonation_revert_form", {"token": token, "record": record}
        )
