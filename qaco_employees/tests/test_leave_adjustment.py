...
class LeaveAdjustmentApprovalRightsTest(common.TransactionCase):
    def setUp(self):
        ...
        self.manager_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.env.ref('qaco_employees.group_qaco_employee_manager').id])],
        })
        ...
        self.partner_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.env.ref('qaco_employees.group_qaco_employee_partner').id])],
        })
        ...
        self.admin_user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [
                ...
                self.env.ref('qaco_employees.group_qaco_employee_administrator').id,
            ])],
        })
    ...
    def test_deduplicate_on_write(self):
        ...
        adj.write({'approval_ids': [(0, 0, {'validating_users_id': self.manager_user.id})]})
        self.assertEqual(len(adj.approval_ids), 2)
