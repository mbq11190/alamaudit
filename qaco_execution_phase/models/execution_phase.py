...
from odoo.exceptions import UserError  # type: ignore[attr-defined]
...
class QacoExecutionPhase(models.Model):
    ...
    audit_id = fields.Many2one(
        "qaco.audit", string="Audit", required=True, ondelete="cascade"
    )
    ...
    planning_phase_id = fields.Many2one(
        ...
        compute="_compute_planning_phase",
        store=True,
        readonly=False,
    )
    ...
    risk_coverage_percent = fields.Float(
        string="Risk Coverage %", compute="_compute_risk_metrics", digits=(3, 2)
    )
    open_critical_risk_count = fields.Integer(
        string="Open Significant Risks", compute="_compute_risk_metrics"
    )
    compliance_beacon_state = fields.Selection(
        ...
        string="Compliance Beacon",
        compute="_compute_compliance_beacon",
        store=True,
    )
    compliance_alert_message = fields.Html(
        string="Compliance Alerts", compute="_compute_compliance_beacon", store=True
    )

    # Relationships to execution artefacts
    execution_risk_line_ids = fields.One2many(
        "qaco.exec.risk.coverage",
        "execution_phase_id",
        string="Risk & Assertion Matrix",
    )
    control_test_ids = fields.One2many(
        "qaco.exec.control.test", "execution_phase_id", string="Tests of Controls"
    )
    substantive_area_ids = fields.One2many(
        "qaco.exec.substantive.area",
        "execution_phase_id",
        string="Substantive Procedures",
    )
    sampling_request_ids = fields.One2many(
        "qaco.exec.sampling.request", "execution_phase_id", string="Sampling Requests"
    )
    evidence_item_ids = fields.One2many(
        "qaco.exec.evidence.item", "execution_phase_id", string="Evidence Items"
    )
    workpaper_ids = fields.One2many(
        "qaco.exec.workpaper", "execution_phase_id", string="Workpapers"
    )

    # Checklists & gating flags
    checklist_controls_linked = fields.Boolean(
        string="All key controls linked to risks", tracking=True
    )
    checklist_walkthrough_documented = fields.Boolean(
        string="Walkthroughs documented & evidenced", tracking=True
    )
    checklist_exceptions_escalated = fields.Boolean(
        string="Control exceptions evaluated & communicated", tracking=True
    )
    checklist_substantive_assertions = fields.Boolean(
        string="Each significant assertion tested substantively", tracking=True
    )
    checklist_sampling_signed = fields.Boolean(
        string="Sampling rationale documented", tracking=True
    )
    checklist_evidence_sufficient = fields.Boolean(
        string="Evidence sufficiency meter green", tracking=True
    )
    checklist_workpapers_reviewed = fields.Boolean(
        string="All workpapers reviewed & signed", tracking=True
    )

    manager_signed_user_id = fields.Many2one(
        "res.users", string="Manager Sign-off", tracking=True, copy=False, readonly=True
    )
    manager_signed_on = fields.Datetime(
        string="Manager Signed On", tracking=True, copy=False, readonly=True
    )
    partner_signed_user_id = fields.Many2one(
        "res.users", string="Partner Sign-off", tracking=True, copy=False, readonly=True
    )
    partner_signed_on = fields.Datetime(
        string="Partner Signed On", tracking=True, copy=False, readonly=True
    )
    quality_reviewer_id = fields.Many2one(
        "res.users", string="Quality Reviewer", tracking=True, copy=False, readonly=True
    )
    quality_reviewed_on = fields.Datetime(
        string="Quality Reviewed On", tracking=True, copy=False, readonly=True
    )

    execution_complete = fields.Boolean(
        string="Execution Phase Complete", tracking=True, copy=False
    )
    execution_completed_on = fields.Datetime(
        string="Execution Completed On", tracking=True, copy=False, readonly=True
    )
    phase_completion_certificate = fields.Boolean(
        string="Phase Completion Certificate Generated", tracking=True, copy=False
    )
    can_finalize_execution = fields.Boolean(
        string="Can Finalize Execution", compute="_compute_can_finalize", store=True
    )
    missing_requirements_note = fields.Html(
        string="Outstanding Requirements", compute="_compute_can_finalize", store=True
    )
    ...
    def _compute_planning_phase(self):
        ...
        for record in self:
            planning = False
            if record.audit_id:
                planning = planning_model.search(
                    [("audit_id", "=", record.audit_id.id)], limit=1
                )
            record.planning_phase_id = planning.id if planning else False

    @api.depends(
        "execution_risk_line_ids.is_fully_addressed",
        "execution_risk_line_ids.is_significant",
    )
    def _compute_risk_metrics(self):
        for record in self:
            lines = record.execution_risk_line_ids
            total = len(lines)
            addressed = len(lines.filtered(lambda line: line.is_fully_addressed))
            record.risk_coverage_percent = (addressed / total * 100.0) if total else 0.0
            record.open_critical_risk_count = len(
                lines.filtered(lambda line: line.is_significant and not line.is_fully_addressed)
            )
    ...
    def _compute_compliance_beacon(self):
        for record in self:
            ...
            statuses = [
                ...
                record.substantive_status,
                record.sampling_status,
                record.evidence_status,
                record.workpaper_status,
            ]
            checklist_ok = all(
                [
            ...
            if all(status == "green" for status in statuses) and checklist_ok:
                ...
                if any(status == "red" for status in statuses):
                    alerts.append(
                        _("Turn the red execution sub-tabs green before finalizing.")
                    )
                if not checklist_ok:
                    alerts.append(_("Complete the mandatory execution checklists."))
                if not alerts:
                    alerts.append(
                        _("Execution compliance requirements are partially satisfied.")
                    )
                state = (
                    "amber"
                    if not any(status == "red" for status in statuses)
                    else "red"
                )
    ...
    def _compute_can_finalize(self):
        for record in self:
            ...
            signoff_ready = bool(
                record.manager_signed_user_id and record.partner_signed_user_id
            )

            record.can_finalize_execution = (
                statuses_ready and checklist_ready and signoff_ready
            )
            ...
            record.missing_requirements_note = (
                "<br/>".join(pending) if pending else False
            )
    ...
    def action_manager_sign(self):
        ...
        self.activity_schedule(
            ...
            user_id=(
                self.partner_signed_user_id.id
                if self.partner_signed_user_id
    ...
    def action_mark_execution_complete(self):
        ...
        if not self.can_finalize_execution:
            raise UserError(
                self.missing_requirements_note
                or _("Execution prerequisites are incomplete.")
            )
        self._ensure_statuses_green()
        self._ensure_checklists_complete()
    ...
    def action_generate_phase_certificate(self):
        ...
        if not self.can_finalize_execution:
            raise UserError(
                _("Complete execution tasks before generating the certificate.")
            )
        self.phase_completion_certificate = True
        self.message_post(
            body=_("Phase completion certificate generated for regulatory file.")
        )
        return {
    ...
    def action_backfill_exception_amounts(self):
        ...
        if not ir_model._get("qaco.finalisation.phase"):
            raise UserError(
                "Install the finalisation phase module to backfill exceptions."
            )
        ...
        for record in self:
            if not record.audit_id:
                continue
            finalisation = finalisation_model.search(
                [("audit_id", "=", record.audit_id.id)], limit=1
            )
            if not finalisation:
                continue
            ...
            for misstatement in finalisation.misstatement_line_ids:
                if (
                    misstatement.source_model != "qaco.exec.substantive.procedure"
                    or not misstatement.source_reference
                ):
                    continue
            ...
            if updates:
                record.message_post(
                    body="Backfilled %s procedure(s) with quantified exceptions."
                    % updates
                )
                total_updates += updates
        if not total_updates:
            raise UserError(
                "No linked misstatements were found to backfill. Run the finalisation sync first."
            )
...
class ExecutionRiskCoverage(models.Model):
    ...
    execution_phase_id = fields.Many2one(
        "qaco.execution.phase",
        string="Execution Phase",
        required=True,
        ondelete="cascade",
    )
    planning_risk_id = fields.Many2one(
        "qaco.risk.register.line", string="Planning Risk", readonly=True
    )
    risk_description = fields.Text(string="Risk Description")
    risk_rating = fields.Selection(
        [("low", "Low"), ("medium", "Medium"), ("high", "High")], string="Risk Rating"
    )
    assertion_type = fields.Selection(
        ASSERTION_TYPES, string="Assertion", default="existence"
    )
    is_significant = fields.Boolean(string="Significant Risk")
    control_test_ids = fields.One2many(
        "qaco.exec.control.test", "risk_line_id", string="Control Tests"
    )
    substantive_procedure_ids = fields.One2many(
        "qaco.exec.substantive.procedure",
        "risk_line_id",
        string="Substantive Procedures",
    )
...
class ExecutionControlTest(models.Model):
    ...
    frequency = fields.Selection(
        CONTROL_FREQUENCY, string="Frequency", default="monthly"
    )
    documentation_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_exec_control_doc_rel",
        "control_id",
        "attachment_id",
    ...
    operating_effectiveness = fields.Selection(
        ...
        string="Operating Effectiveness",
        default="not_tested",
        tracking=True,
    )
    ...
    recommended_sample_size = fields.Integer(
        string="Recommended Sample Size", compute="_compute_sample_recommendation"
    )
    ...
    sampling_request_id = fields.Many2one(
        "qaco.exec.sampling.request", string="Sampling Request", ondelete="set null"
    )
    exception_ids = fields.One2many(
        "qaco.exec.control.exception", "control_test_id", string="Exceptions"
    )
    exception_count = fields.Integer(
        string="Exceptions", compute="_compute_exception_count"
    )
    ...
    test_status = fields.Selection(
        SUBTAB_STATUS, string="Status", default="red", tracking=True
    )
    ...
    def _compute_sample_recommendation(self):
        for record in self:
            ...
            record.recommended_sample_size = int(
                round((baseline * risk_multiplier * 100) / deviation_factor)
            )
    ...
    def _compute_conclusion(self):
        for record in self:
            if (
                ...
                and record.exception_count > record.tolerable_deviation_rate
            ):
                record.conclusion = "no_rely"
            elif (
                record.operating_effectiveness == "effective"
                and record.design_effectiveness == "effective"
            ):
...
class ExecutionSubstantiveArea(models.Model):
    ...
    risk_line_ids = fields.Many2many(
        "qaco.exec.risk.coverage",
        "qaco_exec_area_risk_rel",
        "area_id",
        "risk_id",
        string="Linked Risks",
    )
    ...
    procedure_ids = fields.One2many(
        "qaco.exec.substantive.procedure", "area_id", string="Procedures"
    )

    def _compute_exception_count(self):
        for record in self:
            record.exception_count = len(
                record.procedure_ids.filtered(lambda p: p.result == "exception")
            )
...
class ExecutionSubstantiveProcedure(models.Model):
    ...
    execution_phase_id = fields.Many2one(
        related="area_id.execution_phase_id", store=False
    )
    risk_line_id = fields.Many2one(
        "qaco.exec.risk.coverage", string="Linked Risk", ondelete="set null"
    )
    ...
    company_currency_id = fields.Many2one(
        "res.currency",
        related="execution_phase_id.company_currency_id",
        store=False,
        readonly=True,
    )
    exception_amount = fields.Monetary(
        string="Exception Amount", currency_field="company_currency_id"
    )
    sampling_request_id = fields.Many2one(
        "qaco.exec.sampling.request", string="Sampling Request", ondelete="set null"
    )
    evidence_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_exec_substantive_evidence_rel",
        "procedure_id",
        "attachment_id",
        string="Evidence",
    )
    ...
    status = fields.Selection(
        SUBTAB_STATUS, string="Status", default="amber", tracking=True
    )
...
class ExecutionSamplingRequest(models.Model):
    ...
    name = fields.Char(
        string="Reference", required=True, default=lambda self: _("Sampling Request")
    )
    execution_phase_id = fields.Many2one(
        "qaco.execution.phase",
        string="Execution Phase",
    ...
    control_test_id = fields.Many2one(
        "qaco.exec.control.test", string="Control Test", ondelete="set null"
    )
    substantive_procedure_id = fields.Many2one(
        "qaco.exec.substantive.procedure",
        string="Substantive Procedure",
        ondelete="set null",
    )
    population_description = fields.Char(string="Population Description", required=True)
    population_amount = fields.Monetary(
        string="Population Value", currency_field="company_currency_id"
    )
    ...
    method = fields.Selection(
        SAMPLING_METHODS, string="Sampling Method", default="random"
    )
    sample_size = fields.Integer(
        string="Sample Size", compute="_compute_sample_size", store=True
    )
...
class ExecutionEvidenceItem(models.Model):
    ...
    evidence_type = fields.Selection(
        EVIDENCE_TIERS, string="Evidence Type", required=True, default="external"
    )
    reliability_score = fields.Integer(
        string="Reliability Score", compute="_compute_reliability", store=True
    )
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_exec_evidence_rel",
        "evidence_id",
        "attachment_id",
        string="Attachments",
        required=True,
...
class ExecutionWorkpaper(models.Model):
    ...
    balance_amount = fields.Monetary(
        string="Final Balance", currency_field="company_currency_id"
    )
    company_currency_id = fields.Many2one(
        "res.currency", default=lambda self: self.env.company.currency_id
    )
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_exec_workpaper_rel",
        "workpaper_id",
        "attachment_id",
        string="Supporting Files",
    )
    preparer_id = fields.Many2one(
        "res.users", string="Preparer", default=lambda self: self.env.user
    )
    prepared_on = fields.Datetime(string="Prepared On", default=fields.Datetime.now)
    reviewer_id = fields.Many2one("res.users", string="Reviewer")
    reviewed_on = fields.Datetime(string="Reviewed On")
    review_note_ids = fields.One2many(
        "qaco.exec.workpaper.note", "workpaper_id", string="Review Notes"
    )
    has_open_notes = fields.Boolean(
        string="Open Review Notes", compute="_compute_open_notes", store=True
    )
    locked = fields.Boolean(string="Locked", default=False)

    def _compute_open_notes(self):
        for record in self:
            record.has_open_notes = any(
                note.state == "open" for note in record.review_note_ids
            )
...
class ExecutionWorkpaperNote(models.Model):
    ...
    workpaper_id = fields.Many2one(
        "qaco.exec.workpaper", string="Workpaper", required=True, ondelete="cascade"
    )
...
class ExecutionPhase(models.Model):
    ...
    name = fields.Char(
        string="Reference",
        required=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("execution.phase")
        or "New",
    )
    client_id = fields.Many2one(
        "res.partner",
    ...
    audit_lead_id = fields.Many2one(
        "res.users", string="Audit Lead", default=lambda self: self.env.user
    )
    date_start = fields.Date(string="Start Date", default=fields.Date.today)
    date_end = fields.Date(string="End Date")
    fiscal_year = fields.Char(
        string="Fiscal Year", compute="_compute_fiscal_year", store=True
    )
    ...
    head_details_ids = fields.One2many(
        "execution.head.details", "execution_phase_id", string="Head-wise Details"
    )
    state = fields.Selection(
        [
            ("draft", "Draft"),
    ...
    overall_progress = fields.Float(
        string="Overall Progress", compute="_compute_overall_progress"
    )
    completed_heads_count = fields.Integer(
        string="Completed Heads", compute="_compute_progress_metrics"
    )
    total_heads_count = fields.Integer(
        string="Total Heads", compute="_compute_progress_metrics"
    )
    ...
    def _compute_progress_metrics(self):
        for record in self:
            ...
            record.completed_heads_count = len(
                record.head_details_ids.filtered(lambda h: h.state == "completed")
            )
    ...
    def action_confirm_heads(self):
        for record in self:
            if not record.selected_head_ids:
                raise UserError(
                    "Please select at least one accounting head to proceed."
                )
...
class ExecutionHeadDetails(models.Model):
    ...
    execution_phase_id = fields.Many2one(
        "execution.phase", string="Execution Phase", required=True, ondelete="cascade"
    )
    account_head_id = fields.Many2one(
        "account.account", string="Accounting Head", required=True
    )
    account_code = fields.Char(
        string="Account Code", related="account_head_id.code", store=False
    )
    ...
    variance_amount = fields.Float(
        string="Variance Amount", compute="_compute_variance", digits=(16, 2)
    )
    variance_percentage = fields.Float(
        string="Variance Percentage", compute="_compute_variance", digits=(16, 2)
    )
    ...
    coverage_percentage = fields.Float(
        string="Coverage Percentage", compute="_compute_coverage", digits=(16, 2)
    )
    tested_percentage = fields.Float(
        string="Tested Percentage",
        compute="_compute_tested_percentage",
        digits=(16, 2),
        store=True,
    )
    fully_tested = fields.Boolean(
        string="Fully Tested", compute="_compute_fully_tested", store=True
    )
    ...
    overall_risk_assessment = fields.Selection(
        ...
        string="Overall Risk Assessment",
    ...
    audit_procedure_ids = fields.One2many(
        "audit.procedure", "head_details_id", string="Audit Procedures"
    )
    test_of_details_ids = fields.One2many(
        "test.of.details", "head_details_id", string="Test of Details"
    )
    test_of_controls_ids = fields.One2many(
        "test.of.controls", "head_details_id", string="Test of Controls"
    )
    evidence_ids = fields.One2many(
        "audit.evidence", "head_details_id", string="Evidences"
    )
    analytical_procedure_ids = fields.One2many(
        "execution.analytical.procedure",
        "head_details_id",
        string="Analytical Procedures",
    )
        ...
        for record in self:
            if record.previous_year_amount:
                record.variance_amount = (
                    record.current_year_amount - record.previous_year_amount
                )
                record.variance_percentage = (
                    record.variance_amount / record.previous_year_amount
                ) * 100
            else:
        ...
        for record in self:
            if record.population_amount:
                record.coverage_percentage = (
                    record.sample_amount_tested / record.population_amount
                ) * 100
            else:
    ...
    def _compute_nature(self):
        ...
        for record in self:
            record.nature = (
                mapping.get(record.account_head_id.account_type, False)
                if record.account_head_id
                else False
            )
    ...
    def _compute_progress(self):
        for record in self:
            if record.state == "completed":
                ...
                completed = len(
                    record.audit_procedure_ids.filtered(lambda p: p.is_completed)
                )
...
class AuditProcedure(models.Model):
    ...
    sequence = fields.Integer(string="Sequence", default=10)
    head_details_id = fields.Many2one(
        "execution.head.details", string="Head Details", ondelete="cascade"
    )
    head_execution_id = fields.Many2one(
        "audit.head.execution", string="Head Execution", ondelete="cascade"
    )
    procedure_template_id = fields.Many2one(
        "audit.procedure.template", string="Procedure Template"
    )
    procedure_name = fields.Char(string="Procedure Name", required=True)
    procedure_description = fields.Html(
        string="Description", related="procedure_template_id.description", store=False
    )
...
class ExecutionAnalyticalProcedure(models.Model):
    ...
    variance_cy_py = fields.Float(
        string="Variance CY vs PY", compute="_compute_variances", digits=(16, 2)
    )
    variance_cy_budget = fields.Float(
        string="Variance CY vs Budget", compute="_compute_variances", digits=(16, 2)
    )
    variance_percentage = fields.Float(
        string="Variance %", compute="_compute_variances", digits=(16, 2)
    )
    ...
    def _compute_variances(self):
        for record in self:
            record.variance_cy_py = (
                (record.current_year_amount - record.previous_year_amount)
                if record.previous_year_amount
                else 0.0
            )
            record.variance_cy_budget = (
                (record.current_year_amount - record.budget_amount)
...
class AuditExecutionMaster(models.Model):
    ...
    audit_engagement_id = fields.Many2one(
        "qaco.audit.engagement", string="Audit Engagement"
    )
    audit_lead_id = fields.Many2one(
        "res.users", string="Audit Lead", default=lambda self: self.env.user
    )
    ...
    risk_coverage = fields.Float(
        string="Risk Coverage %", compute="_compute_risk_coverage", store=True
    )
    testing_completion = fields.Float(
        string="Testing Completion %", compute="_compute_testing_completion", store=True
    )
    selected_head_ids = fields.Many2many(
        "account.account",
        string="Selected Accounting Heads",
    ...
    head_execution_ids = fields.One2many(
        "audit.head.execution", "master_id", string="Head-wise Executions"
    )
    total_heads_count = fields.Integer(
        string="Total Heads", compute="_compute_summary_metrics", store=True
    )
    completed_heads_count = fields.Integer(
        string="Completed Heads", compute="_compute_summary_metrics", store=True
    )
    high_risk_heads_count = fields.Integer(
        string="High Risk Heads", compute="_compute_summary_metrics", store=True
    )
    significant_risks_count = fields.Integer(
        string="Significant Risks", compute="_compute_summary_metrics", store=True
    )
    ...
    def _compute_overall_progress(self):
        for record in self:
            if record.head_execution_ids:
                total_progress = sum(
                    head.progress for head in record.head_execution_ids
                )
                record.overall_progress = total_progress / len(
                    record.head_execution_ids
                )
            else:
    ...
    def _compute_risk_coverage(self):
        for record in self:
            if record.head_execution_ids:
                total_coverage = sum(
                    head.risk_coverage for head in record.head_execution_ids
                )
        ...
        for record in self:
            if record.head_execution_ids:
                total_completion = sum(
                    head.testing_completion for head in record.head_execution_ids
                )
                record.testing_completion = total_completion / len(
                    record.head_execution_ids
                )
            else:
    ...
    def _compute_summary_metrics(self):
        for record in self:
            ...
            record.completed_heads_count = len(
                record.head_execution_ids.filtered(lambda h: h.state == "completed")
            )
            record.high_risk_heads_count = len(
                record.head_execution_ids.filtered(
                    lambda h: h.risk_rating in ["high", "significant"]
                )
            )
            record.significant_risks_count = len(
                record.head_execution_ids.filtered(lambda h: h.significant_risk)
            )
    ...
    def action_confirm_planning(self):
        for record in self:
            if not record.selected_head_ids:
                raise UserError(
                    "Please select accounting heads before confirming planning!"
                )
...
class AuditHeadExecution(models.Model):
    ...
    assertion_ids = fields.Many2many(
        "audit.assertion", string="Assertions", required=True
    )
    risk_rating = fields.Selection(
        [
            ("low", "🟢 Low"),
    ...
    sample_amount_tested = fields.Float(string="Sample Amount Tested", digits=(16, 2))
    coverage_percentage = fields.Float(
        string="Coverage %", compute="_compute_coverage_percentage"
    )
    state = fields.Selection(
        [
            ("draft", "📝 Not Started"),
    ...
    progress = fields.Float(string="Progress %", compute="_compute_progress")
    risk_coverage = fields.Float(
        string="Risk Coverage %", compute="_compute_risk_coverage"
    )
    testing_completion = fields.Float(
        string="Testing Completion %", compute="_compute_testing_completion"
    )
    procedure_ids = fields.One2many(
        "audit.procedure", "head_execution_id", string="Audit Procedures"
    )
    test_of_controls_ids = fields.One2many(
        "test.of.controls", "head_execution_id", string="Test of Controls"
    )
    test_of_details_ids = fields.One2many(
        "test.of.details", "head_execution_id", string="Test of Details"
    )
    analytical_ids = fields.One2many(
        "execution.analytical.procedure",
        "head_execution_id",
        string="Analytical Procedures",
    )
    evidence_ids = fields.One2many(
        "audit.evidence", "head_execution_id", string="Evidences"
    )
    ...
    def _compute_nature(self):
        ...
        for record in self:
            record.nature = (
                mapping.get(record.account_head_id.user_type_id.type, False)
                if record.account_head_id
                else False
            )
    ...
    def _compute_risk_coverage(self):
        for record in self:
            if record.test_of_details_ids:
                total_coverage = sum(
                    test.coverage_percentage for test in record.test_of_details_ids
                )
    ...
    @api.depends(
        "procedure_ids.is_completed", "test_of_details_ids", "test_of_controls_ids"
    )
    def _compute_testing_completion(self):
        for record in self:
            total_tests = (
                len(record.procedure_ids)
                + len(record.test_of_details_ids)
                + len(record.test_of_controls_ids)
            )
            if total_tests > 0:
                completed_tests = (
                    len(record.procedure_ids.filtered(lambda p: p.is_completed))
                    + len(
                        record.test_of_details_ids.filtered(
                            lambda t: t.test_result == "satisfactory"
                        )
                    )
                    + len(
                        record.test_of_controls_ids.filtered(
                            lambda c: c.control_effectiveness == "effective"
                        )
                    )
                )
