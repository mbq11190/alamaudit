...
class AuditExecution(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("audit.execution")
        or "New",
    )
    ...
    risk_description = fields.Html(
        ...
        placeholder="<p>Describe the specific risks associated with this account...</p>",
    )
    relying_on_controls = fields.Boolean(
        string="Relying on Entity Controls", default=False
    )
    ...
    total_population_amount = fields.Float(
        string="Total Population Amount", digits=(16, 2)
    )
    tested_amount = fields.Float(string="Tested Amount", digits=(16, 2))
    tested_percentage = fields.Float(
        string="Tested %", compute="_compute_tested_percentage"
    )
    ...
    audit_procedure_ids = fields.One2many(
        "audit.procedure.detail", "execution_id", string="Audit Procedures"
    )
    test_of_controls_ids = fields.One2many(
        "test.of.controls.detail", "execution_id", string="Test of Controls"
    )
    test_of_details_ids = fields.One2many(
        "test.of.details.detail", "execution_id", string="Test of Details"
    )
    analytical_procedure_ids = fields.One2many(
        "analytical.procedure.detail", "execution_id", string="Analytical Procedures"
    )
    evidence_ids = fields.One2many(
        "audit.evidence.detail", "execution_id", string="Evidences"
    )
        ...
        for record in self:
            if record.total_population_amount:
                record.tested_percentage = (
                    record.tested_amount / record.total_population_amount
                ) * 100
            else:
    ...
    def _compute_overall_progress(self):
        for record in self:
            if record.state == "completed":
                ...
                completed = len(
                    record.audit_procedure_ids.filtered(lambda proc: proc.is_completed)
                )
    ...
    def _check_amounts(self):
        for record in self:
            ...
            if (
                record.total_population_amount
                and record.tested_amount > record.total_population_amount
            ):
                raise ValidationError(
                    "Tested amount cannot exceed total population amount."
                )
...
class AuditProcedureDetail(models.Model):
    ...
    name = fields.Char(
        ...
        default=lambda self: self.env["ir.sequence"].next_by_code(
            "audit.procedure.detail"
        )
        or "New",
    )
    sequence = fields.Integer(string="Sequence", default=10)
    execution_id = fields.Many2one(
        "audit.execution", string="Audit Execution", required=True, ondelete="cascade"
    )
    procedure_template_id = fields.Many2one(
        "audit.procedure.template", string="Procedure Template", required=True
    )
    procedure_name = fields.Char(
        string="Procedure Name", related="procedure_template_id.name", store=False
    )
    ...
    procedure_type = fields.Selection(
        ...
        related="procedure_template_id.procedure_type",
        store=False,
    )
...
class TestOfControlsDetail(models.Model):
    ...
    deviation_rate = fields.Float(
        string="Deviation Rate", compute="_compute_deviation_rate"
    )
    ...
    def _compute_deviation_rate(self):
        for record in self:
            if record.sample_size:
                record.deviation_rate = (
                    record.exceptions_found / record.sample_size
                ) * 100
            else:
...
class TestOfDetailsDetail(models.Model):
    ...
    name = fields.Char(
        string="Reference",
        required=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("test.details.detail")
        or "New",
    )
    sequence = fields.Integer(string="Sequence", default=10)
    execution_id = fields.Many2one(
    ...
    assertion_tested = fields.Many2many(
        "audit.assertion", string="Assertions Tested", required=True
    )
        ...
        for record in self:
            if record.population_amount:
                record.coverage_percentage = (
                    record.sample_amount / record.population_amount
                ) * 100
            else:
...
class AnalyticalProcedureDetail(models.Model):
    ...
    variance_cy_py = fields.Float(
        string="Variance CY vs PY", compute="_compute_variances"
    )
    variance_cy_budget = fields.Float(
        string="Variance CY vs Budget", compute="_compute_variances"
    )
    variance_percentage = fields.Float(
        string="Variance %", compute="_compute_variances"
    )
