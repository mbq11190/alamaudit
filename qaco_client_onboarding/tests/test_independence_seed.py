...
class TestIndependenceSeed(TransactionCase):
    ...
    def test_safeguard_master_records_loaded(self):
        codes = ['SEPARATE_TEAM', 'SECOND_PARTNER', 'ROTATION', 'INDEPENDENT_CONSULT', 'SUPERVISION', 'DISCONTINUE_SERVICE', 'CLIENT_GOVERNANCE', 'WITHDRAWAL']
        found = self.Safeguard.search([('code', 'in', codes)])
        found_codes = set(found.mapped('code'))
        for c in codes:
            self.assertIn(c, found_codes, 'Expected safeguard code %s to be present' % c)

    def test_gift_threshold_seeded_in_ir_config(self):
        val = self.Icp.get_param('qaco_client_onboarding.gift_auto_decline_threshold', default=None)
        self.assertIsNotNone(val, 'Expected gift auto-decline threshold to be seeded in ir.config_parameter')
    ...
    def test_onboarding_settings_view_and_action_present(self):
        view = self.env.ref('qaco_client_onboarding.view_res_config_settings_onboarding', raise_if_not_found=False)
        self.assertTrue(view, 'Onboarding settings view should exist')
        action = self.env.ref('qaco_client_onboarding.action_onboarding_settings', raise_if_not_found=False)
        ...
        menu = self.env.ref('qaco_client_onboarding.menu_onboarding_settings', raise_if_not_found=False)
        ...
        if parent:
            ...
            xmlid = None
        self.assertEqual(xmlid, 'qaco_client_onboarding.menu_admin_audit', 'Onboarding menu parent should be the Audit settings group')
        # audit menu should have an icon and a smaller sequence
        audit_menu = self.env.ref('qaco_client_onboarding.menu_admin_audit', raise_if_not_found=False)
