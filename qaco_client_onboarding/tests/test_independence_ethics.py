...
class TestIndependenceEthics(TransactionCase):
    def setUp(self):
        ...
        self.onboarding = self.Onboarding.create({'audit_id': audit.id, 'legal_name': 'IndClient', 'principal_business_address': 'Addr', 'business_registration_number': 'I123'})

    def test_missing_declarations_block_approval(self):
        # Create a pending declaration
        self.Decl.create({'onboarding_id': self.onboarding.id, 'user_id': self.env.uid, 'role': 'Partner', 'status': 'pending'})
    ...
    def test_unresolved_threat_blocks_approval(self):
        ...
        self.Decl.create({'onboarding_id': self.onboarding.id, 'user_id': self.env.uid, 'role': 'Partner', 'status': 'approved'})
        # Add a high likelihood/impact threat unresolved
        self.Threat.create({'onboarding_id': self.onboarding.id, 'category': 'self_review', 'likelihood': 'high', 'impact': 'high', 'resolved': False})
        self.onboarding.action_submit_review()
        with self.assertRaises(exceptions.ValidationError):
            self.onboarding.action_partner_approve()
    ...
    def test_resolved_threat_allows_approval(self):
        ...
        self.Decl.create({'onboarding_id': self.onboarding.id, 'user_id': self.env.uid, 'role': 'Partner', 'status': 'approved'})
        t = self.Threat.create({'onboarding_id': self.onboarding.id, 'category': 'self_review', 'likelihood': 'high', 'impact': 'high', 'resolved': False})
        # Resolve threat
        t.resolved = True
