...
class TestOnboardingTemplate(TransactionCase):
    ...
    def test_mandatory_hard_stop_constraint(self):
        ...
        with self.assertRaises(exceptions.ValidationError):
            self.Template.create({
                ...
                'category_id': self.env['qaco.onboarding.template.category'].create({'name': 'T', 'code': 'T'}).id,
    ...
    def test_attach_indexes_document_vault(self):
        ...
        onboarding = self.Onboarding.create({'name': 'T', 'audit_id': self.env['qaco.audit'].create({'name': 'A', 'client_id': client.id, 'firm_name': self.env['audit.firm.name'].create({'name': 'F', 'code': 'F'}).id}).id})
        cat = self.env['qaco.onboarding.template.category'].create({'name': 'T2', 'code': 't2'})
        tpl = self.Template.create({'name': 'Sample', 'category_id': cat.id, 'template_file': b'test', 'template_filename': 'sample.pdf'})
