...
class TestEngagementDecision(TransactionCase):
    ...
    def test_acceptance_flow_creates_decision_and_memo(self):
        ...
        self.env["qaco.onboarding.independence.declaration"].create(
            {
                "onboarding_id": self.onboarding.id,
        ...
        # create decision
        dec = self.Decision.create(
            {
                "onboarding_id": self.onboarding.id,
                "decision_type": "accept",
                "decision_rationale": "Clear",
                "risk_rating": "low",
            }
        )
        self.assertEqual(dec.version, 1)
        # create superseding decision
        dec2 = self.Decision.create(
            {
                "onboarding_id": self.onboarding.id,
                "decision_type": "accept_conditions",
                "decision_rationale": "Conditional",
            ...
            self.Decision.create(
                {
                    "onboarding_id": self.onboarding.id,
                    "decision_type": "accept",
                    "decision_rationale": "Bad",
                    "risk_rating": "prohibitive",
                }
            )
        # With quality approval it should allow
        q = self.env["res.users"].create(
            {"name": "Quality", "login": "qual@example.com"}
        )
        dec = self.Decision.create(
            {
                "onboarding_id": self.onboarding.id,
                "decision_type": "accept",
                "decision_rationale": "Approved by quality",
                "risk_rating": "prohibitive",
                "quality_approval_id": q.id,
            }
        )
