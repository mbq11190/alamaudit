...
class TestAttachWizard(TransactionCase):
    def setUp(self):
        ...
        self.onboarding = self.Onboarding.create(
            {
                ...
                .create({"client_id": self.env.ref("base.res_partner_1").id})
                .id,
                "entity_type": "pic",
                "legal_name": "Test Co",
        ...
        self.tpl1 = self.Template.create(
            {
                "name": "T1",
                "category_id": self.env["qaco.onboarding.template.category"]
                .create({"name": "C", "code": "C1"})
                .id,
                "template_file": b"foo",
                "template_filename": "foo.docx",
            }
        )
        self.tpl2 = self.Template.create(
            {"name": "T2", "category_id": self.tpl1.category_id.id}
        )

    def test_action_open_attach_wizard_with_templates(self):
        action = self.onboarding.action_open_attach_wizard_with_templates(
            [self.tpl1.id, self.tpl2.id]
        )
        ...
        self.assertEqual(
            sorted(default_templates), sorted([self.tpl1.id, self.tpl2.id])
        )
    ...
    def test_wizard_attach_creates_attached(self):
        wiz = self.env["qaco.onboarding.attach.templates.wizard"].create(
            {
                "onboarding_id": self.onboarding.id,
                "template_ids": [(6, 0, [self.tpl1.id, self.tpl2.id])],
            }
        )
        wiz.action_attach()
        ...
        wiz2 = self.env["qaco.onboarding.attach.templates.wizard"].create(
            {
                "onboarding_id": self.onboarding.id,
                "template_ids": [(6, 0, [self.tpl1.id, self.tpl2.id])],
            }
