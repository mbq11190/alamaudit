...
class TestAuditTrail(TransactionCase):
    ...
    def test_log_action_creates_entry(self):
        ...
        entries = self.onboarding.audit_trail_ids.filtered(
            lambda t: t.action == "test_action"
        )
    ...
    def test_override_logging(self):
        self.onboarding._log_action(
            "override_action",
            notes="override noted",
            action_type="override",
            is_override=True,
            resolution="Resolved by partner",
        )
        entries = self.onboarding.audit_trail_ids.filtered(
            lambda t: t.action == "override_action"
        )
