...
class TestDocumentVault(TransactionCase):
    ...
    def test_folder_autocreation(self):
        self.assertTrue(
            self.onboarding.document_folder_ids,
            "Default document folders should be created on onboarding creation",
        )
    ...
    def test_predecessor_pack_indexed_to_folder(self):
        ...
        req.action_generate_pack_bundle()
        ...
        docs = self.env["qaco.onboarding.document"].search(
            [("onboarding_id", "=", self.onboarding.id), ("folder_id", "=", folder.id)]
        )
