...
class TestRegulatoryCompliance(TransactionCase):
    def setUp(self):
        ...
        self.onboarding = self.Onboarding.create({'audit_id': audit.id, 'legal_name': 'RegClient', 'principal_business_address': 'Addr', 'business_registration_number': '123'})
    ...
    def test_required_document_populate_and_upload_index(self):
        ...
        if not cat:
            cat = self.env['qaco.onboarding.template.category'].create({'name': 'Regulatory', 'code': 'regulatory'})
        tpl = self.env['qaco.onboarding.template.document'].create({'name': 'Reg Profile', 'category_id': cat.id, 'stage': 'pre_onboarding', 'mandatory': 'yes', 'file_type': 'docx'})
        # populate required documents
        self.onboarding.populate_required_documents_from_templates()
        self.assertTrue(self.onboarding.required_document_line_ids, 'Required documents should be populated from templates')
        r = self.onboarding.required_document_line_ids.filtered(lambda l: l.template_id.id == tpl.id)
        # upload a file and verify it gets indexed into Document Vault
        r.write({'uploaded_file': b'1234', 'uploaded_filename': 'evidence.pdf', 'status': 'received'})
        docs = self.Doc.search([('onboarding_id','=',self.onboarding.id), ('file_name','=','evidence.pdf')])
        self.assertTrue(docs, 'Uploaded required document should be indexed into Document Vault')
