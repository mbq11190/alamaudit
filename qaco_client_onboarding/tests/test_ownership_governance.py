...
class TestOwnershipGovernance(TransactionCase):
    def setUp(self):
        ...
        self.onboarding = self.Onboarding.create({'audit_id': audit.id, 'legal_name': 'OwnerClient', 'principal_business_address': 'Addr', 'business_registration_number': '123'})
    ...
    def test_shareholding_sum_constraint(self):
        ...
        self.Shareholder.create({'onboarding_id': self.onboarding.id, 'name': 'A', 'percentage': 60})
        self.Shareholder.create({'onboarding_id': self.onboarding.id, 'name': 'B', 'percentage': 30})
    ...
    def test_ubo_basis_required_when_marked_complete(self):
        ...
        u = self.UBO.create({'onboarding_id': self.onboarding.id, 'name': 'UBO1', 'ownership_percent': 100})
        with self.assertRaises(exceptions.ValidationError):
    ...
    def test_required_document_upload_indexes_into_document_vault(self):
        ...
        if not cat:
            cat = self.env['qaco.onboarding.template.category'].create({'name': 'Ownership', 'code': 'ownership'})
        tpl = self.env['qaco.onboarding.template.document'].create({'name': 'Owner Profile', 'category_id': cat.id, 'stage': 'pre_onboarding', 'mandatory': 'yes', 'file_type': 'docx'})
        ...
        r.write({'uploaded_file': b'pdfcontent', 'uploaded_filename': 'owner.pdf', 'status': 'received'})
        docs = self.Doc.search([('onboarding_id','=',self.onboarding.id), ('file_name','=','owner.pdf')])
