...
def pre_init_hook(cr):
    ...
    try:
        ...
        try:
            ...
            cr.execute(
                "CREATE TABLE IF NOT EXISTS backup_problematic_ir_model_data AS SELECT * FROM ir_model_data WHERE false"
            )
        except Exception:
            _logger.debug(
                "Could not ensure backup tables exist; continuing anyway", exc_info=True
            )

        # Copy problematic views into backup table (idempotent insert behaviour handled by DB uniqueness if needed)
        try:
            insert_views_sql = (
                "INSERT INTO backup_problematic_ir_ui_view SELECT v.* FROM ir_ui_view v WHERE "
                + like_clauses
            )
            cr.execute(insert_views_sql)
        except Exception:
            _logger.debug(
                "Failed to copy problematic ir_ui_view rows into backup table",
                exc_info=True,
            )
        ...
        try:
            specific_view = env.ref(
                "qaco_client_onboarding.view_client_onboarding_form_templates_section",
                raise_if_not_found=False,
            )
            if (
                specific_view
                and getattr(specific_view, "id", False)
                and specific_view.id not in view_ids
            ):
                view_ids.append(specific_view.id)
                try:
                    cr.execute(
                        "INSERT INTO backup_problematic_ir_ui_view SELECT v.* FROM ir_ui_view v WHERE id=%s",
                        (specific_view.id,),
                    )
                except Exception:
                    _logger.debug(
                        "Failed to back up specific templates view", exc_info=True
                    )
        except Exception:
        ...
        if view_ids:
            _logger.info(
                "Pre-init: found %d problematic views, deactivating and backing up",
                len(view_ids),
            )
            ...
            try:
                ...
                _logger.debug(
                    "Failed to copy problematic ir.model.data rows into backup table",
                    exc_info=True,
                )
            ...
            try:
                ...
                _logger.debug(
                    "Failed to remove ir.model.data entries for problematic views",
                    exc_info=True,
                )

        # Lastly, remove any stray model-data entries created by earlier fix attempts that match common fix names
        try:
            stale_md = env["ir.model.data"].search(
                [
                    ("module", "=", "qaco_client_onboarding"),
                    ("name", "ilike", "fix%attach%"),
                ]
            )
            if stale_md:
                _logger.info(
                    "Pre-init: removing %d stale ir.model.data records created by earlier fixes",
                    len(stale_md),
                )
                for md in stale_md:
                    try:
            ...
            _logger.debug(
                "Error while cleaning stale ir.model.data by name", exc_info=True
            )
        ...
        try:
            ...
            _logger.debug(
                "Could not ensure backup table for ir.model.fields exists; continuing",
                exc_info=True,
            )
        try:
            cr.execute(
                "SELECT id FROM ir_model_fields f WHERE f.relation IS NOT NULL AND f.relation NOT IN (SELECT model FROM ir_model)"
            )
            missing_fields = [r[0] for r in cr.fetchall()]
            if missing_fields:
                _logger.info(
                    "Pre-init: found %d ir.model.fields with missing relation model; backing up and removing",
                    len(missing_fields),
                )
                try:
                    cr.execute(
                    ...
                    _logger.debug(
                        "Failed to backup problematic ir.model.fields rows",
                        exc_info=True,
                    )
                try:
                    cr.execute(
                    ...
                    _logger.debug(
                        "Failed to delete problematic ir.model.fields rows",
                        exc_info=True,
                    )
        except Exception:
            _logger.exception(
                "Error while checking for problematic ir.model.fields (continuing)"
            )
            ...
            for vid, model_name, arch in cr.fetchall():
                try:
                    ...
                    for t in trees:
                        ...
                        for token in [
                            x.strip() for x in default_order.split(",") if x.strip()
                        ]:
                            fname = token.split()[0]
                            if fname not in md_fields:
                                invalid = True
                                _logger.warning(
                                    "Pre-init: view %s Default order references unknown field %s on model %s",
                                    vid,
                                    fname,
                                    model_name,
                                )
                    ...
                    _logger.debug(
                        "Failed to parse view arch for view id %s", vid, exc_info=True
                    )
            if bad_views:
                _logger.info(
                    "Pre-init: backing up and deactivating %d views due to invalid default_order",
                    len(bad_views),
                )
                try:
                    cr.execute(
                        "INSERT INTO backup_problematic_ir_ui_view SELECT v.* FROM ir_ui_view v WHERE id IN %s",
                        (tuple(bad_views),),
                    )
                except Exception:
                    _logger.debug(
                        "Failed to back up problematic views with invalid default_order",
                        exc_info=True,
                    )
                try:
                    cr.execute(
                    ...
                    _logger.debug(
                        "Failed to deactivate problematic views with invalid default_order",
                        exc_info=True,
                    )
                try:
                    cr.execute(
                    ...
                    _logger.debug(
                        "Failed to remove ir.model.data entries for problematic views with invalid default_order",
                        exc_info=True,
                    )
        except Exception:
            _logger.exception(
                "Error while scanning views for invalid default_order (continuing)"
            )
        ...
        try:
            ...
            cr.execute(
                "SELECT f.id, f.model, f.name, f.relation, f.inverse_name FROM ir_model_fields f WHERE f.ttype='one2many' AND (f.inverse_name IS NULL OR f.inverse_name = '' OR NOT EXISTS (SELECT 1 FROM ir_model_fields rf WHERE rf.model = f.relation AND rf.name = f.inverse_name))"
            )
            rows = cr.fetchall()
            if rows:
                ids = [r[0] for r in rows]
                _logger.info(
                    "Pre-init: found %d One2many fields with missing inverse on relation model; backing up and removing",
                    len(ids),
                )
                try:
                    cr.execute(
                    ...
                    _logger.debug(
                        "Failed to backup problematic One2many ir.model.fields",
                        exc_info=True,
                    )
                try:
                    cr.execute(
                        "DELETE FROM ir_model_fields WHERE id IN %s", (tuple(ids),)
                    )
                except Exception:
                    _logger.debug(
                        "Failed to delete problematic One2many ir.model.fields",
                        exc_info=True,
                    )
        except Exception:
            _logger.exception(
                "Error while checking One2many inverse consistency (continuing)"
            )
    except Exception:
...
def sanitize_view_labels(cr, env):
    ...
    try:
        ...
        rows = cr.fetchall()
        fixed_views = []
        for vid, name, module, arch_db in rows:
            if not arch_db:
                continue
            try:
                ...
                if changed:
                    ...
                    try:
                        ...
                        try:
                            ...
                            _logger.exception(
                                "Failed to persist sanitized view arch for view %s (id=%s)",
                                name,
                                vid,
                            )
            except Exception:
