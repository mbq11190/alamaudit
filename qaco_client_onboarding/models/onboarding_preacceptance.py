...
class OnboardingSanctionsScreen(models.Model):
    ...
    screening_method = fields.Selection([('manual','Manual'),('tool','Tool'),('third_party','Third party')], string='Screening method')
    ...
    screening_conclusion = fields.Selection([('clear','Clear'),('escalate','Escalate'),('block','Block')], string='Screening conclusion')
    ...
    def _check_sanctions_block(self):
        for rec in self:
            if rec.screening_conclusion == 'block' and not rec.resolution_notes:
                raise ValidationError(_('When screening conclusion is Block, provide resolution notes.'))
...
class OnboardingFeeTerms(models.Model):
    ...
    onboarding_id = fields.Many2one('qaco.client.onboarding', required=True, ondelete='cascade', index=True)
    total_fee = fields.Monetary(string='Total fee (budget)', currency_field='currency_id')
    ...
    billing_frequency = fields.Selection([('monthly','Monthly'),('phase','Phase-wise'),('other','Other')], string='Billing frequency')
    ...
    outstanding_fees_prior_auditor = fields.Monetary(string='Outstanding fees (prior auditor)', currency_field='currency_id')
    dispute_probability = fields.Selection([('low','Low'),('medium','Medium'),('high','High')], string='Dispute probability')
    collection_risk_rating = fields.Selection(RISK_RATING, string='Collection risk rating')
...
class OnboardingConditionalAcceptance(models.Model):
    ...
    status = fields.Selection([('open','Open'),('in_progress','In progress'),('closed','Closed')], string='Status', default='open')
...
class ClientOnboardingPreAcceptance(models.Model):
    ...
    integrity_history_noncompliance = fields.Boolean(string='History of non-compliance / late filings')
    integrity_aggressive_accounting = fields.Boolean(string='Aggressive accounting positions / restatements')
    integrity_scope_limitations = fields.Boolean(string='Management unwilling to provide information / scope limitations expected')
    integrity_prior_auditor_disagreements = fields.Boolean(string='Prior auditor disagreements / resignation / qualifications')
    integrity_litigation_patterns = fields.Boolean(string='Litigation / fraud / whistleblowing')
    integrity_adverse_media = fields.Boolean(string='Adverse media / reputational issues')
    prior_auditor_issues = fields.Text(string='Prior auditor issues description')
    outstanding_disputes = fields.Text(string='Outstanding disputes with auditors/consultants')
    management_attitude = fields.Text(string='Management attitude to internal control and governance')
    ...
    # Sanctions / PEP
    sanctions_screen_performed = fields.Boolean(string='Sanctions / PEP screening performed')
    sanctions_screen_ids = fields.One2many('qaco.onboarding.sanctions.screen', 'onboarding_id', string='Sanctions / PEP screenings')
    adverse_media_ids = fields.One2many('qaco.onboarding.adverse.media', 'onboarding_id', string='Adverse media evidence')
    ...
    complexity_related_party = fields.Boolean(string='Significant related parties / unusual transactions')
    complexity_group = fields.Boolean(string='Group structure / component work expected')
    complexity_regulatory = fields.Boolean(string='Regulatory scrutiny / investigations')
    complexity_it = fields.Boolean(string='Significant IT reliance / ERP change')
    complexity_judgments = fields.Boolean(string='Significant judgments / first time adoption')
    ...
    # Scoring
    engagement_risk_rating = fields.Selection(RISK_RATING, string='Engagement risk rating')
    engagement_score = fields.Integer(string='Score (0-100)', compute='_compute_engagement_score', store=True)
    score_driver_ids = fields.One2many('qaco.onboarding.risk.driver', 'onboarding_id', string='Score drivers')
    score_override = fields.Boolean(string='Score override')
    score_override_reason = fields.Text(string='Override rationale')
    score_override_partner_id = fields.Many2one('res.users', string='Partner approving override')

    # Fee & collection
    fee_terms_id = fields.Many2one('qaco.onboarding.fee.terms', string='Fee terms')
    fee_terms_rel = fields.One2many('qaco.onboarding.fee.terms','onboarding_id', string='Fee terms history')

    # Acceptance decision & safeguards
    engagement_decision = fields.Selection([('accept','Accept'),('reject','Decline'),('conditions','Conditional')], string='Decision')
    ...
    conditional_line_ids = fields.One2many('qaco.onboarding.conditional.acceptance','onboarding_id', string='Conditional Acceptance Tracker')
    fee_term_ids = fields.One2many('qaco.onboarding.fee.terms', 'onboarding_id', string='Fee & Collection Terms')

    @api.depends('complexity_estimates','complexity_revenue','complexity_going_concern','complexity_related_party','complexity_group','complexity_regulatory','complexity_it','complexity_judgments')
    def _compute_engagement_score(self):
        """Simple weighted score: sum of trigger flags * 10 (capped 100)"""
        for rec in self:
            score = 0
            flags = [rec.complexity_estimates, rec.complexity_revenue, rec.complexity_going_concern, rec.complexity_related_party, rec.complexity_group, rec.complexity_regulatory, rec.complexity_it, rec.complexity_judgments]
    ...
    def _compute_eqcr_required(self):
        for rec in self:
            rec.eqcr_required = rec.engagement_risk_rating == 'high' or rec.engagement_decision == 'conditions' or bool(rec.first_year)
        ...
        for rec in self:
            if rec.state in ('partner_approved','locked'):
                ...
                if not rec.engagement_risk_rating:
                    raise ValidationError(_('Engagement risk rating must be set before partner approval.'))
                # AML obligations
                if rec.aml_applicable and (not rec.client_identity_verified or not rec.ubo_verified):
                    raise ValidationError(_('AML/KYC applicable but identity/UBO verification incomplete.'))
                ...
                for s in rec.sanctions_screen_ids:
                    if s.screening_conclusion == 'block' or s.sanctions_hit:
                        raise ValidationError(_('Sanctions / PEP screening blocked approval until resolved (see screening records).'))
                ...
                if ft and ft.collection_risk_rating == 'high' and not ft.retainer_amount:
                    raise ValidationError(_('High collection risk requires a retainer; set retainer before approval.'))
