...
class AuditStandardLibrary(models.Model):
    ...
    regulator_reference = fields.Selection(
        REGULATOR_REFERENCE, string="Regulator Reference", required=True, default="icap"
    )
...
class AuditOnboardingChecklistTemplate(models.Model):
    ...
    code = fields.Char(
        string="Code", help="Structured reference such as 1.1.1 or 1.2.1."
    )
    name = fields.Char(string="Checklist Requirement", required=True)
    onboarding_area = fields.Selection(
        ONBOARDING_AREAS, string="Onboarding Area", required=True
    )
...
class AuditOnboardingChecklist(models.Model):
    ...
    template_id = fields.Many2one(
        "audit.onboarding.checklist.template",
    ...
    standard_ids = fields.Many2many(
        "audit.standard.library", string="Standard Reference"
    )
    mandatory = fields.Boolean(string="Mandatory", default=True)
    ...
    def write(self, vals):
        ...
        if completed_flag is not None:
            newly_completed = self.filtered(
                lambda r: r.completed and not r.completed_on
            )
            if newly_completed:
