...
CONDITION_TYPES = [
    ...
    (
        "information_access",
        "Information access / management responsibility confirmations",
    ),
...
class FinalAuthorizationCondition(models.Model):
    ...
    authorization_id = fields.Many2one(
        "qaco.onboarding.final.authorization", ondelete="cascade"
    )
...
class FinalAuthorization(models.Model):
    ...
    decision_date = fields.Datetime(
        string="Decision date", required=True, default=fields.Datetime.now
    )
    approver_id = fields.Many2one(
        "res.users", string="Approver", default=lambda self: self.env.uid
    )
    digital_signature = fields.Binary(string="Digital signature")
    condition_ids = fields.One2many(
        "qaco.onboarding.final.authorization.condition",
        "authorization_id",
        string="Conditions",
    )
    ...
    created_by = fields.Many2one(
        "res.users", string="Created by", default=lambda self: self.env.uid
    )
...
class FinalAuthorizationWizard(models.TransientModel):
    ...
    onboarding_id = fields.Many2one(
        "qaco.client.onboarding", required=True, ondelete="cascade"
    )
    decision = fields.Selection(DECISION_SELECTION, required=True)
    decision_rationale = fields.Text(string="Decision rationale")
    eqcr_required = fields.Boolean(string="EQCR required")
    ...
    specialists_required = fields.Boolean(string="Specialists required")
    specialists_notes = fields.Text(string="Specialists notes")
    include_summary_pack = fields.Boolean(
        string="Generate Onboarding Summary Pack (bundle)", default=True
    )
    generate_certificate = fields.Boolean(
        string="Generate Completion Certificate", default=True
    )
    planning_policy = fields.Selection(
        [
            ("locked", "Lock Planning until conditions met"),
            ("partial", "Allow Planning; restrict execution until conditions met"),
        ],
        string="Conditional planning policy",
        default="locked",
    )
    ...
    def action_confirm_authorization(self):
        ...
        auth = self.env["qaco.onboarding.final.authorization"].create(
            {
                ...
                "eqcr_partner_id": (
                    self.eqcr_partner_id.id if self.eqcr_partner_id else False
                ),
        ...
        if self.decision == "decline":
            ...
            onboarding._log_action(
                "Final authorization: Declined", notes=self.decision_rationale
            )
            ...
            try:
                ...
                _logger.exception(
                    "Failed to generate decline memo for onboarding %s", onboarding.id
                )
        else:
            # Accept or conditional -> set partner_approved state
            onboarding.write({"state": "partner_approved"})
            onboarding._log_action(
                "Final authorization: Partner approved", notes=self.decision_rationale
            )
            ...
            if self.generate_certificate:
                try:
                    ...
                    if report:
                        ...
                        try:
                            folder = onboarding.get_folder_by_code(
                                "08_Final_Authorization"
                            )
                            if folder:
                            ...
                            _logger.exception(
                                "Failed to index acceptance certificate into folder for onboarding %s",
                                onboarding.id,
                            )
                except Exception:
                    _logger.exception(
                        "Failed to generate acceptance/certificate report for onboarding %s",
                        onboarding.id,
                    )
            if self.include_summary_pack:
                try:
                    ...
                    _logger.exception(
                        "Failed to generate onboarding summary pack for onboarding %s",
                        onboarding.id,
                    )
