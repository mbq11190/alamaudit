...
class OnboardingNote(models.Model):
    ...
    def _compute_has_draft_recovery(self):
        ...
        for record in self:
            if not record.draft_content:
                ...
                record.has_draft_recovery = (
                    record.last_auto_save_date > record.last_save_date
                )
    ...
    def _compute_navigation(self):
        ...
        for record in self:
            ...
            if current_idx >= 0:
                record.prev_note_id = (
                    siblings[current_idx - 1] if current_idx > 0 else False
                )
                record.next_note_id = (
                    siblings[current_idx + 1] if current_idx < len(ids) - 1 else False
                )
            else:
    ...
    def get_draft_recovery_info(self, note_id):
        ...
        if note.exists() and note.has_draft_recovery:
            return {
                ...
                "auto_save_date": (
                    note.last_auto_save_date.isoformat()
                    if note.last_auto_save_date
                    else None
                ),
                "last_save_date": (
                    note.last_save_date.isoformat() if note.last_save_date else None
                ),
            }
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "sequence" not in vals and vals.get("notebook_id"):
                notebook = self.env["qaco.onboarding.notebook"].browse(
                    vals["notebook_id"]
                )
