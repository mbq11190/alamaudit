...
class PredecessorRequest(models.Model):
    ...
    engagement_type = fields.Selection(
        [("audit", "Audit"), ("review", "Review"), ("other", "Other")],
    ...
    authorization_attachment = fields.Many2one(
        "ir.attachment", string="Authorization doc"
    )
    ...
    followup_line_ids = fields.One2many(
        "qaco.onboarding.predecessor.followup",
        "request_id",
        string="Follow-up attempts",
    )
    response_id = fields.Many2one(
        "qaco.onboarding.predecessor.response", string="Response"
    )
    ...
    checklist_professional_reasons = fields.Boolean(
        string="Professional reasons not to accept?"
    )
    ...
    checklist_access_papers = fields.Boolean(
        string="Access to prior working papers requested"
    )

    followup_count = fields.Integer(
        string="Follow-up attempts", compute="_compute_followup_count", store=True
    )
    ...
    def action_generate_pack_bundle(self):
        ...
        report = self.env.ref(
            "qaco_client_onboarding.report_predecessor_clearance_pack"
        )
        try:
        ...
        threshold_kb = int(
            ...
            .sudo()
            .get_param("qaco_client_onboarding.pack_attachment_max_kb", default="512")
        )
        threshold_bytes = threshold_kb * 1024
        candidates = []
        # Authorization doc on request
        if (
            self.authorization_attachment
            and self.authorization_attachment.mimetype == "application/pdf"
        ):
            candidates.append(self.authorization_attachment)
        # Response attachment
        if (
            self.response_id
            and self.response_id.attachment_id
            and self.response_id.attachment_id.mimetype == "application/pdf"
        ):
        ...
        if PdfMerger:
            ...
            _logger.warning(
                "No PDF merger library available (pypdf/PyPDF2); creating pack with base document only."
            )
        ...
        if skipped:
            notes = "\n".join(
                ["Skipped (size>threshold or non-PDF): %s" % (a.name,) for a in skipped]
            )
        ...
        try:
            ...
            _logger.exception(
                "Failed to index merged predecessor pack into folder for request %s",
                self.id,
            )
        return merged_att
...
class PredecessorFollowup(models.Model):
    ...
    request_id = fields.Many2one(
        "qaco.onboarding.predecessor.request", required=True, ondelete="cascade"
    )
...
class PredecessorResponse(models.Model):
    ...
    quality_approval = fields.Many2one(
        "res.users", string="Quality/Compliance approver"
    )
    ...
    def is_adverse(self):
        ...
        return bool(
            self.issue_integrity
            or self.issue_scope_limit
            or self.issue_fee_dispute
            or self.conclusion == "do_not_proceed"
        )
...
class ClientOnboardingPredecessor(models.Model):
    ...
    predecessor_escalated = fields.Boolean(
        string="Predecessor Escalated",
        compute="_compute_predecessor_escalated",
        store=True,
    )
    predecessor_locked = fields.Boolean(
        string="Predecessor locked (Do Not Proceed)", default=False
    )

    @api.depends("predecessor_response_ids")
    def _compute_predecessor_escalated(self):
        for rec in self:
            rec.predecessor_escalated = any(
                r.is_adverse() for r in rec.predecessor_response_ids
            )
            rec.predecessor_locked = any(
                r.conclusion == "do_not_proceed" for r in rec.predecessor_response_ids
            )
        ...
        for rec in self:
            ...
            if not rec.predecessor_request_ids:
                raise ValidationError(
                    _(
                        "A predecessor clearance request must be issued when a predecessor exists."
                    )
                )
            ...
            if not (has_response or has_followups):
                raise ValidationError(
                    _(
                        "A predecessor response or follow-up attempts must be recorded before partner approval."
                    )
                )
            # If there's an adverse response, escalate and block unless Quality approved
            if rec.predecessor_escalated and not any(
                r.quality_approval
                for r in rec.predecessor_response_ids
                if r.is_adverse()
            ):
                raise ValidationError(
                    _(
                        "Adverse predecessor response requires Quality/Compliance approval before partner approval."
                    )
                )
            ...
            if rec.predecessor_locked:
                raise ValidationError(
                    _(
                        'The predecessor response concluded "Do Not Proceed"; engagement creation/acceptance is blocked.'
                    )
                )
    ...
    def action_generate_predecessor_pack_bundle(self):
        ...
        if not self.predecessor_request_ids:
            raise ValidationError(
                _("No predecessor request exists to generate a pack from.")
            )
