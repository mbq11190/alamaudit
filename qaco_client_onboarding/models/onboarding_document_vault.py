...
class OnboardingDocumentFolder(models.Model):
    ...
    parent_id = fields.Many2one('qaco.onboarding.document.folder', string='Parent Folder')
    ...
    def _compute_doc_count(self):
        for f in self:
            f.doc_count = self.env['qaco.onboarding.document'].search_count([('folder_id', '=', f.id)])
...
class OnboardingDocumentHistory(models.Model):
    ...
    document_id = fields.Many2one('qaco.onboarding.document', required=True, ondelete='cascade', index=True)
...
class OnboardingDocumentAudit(models.Model):
    ...
    document_id = fields.Many2one('qaco.onboarding.document', required=True, ondelete='cascade', index=True)
    action = fields.Selection([('upload', 'Upload'), ('view', 'View'), ('download', 'Download'), ('rename', 'Rename'), ('move', 'Move'), ('supersede', 'Supersede'), ('delete', 'Delete'), ('restore', 'Restore')], required=True)
...
class OnboardingDocument(models.Model):
    ...
    sensitivity = fields.Selection(SENSITIVITY_LEVELS, string='Sensitivity', default='normal')
    version = fields.Integer(string='Version', default=1)
    state = fields.Selection(DOC_STATE, string='Document state', default='draft')
    checksum = fields.Char(string='SHA256 Checksum')
    file_size = fields.Integer(string='File size (bytes)')
    legal_hold = fields.Boolean(string='Legal hold', default=False, help='When set deletion/archival is prevented')
    retention_date = fields.Date(string='Retention until')
    retention_policy = fields.Char(string='Retention policy name')
    history_ids = fields.One2many('qaco.onboarding.document.history', 'document_id', string='Version history')
    audit_ids = fields.One2many('qaco.onboarding.document.audit', 'document_id', string='Document audit log')
        ...
        for rec in self:
            if rec.file:
                try:
                    ...
                    data = base64.b64decode(rec.file.decode('utf-8') if isinstance(rec.file, bytes) else rec.file or '')
    ...
    def unlink(self):
        for rec in self:
            if rec.legal_hold:
                raise ValidationError(_('Document is on legal hold and cannot be deleted.'))
            if rec.retention_date and rec.retention_date > date.today():
                raise ValidationError(_('Document retention policy prevents deletion until %s') % (rec.retention_date,))
    ...
    def action_set_legal_hold(self, flag=True):
        ...
        for rec in self:
            rec._record_audit('move' if flag else 'restore', notes=_('Legal hold set' if flag else 'Legal hold removed'))
        return True
