...
class AuditExportWizard(models.TransientModel):
    ...
    action_type = fields.Selection([('activity','Activity'),('override','Override'),('system','System'),('user','User')], string='Action type')
    user_id = fields.Many2one('res.users', string='User')
    include_overrides = fields.Boolean(string='Include overrides', default=True)
    export_format = fields.Selection([('csv','CSV'),('pdf','PDF')], string='Export format', default='csv')
    ...
    def action_export(self):
        ...
        entries = self.env['qaco.onboarding.audit.trail'].search(domain, order='create_date')
    ...
    def _export_csv(self, entries):
        ...
        writer.writerow(['Timestamp','User','Action','Type','Override','Resolution','Notes','Onboarding','Related model','Related id'])
        for e in entries:
            writer.writerow([
                e.create_date,
                e.user_id.name or '',
        ...
        name = 'onboarding_audit_export_%s.csv' % (fields.Date.context_today(self).strftime('%Y%m%d'))
    ...
    def _export_pdf(self, entries):
        ...
        return self.env.ref('qaco_client_onboarding.report_audit_snapshot').report_action(entries, data=data)
...
class AuditResolutionWizard(models.TransientModel):
    ...
    def action_apply_resolution(self):
        ...
        audit.onboarding_id._log_action('audit resolution added', notes=self.resolution, action_type='override', is_override=True, related_model='qaco.onboarding.audit.trail', related_res_id=audit.id)
