...
class OnboardingTemplateDocument(models.Model):
    ...
    requires_working_paper = fields.Boolean(
        string="Requires Working Paper (PDF)", default=True
    )
    ...
    def action_attach_to_onboarding(self):
        ...
        self.ensure_one()
        onboarding_id = (
            self.env.context.get("onboarding_id")
            or self.env.context.get("active_id")
            or self.env.context.get("res_id")
        )
        if not onboarding_id:
            raise exceptions.UserError(
                _("Open an Onboarding record to attach this template.")
            )
        ...
        existing = onboarding.attached_template_ids.filtered(
            lambda t: t.template_id.id == self.id
        )
        if existing:
        ...
        if attached and attached.attached_file:
            ...
            try:
                ...
                _logger.exception(
                    "Failed to index template %s into Document Vault (onboarding %s)",
                    self.name,
                    onboarding.id,
                )
        onboarding.message_post(
            body=_("Template %s attached by %s") % (self.name, self.env.user.name)
        )
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
        ...
        for r in self:
            if r.mandatory == "yes_hard_stop" and r.stage != "pre_onboarding":
                raise exceptions.ValidationError(
                    _("Hard-stop mandatory templates must be Pre Onboarding")
                )
...
class OnboardingAttachedTemplate(models.Model):
    ...
    def create(self, vals_list):
        ...
        for rec in Created:
            ...
            if (
                rec.template_id
                and not rec.attached_file
                and rec.template_id.template_file
            ):
            ...
            if rec.attached_file:
                try:
                    ...
                    _logger.exception(
                        "Failed to index attached file for template id %s on onboarding %s",
                        rec.template_id.id if rec.template_id else None,
...
class OnboardingAttachTemplatesWizard(models.TransientModel):
    ...
    onboarding_id = fields.Many2one(
        "qaco.client.onboarding", string="Onboarding", required=True
    )
    ...
    def default_get(self, fields):
        ...
        onboarding_id = self.env.context.get("onboarding_id") or self.env.context.get(
            "active_id"
        )
    ...
    def action_attach(self):
        ...
        for tpl in self.template_ids:
            if not onboarding.attached_template_ids.filtered(
                lambda t: t.template_id.id == tpl.id
            ):
        ...
        if attached_templates:
            onboarding.message_post(
                body=_("Attached templates: %s") % ", ".join(attached_templates)
            )
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
