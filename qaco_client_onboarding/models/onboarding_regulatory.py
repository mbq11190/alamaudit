...
class OnboardingLicense(models.Model):
    ...
    non_renewal_impact = fields.Selection(NON_RENEWAL_IMPACT, string='Non-renewal Impact')
    owner_responsible = fields.Char(string='Client Owner')
    auditor_followup_user_id = fields.Many2one('res.users', string='Auditor Follow-up Owner')
        ...
        for rec in self:
            if rec.required:
                if not rec.expiry_date:
                    raise ValidationError(_('Expiry date must be set for required licenses.'))
                if not rec.renewal_status:
                    raise ValidationError(_('Renewal status must be set for required licenses.'))
    ...
    def _compute_needs_renewal_soon(self):
        ...
        for rec in self:
            rec.needs_renewal_soon = bool(rec.expiry_date and rec.expiry_date <= soon and rec.expiry_date >= today)

    needs_renewal_soon = fields.Boolean(string='Due within 90 days', compute='_compute_needs_renewal_soon')
            ...
            if rec.evidence_file:
                try:
                    self.env['qaco.onboarding.document'].create({
                        ...
                        'file_name': rec.evidence_filename or f'{rec.filing_name}.pdf',
                        'state': 'received',
                    })
                except Exception:
                    _logger.exception('Failed to index filing evidence for %s (onboarding %s)', rec.filing_name, rec.onboarding_id.id)
        return Created
            ...
            if 'evidence_file' in vals and vals.get('evidence_file'):
                try:
                    self.env['qaco.onboarding.document'].create({
                        ...
                        'file_name': vals.get('evidence_filename') or f'{rec.filing_name}.pdf',
                        'state': 'received',
                    })
                except Exception:
                    _logger.exception('Failed to index filing evidence for %s (onboarding %s)', rec.filing_name, rec.onboarding_id.id)
...
class OnboardingDispute(models.Model):
    ...
    amount_exposure = fields.Monetary(string='Amount Exposure', currency_field='currency_id')
    currency_id = fields.Many2one('res.currency', string='Currency', readonly=True, default=lambda self: self.env.company.currency_id)
    status = fields.Selection([('notice','Notice'),('appeal','Appeal'),('court','Court'),('settled','Settled')], string='Status')
    counsel_name = fields.Char(string='Counsel')
    management_assessment = fields.Selection(MANAGEMENT_ASSESSMENT, string='Management Assessment')
...
class OnboardingNonCompliance(models.Model):
    ...
    status = fields.Selection([('open','Open'),('in_progress','In progress'),('closed','Closed')], string='Status')
            ...
            if 'evidence' in vals and vals.get('evidence'):
                try:
                    self.env['qaco.onboarding.document'].create({
                        ...
                        'file_name': vals.get('evidence_filename') or 'noncompliance.pdf',
                        'state': 'received',
                    })
                except Exception:
                    _logger.exception('Failed to index non-compliance evidence (onboarding %s)', rec.onboarding_id.id)
        return res
...
class OnboardingRequiredDocument(models.Model):
    ...
    onboarding_id = fields.Many2one('qaco.client.onboarding', required=True, ondelete='cascade', index=True)
    template_id = fields.Many2one('qaco.onboarding.template.document', string='Template')
    template_name = fields.Char(related='template_id.name', readonly=True)
    stage = fields.Selection([('pre_onboarding','Pre Onboarding'),('throughout_onboarding','Throughout Onboarding')], string='Stage')
    mandatory = fields.Selection([('yes','Yes'),('conditional','Conditional'),('as_applicable','As applicable')], string='Mandatory')
    ...
    status = fields.Selection([('not_started','Not started'),('downloaded','Downloaded'),('received','Received'),('verified','Verified'),('accepted','Accepted'),('rejected','Rejected'),('superseded','Superseded')], string='Status', default='not_started')
            ...
            if rec.uploaded_file:
                try:
                    self.env['qaco.onboarding.document'].create({
                        ...
                        'file_name': rec.uploaded_filename or (rec.template_id.template_filename if rec.template_id else 'upload.pdf'),
                        'state': 'received',
                    })
                except Exception:
                    _logger.exception('Failed to index required document for onboarding %s', rec.onboarding_id.id)
        return Created
            ...
            if 'uploaded_file' in vals and vals.get('uploaded_file'):
                try:
                    self.env['qaco.onboarding.document'].create({
                        ...
                        'file_name': vals.get('uploaded_filename') or (rec.template_id.template_filename if rec.template_id else 'upload.pdf'),
                        'state': 'received',
                    })
                except Exception:
                    _logger.exception('Failed to index required document for onboarding %s', rec.onboarding_id.id)
...
class ClientOnboardingRegulatory(models.Model):
    ...
    regulator_ids = fields.Many2many('qaco.onboarding.regulator', string='Regulators in Scope')
    regulator_contact_ids = fields.One2many('qaco.onboarding.regulator.contact', 'onboarding_id', string='Regulator Contacts')
    license_ids = fields.One2many('qaco.onboarding.license', 'onboarding_id', string='Licenses & Approvals')
    filing_ids = fields.One2many('qaco.onboarding.filing', 'onboarding_id', string='Filings Tracker')
    dispute_ids = fields.One2many('qaco.onboarding.dispute', 'onboarding_id', string='Legal Matters & Disputes')
    non_compliance_ids = fields.One2many('qaco.onboarding.noncompliance', 'onboarding_id', string='Prior Non-Compliances')
    required_document_line_ids = fields.One2many('qaco.onboarding.required.document', 'onboarding_id', string='Required Documents (Regulatory)')

    # Tax & labour high-level
    income_tax_status = fields.Selection([('active','Active'),('inactive','Inactive'),('not_registered','Not registered')], string='Income Tax Filer Status')
    withholding_agent_status = fields.Selection([('yes','Yes'),('no','No')], string='Withholding Agent Status')
    sales_tax_status = fields.Selection([('registered','Registered'),('not_registered','Not registered')], string='Sales Tax Status')
    ...
    eobi_registration = fields.Selection([('yes','Yes'),('no','No')], string='EOBI Registered')
    social_security_registration = fields.Selection([('yes','Yes'),('no','No')], string='Social Security Registered')
    min_wage_overtime_risk = fields.Boolean(string='Minimum Wage / Overtime Compliance Risk')
    hr_legal_matters = fields.Text(string='Key HR Legal Matters')

    # Compliance risk assessment
    compliance_history_rating = fields.Selection([('low','Low'),('medium','Medium'),('high','High')], string='Compliance History Rating')
    compliance_risk_drivers = fields.Text(string='Key Compliance Risk Drivers')
    residual_compliance_risk = fields.Selection([('low','Low'),('medium','Medium'),('high','High')], string='Residual Compliance Risk')
    financial_reporting_impact = fields.Selection([('direct','Direct effect'),('indirect','Indirect effect'),('disclosure','Disclosure-only')], string='Financial Reporting Impact')

    # Alerts & counters
    upcoming_renewals_count = fields.Integer(string='Upcoming Renewals (90d)', compute='_compute_renewal_and_overdues', store=True)
    overdue_filings_count = fields.Integer(string='Overdue Filings', compute='_compute_renewal_and_overdues', store=True)

    @api.depends('license_ids.expiry_date', 'filing_ids.statutory_due_date', 'filing_ids.status')
    def _compute_renewal_and_overdues(self):
        today = date.today()
        soon = today + timedelta(days=90)
        for rec in self:
            rec.upcoming_renewals_count = len(rec.license_ids.filtered(lambda l: l.expiry_date and l.expiry_date <= soon and l.expiry_date >= today))
            rec.overdue_filings_count = len(rec.filing_ids.filtered(lambda f: f.statutory_due_date and f.statutory_due_date < today and f.status != 'filed'))
    ...
    def _check_overdue_filings_before_approval(self):
        ...
        for rec in self:
            if rec.state in ('partner_approved', 'locked'):
                overdue = rec.filing_ids.filtered(lambda f: f.statutory_due_date and f.statutory_due_date < date.today() and f.status != 'filed')
                if overdue:
                    names = ', '.join(overdue.mapped('filing_name'))
                    raise ValidationError(_('Overdue filings detected (%s). Resolve or obtain partner acknowledgement before approval.') % names)
    ...
    def populate_required_documents_from_templates(self):
        ...
        cat = self.env['qaco.onboarding.template.category'].search([('code', '=', 'regulatory')], limit=1)
        if not cat:
            return
