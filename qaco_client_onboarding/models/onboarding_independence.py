...
class IndependenceThreat(models.Model):
    ...
    conflict_id = fields.Many2one(
        "qaco.onboarding.conflict",
        string="Related Conflict",
        ondelete="cascade",
        index=True,
    )
    ...
    overall = fields.Selection(
        OVERALL_RATING, string="Overall risk", compute="_compute_overall", store=True
    )
    safeguards = fields.Many2many(
        "qaco.onboarding.safeguard",
        "threat_safeguard_rel",
        "threat_id",
        "safeguard_id",
        string="Safeguards",
    )
...
class IndependenceConflict(models.Model):
    ...
    description = fields.Text(string="Description")
    threat_ids = fields.One2many(
        "qaco.onboarding.independence.threat", "conflict_id", string="Threats"
    )
...
class ClientOnboardingIndependence(models.Model):
    ...
    independence_declaration_ids = fields.One2many(
        ...
        "onboarding_id",
        string="Independence declarations",
    )
    independence_threat_ids = fields.One2many(
        "qaco.onboarding.independence.threat", "onboarding_id", string="Threat records"
    )
    independence_safeguard_ids = fields.Many2many(
        "qaco.onboarding.safeguard", string="Applicable safeguards"
    )
    conflict_ids = fields.One2many(
        "qaco.onboarding.conflict", "onboarding_id", string="Conflicts register"
    )
    non_audit_service_ids = fields.One2many(
        "qaco.onboarding.non.audit", "onboarding_id", string="Non-audit services"
    )
    gift_ids = fields.One2many(
        "qaco.onboarding.gift", "onboarding_id", string="Gifts & hospitality"
    )
    ...
    independence_status = fields.Selection(
        ...
        string="Independence status",
        compute="_compute_independence_status",
        store=True,
    )
    independence_unresolved_threats = fields.Integer(
        string="Unresolved threats", compute="_compute_unresolved_threats", store=True
    )

    # Configurable thresholds (read from ir.config_parameter via res.config.settings)
    gift_auto_decline_threshold = fields.Float(
        string="Gift auto-decline threshold", readonly=True
    )
    ...
    long_association_years = fields.Integer(
        string="Long association years", readonly=True
    )
    independence_status_feedback = fields.Char(
        string="Independence summary", compute="_compute_independence_feedback"
    )
        ...
        for rec in self:
            if rec.independence_status == "compliant":
                rec.independence_status_feedback = (
                    "No unresolved threats, declarations complete."
                )
                ...
                rec.independence_status_feedback = (
                    "Unresolved threats require escalation."
                )
            elif rec.independence_status == "not_compliant":
                rec.independence_status_feedback = (
                    "Not compliant: withdrawal/decline decision required."
                )
            else:
                rec.independence_status_feedback = ""
    ...
    def _compute_unresolved_threats(self):
        for rec in self:
            rec.independence_unresolved_threats = len(
                rec.independence_threat_ids.filtered(
                    lambda t: not t.resolved and t.overall in ("high", "medium")
                )
            )

    @api.depends(
        "independence_declaration_ids.status",
        "independence_threat_ids.overall",
        "independence_threat_ids.resolved",
    )
    def _compute_independence_status(self):
        for rec in self:
            # If any unresolved high/medium threats -> pending/escalated
            unresolved = rec.independence_threat_ids.filtered(
                lambda t: not t.resolved and t.overall in ("high", "medium")
            )
            # Declarations: ensure all required declarations are submitted and approved
            pending_decls = rec.independence_declaration_ids.filtered(
                lambda d: d.status != "approved"
            )
            if pending_decls:
    ...
    def _check_independence_compliance(self):
        ...
        for rec in self:
            ...
            if rec.independence_declaration_ids and any(
                d.status != "approved" for d in rec.independence_declaration_ids
            ):
                raise ValidationError(
                    _(
                        "All required independence declarations must be approved before partner approval."
                    )
                )
            ...
            if unresolved:
                ...
                if (
                    rec.independence_status != "not_compliant"
                    and rec.independence_status != "escalated"
                ):
                    raise ValidationError(
                        _(
                            "Unresolved independence threats block partner approval. Resolve or escalate to Quality/Compliance."
                        )
                    )
...
class ResConfigSettings(models.TransientModel):
    ...
    pack_attachment_max_kb = fields.Integer(
        string="Pack attachments max size (KB)",
        help="Attachments larger than this (KB) will not be merged into the Clearance Pack; they will be referenced instead.",
    )
    ...
    def get_values(self):
        ...
        res.update(
            gift_auto_decline_threshold=float(
                icp.get_param(
                    "qaco_client_onboarding.gift_auto_decline_threshold", default="0.0"
                )
            ),
            pack_attachment_max_kb=int(
                icp.get_param(
                    "qaco_client_onboarding.pack_attachment_max_kb", default="512"
                )
            ),
        )
        return res
    ...
    def set_values(self):
        ...
        icp = self.env["ir.config_parameter"].sudo()
        icp.set_param(
            "qaco_client_onboarding.gift_auto_decline_threshold",
            self.gift_auto_decline_threshold or 0.0,
        )
        icp.set_param(
            "qaco_client_onboarding.pack_attachment_max_kb",
            int(self.pack_attachment_max_kb or 512),
        )
