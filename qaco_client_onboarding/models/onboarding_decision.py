...
class OnboardingDecision(models.Model):
    ...
    risk_rating = fields.Selection(
        RISK_RATINGS, string="Final risk rating", required=True
    )
    risk_driver_ids = fields.Many2many(
        "qaco.onboarding.decision.driver", string="Risk drivers"
    )
    ...
    superseded_by_id = fields.Many2one(
        "qaco.onboarding.decision", string="Superseded by"
    )
    evidence_attachment_ids = fields.Many2many(
        "ir.attachment", string="Evidence attachments"
    )
    auto_generated_certificate = fields.Many2one(
        "ir.attachment", string="Decision memo"
    )
    quality_approval_id = fields.Many2one(
    ...
    def create(self, vals):
        ...
        if onboarding_id:
            last = self.search(
                [("onboarding_id", "=", onboarding_id)], order="version desc", limit=1
            )
            if last:
                vals["version"] = (last.version or 1) + 1
        # If creating an Accept decision, enforce preconditions immediately
        if (
            vals.get("decision_type") in ("accept", "accept_conditions")
            and onboarding_id
        ):
            ...
            if vals.get("risk_rating") == "prohibitive" and not vals.get(
                "quality_approval_id"
            ):
                raise ValidationError(
                    _(
                        "Prohibitive risk requires Quality/Compliance approval to accept."
                    )
                )
        ...
        if last:
            try:
                ...
                _logger.exception(
                    "Failed to mark previous decision %s as superseded by %s",
                    last.id,
                    rec.id,
                )
        try:
            ...
            _logger.exception(
                "Failed to set latest_decision_id on onboarding %s",
    ...
    def action_unlock(self, approver=None):
        ...
        for rec in self:
            ...
            user = self.env.user
            allowed = user.has_group(
                "qaco_client_onboarding.group_onboarding_confidential"
            ) or user.has_group("qaco_audit.group_audit_manager")
            if not allowed:
            ...
            rec._log_action(
                "Decision unlocked",
                notes=f"Unlocked by {approver and approver.name or self.env.user.name}",
            )
        return True
        ...
        for rec in self:
            if rec.locked:
                ...
                if not any(k in vals for k in ("superseded_by_id",)):
                    raise ValidationError(
                        _(
                            "Locked decision records cannot be modified. Create a superseding decision instead."
                        )
                    )
    ...
    def action_generate_decision_memo(self):
        self.ensure_one()
        report = self.env.ref(
            "qaco_client_onboarding.report_engagement_authorization",
        ...
        try:
            ...
            try:
                ...
                _logger.exception(
                    "Failed to index decision memo for onboarding %s",
                    self.onboarding_id.id,
                )
            return att
        except Exception:
            _logger.exception(
                "Failed to render decision memo for onboarding %s",
                self.onboarding_id.id,
            )
            return False
    ...
    def action_apply_decision(self):
        ...
        for rec in self:
            if rec.decision_type in ("accept", "accept_conditions"):
                ...
                rec.onboarding_id._log_action(
                    "Engagement decision applied: %s" % rec.decision_type,
                    notes=rec.decision_rationale,
                )
