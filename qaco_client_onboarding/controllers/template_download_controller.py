...
try:
    ...
    _logger.info(
        "WeasyPrint not available - PDF generation will use alternative method"
    )
...
class FitProperTemplateController(http.Controller):
    ...
    def _get_template_path(self):
        ...
        return os.path.join(
            module_path, "static", "templates", "fit_proper_assessment_template.html"
        )
    ...
    def _get_onboarding_data(self, onboarding_id):
        ...
        if onboarding_id:
            try:
                onboarding = (
                    request.env["qaco.client.onboarding"]
                    .sudo()
                    .browse(int(onboarding_id))
                )
                if onboarding.exists():
                    ...
                    # Reference
                    data["reference"] = (
                        f"FP-{onboarding.audit_id.name or 'N/A'}"
                        if onboarding.audit_id
                        else ""
                    )
    ...
    def _read_html_template(self, data=None):
        ...
        if data:
            html_content = html_content.replace(
                "{{ENTITY_NAME}}",
                data.get("entity_name", "") or "_________________________",
            )
            html_content = html_content.replace(
                "{{AUDIT_PERIOD}}",
                data.get("audit_period", "") or "_________________________",
            )
            html_content = html_content.replace(
                "{{REFERENCE}}",
                data.get("reference", "") or "_________________________",
            )
            html_content = html_content.replace(
                "{{GENERATED_DATE}}",
                data.get("generated_date", datetime.now().strftime("%d %B %Y")),
            )
        else:
            # Blank template
            html_content = html_content.replace(
                "{{ENTITY_NAME}}", "_________________________"
            )
            html_content = html_content.replace(
                "{{AUDIT_PERIOD}}", "_________________________"
            )
            html_content = html_content.replace(
    ...
    @http.route(
        "/onboarding/template/fit_proper/html",
        type="http",
        auth="user",
        methods=["GET"],
    )
    ...
    @http.route(
        "/onboarding/template/fit_proper/pdf", type="http", auth="user", methods=["GET"]
    )
        ...
        try:
            ...
            if WEASYPRINT_AVAILABLE:
                ...
                try:
                    ...
                    _logger.warning(
                        "No PDF generator available, returning HTML for print-to-PDF"
                    )
                    return request.make_response(
                        html_content,
                        headers=[
                            ...
                            (
                                "Content-Disposition",
                                content_disposition(f"{filename}_PrintToPDF.html"),
                            ),
                        ],
                    )
    ...
    @http.route(
        "/onboarding/template/fit_proper/docx",
        type="http",
        auth="user",
        methods=["GET"],
    )
    def download_fit_proper_docx(self, onboarding_id=None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                _logger.warning(
                    "python-docx not available for .docx generation: %s", re
                )
                html_content = self._read_html_template(data)
                return request.make_response(
                    html_content,
    ...
    def _create_word_document(self, data=None):
        ...
        doc.add_paragraph(
            "☐ PEP    ☐ Foreign national (high-risk jurisdiction)    ☐ Complex ownership / nominee structure    ☐ Adverse media"
        )
        doc.add_paragraph()
        ...
        doc.add_paragraph(
            "Based on the above assessment, the individual meets / does not meet the Fit & Proper criteria under applicable laws, regulations, and ethical requirements."
        )
