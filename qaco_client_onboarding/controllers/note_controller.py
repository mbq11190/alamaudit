...
class NoteController(http.Controller):
    ...
    def auto_save_note(self, note_id, draft_content):
        ...
        try:
            ...
            _logger.info(
                "Auto-saved draft for note %s by user %s", note_id, request.env.user.id
            )
    ...
    def restore_draft(self, note_id):
        ...
        try:
            ...
            return {"success": True, "content": note.content, "result": result}
    ...
    def get_notebook_notes(self, notebook_id):
        ...
        try:
                ...
                notes.append(
                    {
                        ...
                        "last_save_date": (
                            note.last_save_date.isoformat()
                            if note.last_save_date
                            else None
                        ),
                    }
                )
            ...
            return {
                ...
                "completed_count": sum(
                    1 for n in notes if n["has_content"] and not n["is_draft"]
                ),
            }

        except Exception as e:
            _logger.exception(
                "Error getting notes for notebook %s: %s", notebook_id, str(e)
            )
            return {"success": False, "error": str(e)}
    ...
    def get_notebook_progress(self, notebook_id):
        ...
        try:
            ...
            _logger.exception(
                "Error getting progress for notebook %s: %s", notebook_id, str(e)
            )
            return {"success": False, "error": str(e)}
    ...
    def save_session_state(self, notebook_id, current_note_id, scroll_position=0):
        ...
        try:
            ...
            return {"success": True, "session_state": session_state}
    ...
    @http.route(
        "/qaco/note/session/restore", type="json", auth="user", methods=["POST"]
    )
    def restore_session_state(self, session_state):
        ...
        try:
            ...
            return {
                ...
                "notebook_name": (
                    note.parent_notebook_id.name if note.parent_notebook_id else None
                ),
            }
