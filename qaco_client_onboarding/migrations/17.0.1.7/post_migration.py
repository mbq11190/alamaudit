...
def migrate(cr, installed_version):
    ...
    if missing_fields:
        ...
        cr.execute(
            "DELETE FROM ir_model_fields WHERE id IN %s", (tuple(missing_fields),)
        )

    # Backup and remove One2many fields with missing inverse
    try:
        cr.execute(
            "SELECT id FROM ir_model_fields f WHERE f.ttype='one2many' AND (f.inverse_name IS NULL OR f.inverse_name = '' OR NOT EXISTS (SELECT 1 FROM ir_model_fields rf WHERE rf.model = f.relation AND rf.name = f.inverse_name))"
        )
    ...
    try:
        ...
        if bad_views:
            ...
            cr.execute(
                "UPDATE ir_ui_view SET active=false WHERE id IN %s", (tuple(bad_views),)
            )
            cr.execute(
