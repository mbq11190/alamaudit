...
MASTER_CHECKLIST: Dict[str, List[Dict[str, Any]]] = {
    "eqr_trigger": [
        {
            ...
            "weight": 10,
            "guidance": _(
                "Confirm if the engagement falls under listed/PIE definitions."
            ),
        },
        {
            ...
            "weight": 10,
            "guidance": _(
                "Consider rotation breaches, familiarity threats and safeguards."
            ),
        },
        {
            "question": _(
                "Complex estimates or going concern uncertainties identified."
            ),
            "blocking": False,
            "weight": 5,
            "guidance": _(
                "Capture high-risk areas that require specialist review inputs."
            ),
        },
    ],
    "eqr_completion": [
        {
            "question": _(
                "EQR partner reviewed significant judgements and conclusions."
            ),
            "blocking": True,
            "weight": 15,
        },
        {
            "question": _(
                "Financial statements match reviewed version with no late changes."
            ),
            "blocking": True,
            "weight": 10,
        },
        {
            "question": _(
                "Key audit matters / emphasis paragraphs evaluated for accuracy."
            ),
            "blocking": False,
            "weight": 10,
        },
    ...
    "remediation": [
        {
            "question": _(
                "Prior inspection findings relevant to this client addressed."
            ),
            "blocking": True,
            "weight": 10,
        },
    ...
    "archiving": [
        ...
        {
            "question": _(
                "Archival location reference and retention calendar updated."
            ),
            "blocking": False,
            "weight": 10,
        },
...
DEFAULT_SCOPE_TEMPLATE: List[Dict[str, Any]] = [
    {
        ...
        "risk_rating": "high",
        "procedures": _(
            "<ul><li>Review significant contracts</li><li>Inspect manual JE overrides</li></ul>"
        ),
    },
    {
        ...
        "risk_rating": "medium",
        "procedures": _(
            "<ul><li>Challenge cash flow forecasts</li><li>Inspect covenant letters</li></ul>"
        ),
    },
    {
        ...
        "risk_rating": "medium",
        "procedures": _(
            "<ul><li>Cross-check disclosure checklist</li><li>Trace subsequent events</li></ul>"
        ),
    },
]
...
class QualityReview(models.Model):
    ...
    eqr_partner_employee_id = fields.Many2one(
        "hr.employee",
        string="EQR Partner",
        tracking=True,
    ...
    eqr_scope_line_ids = fields.One2many(
        "qaco.quality.eqr.scope", "quality_review_id", string="Scope Areas"
    )
    checklist_line_ids = fields.One2many(
        "qaco.quality.checklist.line", "quality_review_id", string="Checklist"
    )

    trigger_status = fields.Selection(
        SUBTAB_STATUS, string="Trigger Status", compute="_compute_dashboard", store=True
    )
    procedures_status = fields.Selection(
        SUBTAB_STATUS,
        string="Procedures Status",
    ...
    locking_status = fields.Selection(
        SUBTAB_STATUS, string="Locking Status", compute="_compute_dashboard", store=True
    )
    overall_status = fields.Selection(
        SUBTAB_STATUS, string="Overall Status", compute="_compute_dashboard", store=True
    )

    significant_judgments_reviewed = fields.Boolean(
        string="Judgements Reviewed", tracking=True
    )
    ...
    compliance_confirmed = fields.Boolean(
        string="Independence & Compliance confirmed", tracking=True
    )
    ...
    eqr_conclusion_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_quality_eqr_conclusion_rel",
        "review_id",
        "attachment_id",
        string="Conclusion Attachments",
    )

    eqr_review_note_ids = fields.One2many(
        "qaco.quality.review.note", "quality_review_id", string="Review Notes"
    )
    finding_ids = fields.One2many(
        "qaco.quality.finding", "quality_review_id", string="Findings"
    )
    monitoring_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_quality_monitoring_rel",
        "review_id",
        "attachment_id",
    ...
    quality_score = fields.Float(
        string="Quality Score", compute="_compute_quality_score", store=True
    )
    quality_gate_passed = fields.Boolean(string="Quality Gate Passed", tracking=True)
    quality_gate_validated_on = fields.Datetime(
        string="Quality Gate Timestamp", readonly=True
    )

    open_review_note_count = fields.Integer(
        string="Open Notes", compute="_compute_badges", store=True
    )
    open_finding_count = fields.Integer(
        string="Open Findings", compute="_compute_badges", store=True
    )
    ...
    retention_expiry_date = fields.Date(
        string="Retention Expiry", compute="_compute_retention", store=True
    )
    ...
    locking_alerts_html = fields.Html(
        string="Locking Alerts", compute="_compute_locking_alerts"
    )
    eqr_checklist_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_quality_eqr_check_rel",
        "review_id",
        "attachment_id",
        string="Checklist Evidence",
    )
    ...
    file_lock_certificate_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_quality_lock_cert_rel",
        "review_id",
        "attachment_id",
        string="Lock Certificates",
    )
    archiving_evidence_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_quality_archive_rel",
        "review_id",
        "attachment_id",
        string="Archiving Evidence",
    )

    locking_status_message = fields.Text(
        string="Locking Message", compute="_compute_locking_alerts"
    )
    ...
    @api.depends(
        "checklist_line_ids.response",
        "checklist_line_ids.weight",
        "checklist_line_ids.checklist_type",
    )
    def _compute_quality_score(self):
        for record in self:
            ...
            record.quality_score = (
                round((earned / total_weight * 100.0), 2) if total_weight else 0.0
            )

    @api.depends("eqr_review_note_ids.status", "finding_ids.status")
    def _compute_badges(self):
        for record in self:
            record.open_review_note_count = len(
                record.eqr_review_note_ids.filtered(lambda n: n.status != "cleared")
            )
            record.open_finding_count = len(
                record.finding_ids.filtered(lambda f: f.status != "closed")
            )
    ...
    def _compute_retention(self):
        for record in self:
            ...
            if base_date and record.retention_years:
                record.retention_expiry_date = base_date + timedelta(
                    days=record.retention_years * 365
                )
            else:
    ...
    @api.depends(
        "eqr_required",
        "eqr_partner_employee_id",
        "eqr_scope_line_ids.status",
        "eqr_concluded",
        "significant_judgments_reviewed",
        "finding_ids.status",
        "quality_gate_passed",
        "file_lock_date",
        "archiving_completed",
        "open_review_note_count",
        "open_finding_count",
    )
    def _compute_dashboard(self):
        for record in self:
            ...
            if record.eqr_required:
                trigger_complete = trigger_complete and bool(
                    record.eqr_partner_employee_id
                )
            trigger_started = bool(record.eqr_engine_reason or record.eqr_required)
            record.trigger_status = self._status_from_flags(
                trigger_complete, trigger_started
            )

            procedures_started = record.eqr_required or bool(record.eqr_scope_line_ids)
            scope_closed = all(
                line.status == "closed" for line in record.eqr_scope_line_ids
            )
            ...
            procedures_complete = (
                scope_closed
                and (not record.eqr_required or record.eqr_concluded)
                and completion_checks
                and not record.open_review_note_count
            )
            record.procedures_status = self._status_from_flags(
                procedures_complete, procedures_started
            )

            monitoring_started = bool(
                record.finding_ids or record.monitoring_attachment_ids
            )
            monitoring_complete = not record.open_finding_count
            record.monitoring_status = self._status_from_flags(
                monitoring_complete, monitoring_started
            )

            locking_started = bool(
                record.file_completion_date or record.file_assembly_date
            )
            locking_complete = bool(
                record.file_lock_date and record.archiving_completed
            )
            record.locking_status = self._status_from_flags(
                locking_complete, locking_started
            )
            ...
            if (
                all(
                    ...
                    for status in [
                        ...
                        record.procedures_status,
                        record.monitoring_status,
                        record.locking_status,
                    ]
                )
                and record.quality_gate_passed
            ):
    ...
    def _compute_locking_alerts(self):
        for record in self:
            ...
            if record.file_completion_date and not record.file_assembly_date:
                alerts.append(
                    _(
                        "Assembly date missing. ISA 230 requires completion within 60 days."
                    )
                )
            if record.file_completion_date and record.file_assembly_date:
                ...
                if delta > ASSEMBLY_DEADLINE_DAYS:
                    alerts.append(
                        _("Assembly exceeded %s day limit by %s day(s).")
                        % (ASSEMBLY_DEADLINE_DAYS, delta - ASSEMBLY_DEADLINE_DAYS)
                    )
            if record.file_lock_date and not record.archiving_completed:
                alerts.append(_("File locked but archive confirmation pending."))
            if not alerts:
                record.locking_alerts_html = _(
                    '<p style="color:green;">No locking alerts.</p>'
                )
    ...
    def _compute_planning_metrics(self):
        ...
        for record in self:
            planning = self.env["qaco.planning.main"].search(
            ...
            record.planning_completion_pct = round(
                (approved_tabs / total_tabs) * 100, 1
            )
            record.planning_missing_approvals = (
                ", ".join(missing_tabs) if missing_tabs else ""
            )
            ...
            if planning.p6_risk_id:
                ...
                record.planning_significant_risk_count = len(
                    risk_lines.filtered(lambda r: r.is_significant_risk)
                )
            else:
                record.planning_p6_risk_count = 0
                record.planning_significant_risk_count = 0
    ...
    def _compute_planning_red_flags(self):
        ...
        for record in self:
            ...
            if not record.planning_locked:
                flags.append(
                    "🔓 <strong>Planning phase not locked</strong> - ISA 300 requires planning completion before execution."
                )

            # Red Flag: Low completion percentage
            if record.planning_completion_pct < 100:
                flags.append(
                    f"📊 <strong>Planning only {record.planning_completion_pct}% complete</strong> - Missing approvals: {record.planning_missing_approvals}"
                )

            # Red Flag: No risks identified
            if record.planning_p6_risk_count == 0:
                flags.append(
                    "⚠️ <strong>No risks identified in P-6 Risk Assessment</strong> - ISA 315 requires documented risk assessment."
                )

            # Red Flag: No significant risks (unusual)
            if (
                record.planning_p6_risk_count > 0
                and record.planning_significant_risk_count == 0
            ):
                flags.append(
                    f"🟡 <strong>No significant risks flagged</strong> - Consider if {record.planning_p6_risk_count} identified risks include any significant risks per ISA 315."
                )
            ...
            if not flags:
                ...
                formatted_flags = (
                    "<ul>" + "".join(f"<li>{flag}</li>" for flag in flags) + "</ul>"
                )
    ...
    def _seed_master_checklist(self):
        for record in self:
            existing_keys = {
                (line.checklist_type, line.question)
                for line in record.checklist_line_ids
            }
        ...
        for record in self:
            ...
            if record.eqr_override_option == "force_on":
                ...
                if not record.eqr_override_reason:
                    raise UserError(
                        _("Provide an override reason when forcing the EQR off.")
                    )
            ...
            record.write(
                {
                    ...
                    "eqr_engine_reason": (
                        "\n".join(reasons)
                        if reasons
                        else _("No trigger breached. EQR optional.")
                    ),
                }
            )
    ...
    def _evaluate_trigger_rules(self) -> Tuple[bool, List[str]]:
        ...
        if (
            audit.qaco_audit_partner
            and audit.qaco_assigning_partner
            and audit.qaco_audit_partner == audit.qaco_assigning_partner
        ):
    ...
    def action_open_planning_dashboard(self):
        ...
        return {
            ...
            "view_id": self.env.ref(
                "qaco_planning_phase.view_planning_dashboard_kanban"
            ).id,
            "target": "current",
        }
    ...
    def action_validate_quality_gate(self):
        for record in self:
            ...
            record.message_post(
                body=_("Quality gate validated at score %.2f%%.") % record.quality_score
            )
        return True
    ...
    def action_lock_file(self):
        ...
        for record in self:
            ...
            if not record.quality_gate_passed:
                raise UserError(
                    _(
                        "Run and pass the quality gate validation before locking the file."
                    )
                )
            ...
            delta = (
                (record.file_assembly_date - record.file_completion_date).days
                if record.file_completion_date and record.file_assembly_date
                else 0
            )
            if delta > ASSEMBLY_DEADLINE_DAYS:
                raise UserError(
                    _(
                        "Assembly exceeded the %s day limit. Provide remediation before locking."
                    )
                    % ASSEMBLY_DEADLINE_DAYS
                )
            record.write(
                {
    ...
    def action_open_review_notes(self):
        self.ensure_one()
        return self._open_related_tree(
            "qaco.quality.review.note",
            _("Review Notes"),
    ...
    def action_open_findings(self):
        self.ensure_one()
        return self._open_related_tree(
            "qaco.quality.finding", _("Findings"), [("quality_review_id", "=", self.id)]
        )

    def _open_related_tree(
        self, model: str, name: str, domain: List[Tuple[str, str, Any]]
    ):
        return {
            "type": "ir.actions.act_window",
            "name": name,
    ...
    def _ensure_procedural_checks(self):
        ...
        if self.open_review_note_count:
            missing.append(
                _("Resolve outstanding review notes (%s pending).")
                % self.open_review_note_count
            )
        eqr_checklist_lines = self.checklist_line_ids.filtered(
            lambda line: line.checklist_type == "eqr_completion"
        )
        blocking_failures = eqr_checklist_lines.filtered(
            lambda line: line.blocking and line.response != "yes"
        )
        if blocking_failures:
            missing.append(
                _("All blocking completion checklist items must be marked Yes.")
            )
        if missing:
            raise UserError("\n".join(missing))
    ...
    def _run_quality_gate_checks(self):
        ...
        blocking_lines = self.checklist_line_ids.filtered(
            lambda line: line.blocking and line.response not in ("yes", "na")
        )
        if blocking_lines:
            errors.append(_("Complete all blocking checklist items before validation."))
        if self.open_finding_count:
            errors.append(
                _("Close outstanding findings (%s pending).") % self.open_finding_count
            )
        if self.quality_score < QUALITY_PASS_THRESHOLD:
            errors.append(
                _("Quality score %.2f is below the %.2f%% threshold.")
                % (self.quality_score, QUALITY_PASS_THRESHOLD)
            )
...
class QualityChecklistLine(models.Model):
    ...
    checklist_type = fields.Selection(
        CHECKLIST_TYPES, required=True, default="eqr_trigger"
    )
...
class QualityEQRSscope(models.Model):
    ...
    quality_review_id = fields.Many2one(
        "qaco.quality.review", required=True, ondelete="cascade"
    )
    sequence = fields.Integer(default=10)
...
class QualityReviewNote(models.Model):
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment", "qaco_quality_review_note_rel", "note_id", "attachment_id"
    )
...
class QualityFinding(models.Model):
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment", "qaco_quality_finding_rel", "finding_id", "attachment_id"
    )
    action_ids = fields.One2many(
        "qaco.quality.finding.action", "finding_id", string="Action Plan"
    )

    @api.constrains("status", "action_ids")
    def _check_actions_before_close(self):
        for record in self:
            if record.status == "closed" and record.action_ids.filtered(
                lambda a: a.status != "completed"
            ):
                raise ValidationError(
                    _(
                        "All remediation actions must be completed before closing a finding."
                    )
                )
...
class QualityFindingAction(models.Model):
    ...
    finding_id = fields.Many2one(
        "qaco.quality.finding", required=True, ondelete="cascade"
    )
