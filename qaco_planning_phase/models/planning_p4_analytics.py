...
class PlanningP4FSLine(models.Model):
    ...
    fs_caption = fields.Char(
        string="FS Caption", required=True, help="Financial statement line item caption"
    )
    ...
    variance = fields.Float(
        string="Variance", compute="_compute_variance", store=True, digits=(16, 2)
    )
    variance_pct = fields.Float(
        string="% Change", compute="_compute_variance", store=True, digits=(5, 2)
    )
    ...
    explanation_required = fields.Boolean(
        string="Explanation Required", compute="_compute_variance", store=True
    )
    auditor_explanation = fields.Text(
        string="Auditor's Explanation", help="Mandatory if variance exceeds threshold"
    )
    management_explanation = fields.Text(
        string="Management Explanation", help="Explanation provided by management"
    )
    ...
    def _compute_variance(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-4 _compute_variance failed for record {rec.id}: {e}"
                )
                rec.variance = 0
                rec.variance_pct = 0
...
class PlanningP4RatioLine(models.Model):
    ...
    movement = fields.Float(
        string="Movement", compute="_compute_movement", store=True, digits=(10, 2)
    )
    is_unusual = fields.Boolean(
        string="Unusual Movement", help="Flag if ratio movement is unexpected"
    )
    ...
    def _compute_movement(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-4 Ratio _compute_movement failed for record {rec.id}: {e}"
                )
                rec.movement = 0
...
class PlanningP4BudgetLine(models.Model):
    ...
    is_significant = fields.Boolean(
        string="Significant Deviation", compute="_compute_budget_variance", store=True
    )
    ...
    def _compute_budget_variance(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-4 Budget _compute_budget_variance failed for record {rec.id}: {e}"
                )
                rec.variance = 0
                rec.variance_pct = 0
...
class PlanningP4TrendLine(models.Model):
    ...
    cagr = fields.Float(
        string="CAGR %", digits=(5, 2), help="Compound Annual Growth Rate"
    )
...
class PlanningP4NonFinancial(models.Model):
    ...
    change_pct = fields.Float(
        string="Change %", compute="_compute_change", store=True, digits=(5, 2)
    )
        ...
        for rec in self:
            try:
                if rec.prior_value and rec.prior_value != 0:
                    rec.change_pct = (
                        (rec.current_value - rec.prior_value) / abs(rec.prior_value)
                    ) * 100
                else:
                    rec.change_pct = 0
            except Exception as e:
                _logger.warning(
                    f"P-4 NFA _compute_change failed for record {rec.id}: {e}"
                )
                rec.change_pct = 0
        for rec in self:
            if rec.prior_value and rec.prior_value != 0:
...
class PlanningP4GoingConcern(models.Model):
    ...
    linked_to_p8 = fields.Boolean(
        string="Linked to P-8", help="Auto-link to P-8 Going Concern assessment"
    )
...
class PlanningP4Analytics(models.Model):
    ...
    section_a_header = fields.Char(
        default="Section A: Data Source Identification & Integrity", readonly=True
    )
    ...
    data_consistent_with_py = fields.Selection(
        ...
        string="Consistent with Prior Year Audited Figures?",
        tracking=True,
    )
    ...
    data_reliability_concerns = fields.Selection(
        ...
        string="Data Reliability Concerns Identified?",
        tracking=True,
    )
    ...
    section_b_header = fields.Char(
        default="Section B: Comparative Financial Analysis (YoY)", readonly=True
    )

    # One2many to FS Line Variance Analysis
    fs_line_ids = fields.One2many(
        "qaco.planning.p4.fs.line", "p4_id", string="FS Line Items"
    )
    ...
    section_c_header = fields.Char(
        default="Section C: Ratio Analysis (Mandatory Set)", readonly=True
    )

    # One2many to Ratio Lines
    ratio_line_ids = fields.One2many(
        "qaco.planning.p4.ratio.line", "p4_id", string="Ratio Analysis Lines"
    )
    ...
    # Efficiency
    inventory_turnover_cy = fields.Float(
        string="Inventory Turnover (CY)", digits=(5, 2)
    )
    ...
    unexpected_ratio_movements = fields.Selection(
        ...
        string="Unexpected Ratio Movements?",
        tracking=True,
    )
    ...
    section_d_header = fields.Char(
        default="Section D: Budget vs Actual / Forecast Comparison", readonly=True
    )
    ...
    budget_available = fields.Selection(
        ...
        string="Budget / Forecast Available?",
        tracking=True,
    )
    ...
    budget_line_ids = fields.One2many(
        "qaco.planning.p4.budget.line", "p4_id", string="Budget vs Actual Lines"
    )
    ...
    significant_budget_deviations = fields.Selection(
        ...
        string="Significant Deviations Identified?",
        tracking=True,
    )
    ...
    checklist_d_deviations_evaluated = fields.Boolean(string="☐ Deviations evaluated")

    # =========================================================================
    # SECTION E - TREND & PATTERN ANALYSIS (MULTI-YEAR)
    # =========================================================================
    section_e_header = fields.Char(
        default="Section E: Trend & Pattern Analysis (Multi-Year)", readonly=True
    )
    ...
    years_compared = fields.Selection(
        ...
        string="Number of Years Compared",
        default="3",
    )

    # One2many to Trend Lines
    trend_line_ids = fields.One2many(
        "qaco.planning.p4.trend.line", "p4_id", string="Multi-Year Trend Lines"
    )
    ...
    non_financial_ids = fields.One2many(
        "qaco.planning.p4.non.financial", "p4_id", string="Non-Financial Metrics"
    )
    ...
    section_g_header = fields.Char(
        default="Section G: Fraud Indicators from Analytics (ISA 240)", readonly=True
    )

    # One2many to Fraud Indicators
    fraud_indicator_ids = fields.One2many(
        "qaco.planning.p4.fraud.indicator", "p4_id", string="Fraud Indicators"
    )
    ...
    unusual_revenue_near_period_end = fields.Selection(
        ...
        string="Unusual Revenue Trends Near Period End?",
    ...
    margin_manipulation_indicators = fields.Selection(
        ...
        string="Margin Manipulation Indicators?",
        default="not_assessed",
    )
    unexpected_expense_capitalization = fields.Selection(
        ...
        string="Unexpected Expense Capitalization?",
        default="not_assessed",
    )
    cash_profit_inconsistencies = fields.Selection(
        ...
        string="Cash Flow vs Profit Inconsistencies?",
        default="not_assessed",
    )
    override_red_flags = fields.Selection(
        ...
        string="Override-Type Analytical Red Flags?",
        default="not_assessed",
    )

    fraud_indicators_conclusion = fields.Html(string="Fraud Indicators Conclusion")
    ...
    going_concern_ids = fields.One2many(
        "qaco.planning.p4.going.concern", "p4_id", string="Going Concern Indicators"
    )
    ...
    liquidity_stress_indicators = fields.Selection(
        ...
        string="Liquidity Stress Indicators?",
        default="not_assessed",
    )
    ...
    dependency_on_external_financing = fields.Selection(
        ...
        string="Dependency on External Financing?",
        default="not_assessed",
    )
    ...
    section_i_header = fields.Char(
        default="Section I: Linkage to Risk Assessment (Auto-Flow)", readonly=True
    )
    ...
    # Document Checklist
    checklist_j_trial_balance = fields.Boolean(string="☐ Trial balance used")
    checklist_j_prior_year_fs = fields.Boolean(string="☐ Prior-year audited FS")
    checklist_j_budget = fields.Boolean(string="☐ Budget / forecasts (if available)")
    checklist_j_mgmt_explanations = fields.Boolean(
        string="☐ Management explanations (if separate)"
    )
    checklist_j_workpapers = fields.Boolean(string="☐ Analytical working papers")

    # =========================================================================
    # SECTION K - P-4 CONCLUSION & PROFESSIONAL JUDGMENT
    # =========================================================================
    section_k_header = fields.Char(
        default="Section K: P-4 Conclusion & Professional Judgment", readonly=True
    )
...
and the overall audit strategy.</strong></p>""",
    ...
    # Final Confirmations
    confirm_analytics_complete = fields.Boolean(string="☐ Analytics complete")
    ...
    def _compute_name(self):
        for rec in self:
            if rec.client_id:
                rec.name = (
                    f"P4-{rec.client_id.name[:20] if rec.client_id.name else 'NEW'}"
                )
            else:
    ...
    def _compute_risk_linkage(self):
        ...
        for rec in self:
            try:
                ...
                unexplained_count = len(
                    rec.fs_line_ids.filtered(
                        lambda line: line.exceeds_threshold and not line.auditor_explanation
                    )
                )
                rec.risks_created_in_p6 = unexplained_count
            except Exception as e:
                _logger.warning(
                    f"P-4 _compute_risk_linkage failed for record {rec.id}: {e}"
                )
                rec.risks_created_in_p6 = 0
    ...
    def _validate_section_b(self):
        ...
        unexplained = self.fs_line_ids.filtered(
            lambda line: line.explanation_required and not line.auditor_explanation
        )
        if unexplained:
            errors.append(
                f"Explanations required for {len(unexplained)} significant variances (Section B)"
            )
        return errors
    ...
    def _validate_section_c(self):
        ...
        errors = []
        if (
            self.unexpected_ratio_movements == "yes"
            and not self.ratio_analysis_conclusion
        ):
            errors.append(
                "Ratio analysis conclusion required when unexpected movements identified (Section C)"
            )
        return errors
    ...
    def _validate_section_d(self):
        ...
        if self.budget_available == "no" and not self.budget_not_available_reason:
            errors.append(
                "Justification required for not performing budget comparison (Section D)"
            )
        if (
            self.significant_budget_deviations == "yes"
            and not self.budget_auditor_assessment
        ):
            errors.append(
                "Auditor assessment required for significant budget deviations (Section D)"
            )
        return errors
    ...
    def action_partner_approve(self):
        ...
        for rec in self:
            ...
            rec.state = "locked"
            rec.message_post(
                body=f"P-4 approved and locked by Partner: {self.env.user.name}"
            )
    ...
    def _auto_unlock_p5(self):
        ...
        if "qaco.planning.p5.materiality" in self.env:
            ...
            if p5 and p5.state == "not_started":
                _logger.info(
                    f"P-5 auto-unlock triggered by P-4 approval for audit {self.audit_id.id}"
                )
    ...
    def action_create_risks_from_variances(self):
        ...
        if not unexplained:
            raise UserError(
                "No unexplained significant variances to create risks from."
            )
    ...
    def action_link_gc_to_p8(self):
        ...
        gc_indicators = self.going_concern_ids.filtered(
            lambda g: not g.linked_to_p8
            and g.severity in ["medium", "high", "critical"]
        )
    ...
    def action_populate_standard_ratios(self):
        ...
        if self.ratio_line_ids:
            raise UserError(
                "Ratio lines already exist. Delete existing lines to repopulate."
            )
