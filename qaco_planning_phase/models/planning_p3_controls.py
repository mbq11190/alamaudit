...
class PlanningP3TransactionCycle(models.Model):
    ...
    walkthrough_completed = fields.Boolean(
        string="Walkthrough Completed", default=False
    )
    misstatement_points_identified = fields.Boolean(
        string="Misstatement Points Identified", default=False
    )
...
class PlanningP3Controls(models.Model):
        ...
        for rec in self:
            if rec.state != "not_started" and not rec.can_open:
                raise UserError(
                    ...
                    "P-3 (Internal Controls) cannot be started until P-2 (Entity Understanding) "
                    "has been Partner-approved.\n\n"
                    "Reason: Assessing controls requires prior understanding of the entity, "
                    "its environment, and business risks.\n\n"
                    "Action: Please complete and obtain Partner approval for P-2 first."
                )
    ...
    _sql_constraints = [
        (
            "audit_unique",
            "UNIQUE(audit_id)",
            "Only one P-3 record per Audit Engagement is allowed.",
        )
    ]
        ...
        for record in self:
            try:
                ...
                record.walkthroughs_completed = len(
                    record.transaction_cycle_ids.filtered(
                        lambda c: c.walkthrough_completed
                    )
                )
            except Exception as e:
                _logger.warning(
                    f"P-3 _compute_cycle_summary failed for record {record.id}: {e}"
                )
                record.total_cycles = 0
                record.walkthroughs_completed = 0
        ...
        for record in self:
            try:
                ...
                record.controls_implemented = len(
                    record.key_control_ids.filtered(
                        lambda c: c.implementation_confirmed
                    )
                )
            except Exception as e:
                _logger.warning(
                    f"P-3 _compute_control_summary failed for record {record.id}: {e}"
                )
                record.total_key_controls = 0
                record.controls_implemented = 0
        ...
        for record in self:
            try:
                ...
                record.significant_deficiencies_count = len(
                    record.deficiency_ids.filtered(
                        lambda d: d.severity == "significant"
                    )
                )
                record.material_weaknesses_count = len(
                    record.deficiency_ids.filtered(
                        lambda d: d.severity == "material_weakness"
                    )
                )
                record.tcwg_communication_required = (
                    record.significant_deficiencies_count > 0
                    or record.material_weaknesses_count > 0
                )
            except Exception as e:
                _logger.warning(
                    f"P-3 _compute_deficiency_summary failed for record {record.id}: {e}"
                )
                record.deficiencies_identified = False
                record.significant_deficiencies_count = 0
                record.material_weaknesses_count = 0
    ...
    def _compute_reliance_decision(self):
        ...
        for record in self:
            try:
                record.reliance_planned = record.reliance_strategy in (
                    "limited_reliance",
                    "combined",
                )
            except Exception as e:
                _logger.warning(
                    f"P-3 _compute_reliance_decision failed for record {record.id}: {e}"
                )
    ...
    def _compute_attachment_counts(self):
        ...
        for record in self:
            try:
                record.flowchart_count = (
                    len(record.process_flowchart_ids)
                    if record.process_flowchart_ids
                    else 0
                )
                record.walkthrough_doc_count = (
                    len(record.walkthrough_doc_ids) if record.walkthrough_doc_ids else 0
                )
            except Exception as e:
                _logger.warning(
                    f"P-3 _compute_attachment_counts failed for record {record.id}: {e}"
                )
    ...
    def _compute_proceed_to_p4(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-3 _compute_proceed_to_p4 failed for record {record.id}: {e}"
                )
    ...
    def _check_p2_preconditions(self):
        ...
        if "qaco.planning.p2.entity" in self.env:
            ...
            if not p2:
                raise UserError(
                    "P-2 (Entity & Environment) must be created before starting P-3."
                )
    ...
    def _validate_mandatory_fields(self):
        ...
        if self.total_cycles == 0:
            errors.append(
                "At least one transaction cycle must be documented (Section B/C)"
            )
        ...
        if self.system_based_accounting and not self.itgc_overall_rating:
            errors.append(
                "ITGC overall rating is required for system-based accounting (Section E)"
            )
        ...
        if not self.controls_understood:
            errors.append(
                "Confirm: Internal controls sufficiently understood (Section L)"
            )
    ...
    def action_send_back(self):
        ...
        for record in self:
            ...
            record.state = "in_progress"
            record.message_post(
                body=f"P-3 sent back for rework from {old_state} state."
            )
