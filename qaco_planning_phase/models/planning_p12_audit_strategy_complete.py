...
class AuditPlanningP12AuditStrategy(models.Model):
    """P-12: Audit Strategy & Detailed Audit Plan (ISA 300) - MASTER MODEL"""
    _name = 'audit.planning.p12.audit_strategy'
    ...
    risk_response_ids = fields.One2many(
        'audit.planning.p12.risk_response',
        'p12_id',
    ...
    fs_area_strategy_ids = fields.One2many(
        'audit.planning.p12.fs_area_strategy',
        'p12_id',
    ...
    audit_program_ids = fields.One2many(
        'audit.planning.p12.audit_program',
        'p12_id',
    ...
    sampling_plan_ids = fields.One2many(
        'audit.planning.p12.sampling_plan',
        'p12_id',
    ...
    kam_candidate_ids = fields.One2many(
        'audit.planning.p12.kam_candidate',
        'p12_id',
    ...
    def _compute_group_audit_applicable(self):
        ...
        for rec in self:
            p11 = self.env['audit.planning.p11.group_audit'].search([
    ...
    def _check_preconditions(self):
        ...
        planning_phases = [
            ('audit.planning.p1.engagement', 'P-1 (Engagement Understanding)'),
            ('audit.planning.p2.entity', 'P-2 (Entity Understanding)'),
            ('audit.planning.p3.controls', 'P-3 (Internal Controls)'),
            ('audit.planning.p4.analytics', 'P-4 (Preliminary Analytics)'),
            ('audit.planning.p5.materiality', 'P-5 (Materiality)'),
            ('audit.planning.p6.risk', 'P-6 (Risk Assessment)'),
            ('audit.planning.p7.fraud', 'P-7 (Fraud Assessment)'),
            ('audit.planning.p8.going_concern', 'P-8 (Going Concern)'),
            ('audit.planning.p9.laws', 'P-9 (Laws & Regulations)'),
            ('audit.planning.p10.related_parties', 'P-10 (Related Parties)'),
            ('audit.planning.p11.group_audit', 'P-11 (Group Audit)'),
        ]
    ...
    def _auto_populate_risk_responses(self):
        ...
        RiskResponse = self.env['audit.planning.p12.risk_response']
        
        # Clear existing
        self.risk_response_ids.unlink()
        
        # From P-6 (Risk Assessment)
        p6 = self.env['audit.planning.p6.risk'].search([
            ('engagement_id', '=', self.engagement_id.id),
            ('audit_year_id', '=', self.audit_year_id.id),
        ], limit=1)
        ...
        # From P-7 (Fraud)
        p7 = self.env['audit.planning.p7.fraud'].search([
            ('engagement_id', '=', self.engagement_id.id),
            ('audit_year_id', '=', self.audit_year_id.id),
        ], limit=1)
...
# ============================================================================
class AuditPlanningP12RiskResponse(models.Model):
    """Risk-to-Response Mapping (ISA 330)"""
    _name = 'audit.planning.p12.risk_response'
    _description = 'Risk-Response Mapping'
    _order = 'risk_level desc, fs_area'

    p12_id = fields.Many2one(
        'audit.planning.p12.audit_strategy',
...
# ============================================================================
class AuditPlanningP12FSAreaStrategy(models.Model):
    """FS Area-Wise Audit Strategy"""
    _name = 'audit.planning.p12.fs_area_strategy'
...
# ============================================================================
class AuditPlanningP12AuditProgram(models.Model):
    """Detailed Audit Programs"""
    _name = 'audit.planning.p12.audit_program'
...
# ============================================================================
class AuditPlanningP12SamplingPlan(models.Model):
    """Sampling Plans (ISA 530)"""
    _name = 'audit.planning.p12.sampling_plan'
...
# ============================================================================
class AuditPlanningP12KAMCandidate(models.Model):
    """Key Audit Matter Candidates (ISA 701)"""
    _name = 'audit.planning.p12.kam_candidate'
