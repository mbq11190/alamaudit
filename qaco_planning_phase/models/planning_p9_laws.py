...
class PlanningP9Laws(models.Model):
    ...
    law_line_ids = fields.One2many(
        "qaco.law.line", "p9_laws_id", string="Laws Register"
    )
    ...
    compliance_assessment = fields.Selection(
        ...
        string="Overall Compliance Assessment",
        tracking=True,
    )
    ...
    direct_effect_laws = fields.Html(
        string="Direct Effect Laws", related="category_a_laws", readonly=False
    )
    ...
    indirect_effect_laws = fields.Html(
        string="Indirect Effect Laws", related="category_b_laws", readonly=False
    )
    ...
    companies_act_applicable = fields.Boolean(
        string="Companies Act 2017 Applicable", default=True
    )
    ...
    fbr_compliance_applicable = fields.Boolean(
        string="FBR/Tax Compliance Applicable", default=True
    )
    ...
    environmental_laws_applicable = fields.Boolean(
        string="Environmental Laws Applicable"
    )
    ...
    procedure_legal_confirmations = fields.Boolean(
        string="☐ Legal Confirmations", help="Obtain confirmations from legal counsel"
    )
    ...
    inquiries_made = fields.Html(
        string="Inquiries Made", help="Inquiries made regarding compliance"
    )
    ...
    last_inspection_date = fields.Date(
        string="Last Inspection Date", help="Date of last regulatory inspection"
    )
    ...
    non_compliance_status = fields.Selection(
        ...
        string="Status of Non-Compliance",
        tracking=True,
    )
    ...
    compliance_risk_level = fields.Selection(
        ...
        string="Overall Compliance Risk Level",
        tracking=True,
        help="Overall assessment of compliance risk per ISA 315",
    )
    ...
    non_compliance_line_ids = fields.One2many(
        "qaco.non.compliance.line", "p9_laws_id", string="Non-Compliance Items"
    )
    ...
    regulatory_reporting_details = fields.Html(string="Regulatory Reporting Details")
    # XML view compatible aliases
    ...
    audit_report_impact = fields.Html(
        string="Impact on Audit Report", help="Assessment of impact on audit report"
    )
    ...
    isa_reference = fields.Char(
        string="ISA Reference", default="ISA 250 (Revised)", readonly=True
    )
    ...
    _sql_constraints = [
        (
            "audit_unique",
            "UNIQUE(audit_id)",
            "Only one P-9 record per Audit Engagement is allowed.",
        )
    ]
...
"""
    ...
    def _validate_mandatory_fields(self):
        ...
        if not self.confirm_laws_considered:
            errors.append(
                "Section K: Confirm all laws & regulations adequately considered"
            )
        if not self.confirm_risks_assessed_linked:
            errors.append(
                "Section K: Confirm compliance risks assessed and linked to P-6/P-12"
            )
        ...
        if not self.mgmt_representation_draft_attachment_ids:
            errors.append(
                "Section J: Management representation drafts must be uploaded"
            )
    ...
    @api.depends(
        ...
        "non_compliance_financial_impact",
    )
        ...
        for record in self:
            try:
                ...
                record.disclosure_risk_from_noncompliance = (
                    record.non_compliance_identified
                    and record.non_compliance_status in ["ongoing", "disputed"]
                )
                # RMM impact if non-compliance is material (financial impact significant)
                record.rmm_impact_flagged = (
                    record.non_compliance_identified
                    and record.non_compliance_financial_impact > 0
                )
            except Exception as e:
                _logger.warning(
                    f"P-9 _compute_noncompliance_flags failed for record {record.id}: {e}"
                )
    ...
    def _compute_compliance_risk_escalation(self):
        ...
        for record in self:
            try:
                record.high_risk_requires_escalation = (
                    record.compliance_risk_level == "high"
                )
            except Exception as e:
                _logger.warning(
                    f"P-9 _compute_compliance_risk_escalation failed for record {record.id}: {e}"
                )
                record.high_risk_requires_escalation = False

    @api.depends(
        "indicators_illegal_acts",
        "management_involvement_suspected",
    ...
    def _compute_fraud_linkage(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-9 _compute_fraud_linkage failed for record {record.id}: {e}"
                )
    ...
    def _compute_gc_linkage(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-9 _compute_gc_linkage failed for record {record.id}: {e}"
                )
...
class PlanningP9LawLine(models.Model):
    ...
    p9_laws_id = fields.Many2one(
        "qaco.planning.p9.laws", string="P-9 Laws", required=True, ondelete="cascade"
    )
    ...
    law_name = fields.Char(string="Law/Regulation Name", required=True)
    regulator = fields.Char(string="Regulatory Body")
    relevance = fields.Text(
        string="Relevance to Entity", help="Why this law is applicable to the entity"
    )
    ...
    audit_procedures = fields.Text(string="Planned Audit Procedures")
...
class LawLine(models.Model):
    ...
    audit_procedures = fields.Text(string="Audit Procedures")
...
class NonComplianceLine(models.Model):
    ...
    sequence = fields.Integer(string="Sequence", default=10)
    law_reference = fields.Char(string="Law/Regulation Reference", required=True)
    description = fields.Text(string="Description of Non-Compliance")
