...
class PlanningP12Strategy(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Engagement Partner", tracking=True, required=True
    )
    ...
    interim_audit_details = fields.Html(
        string="Interim Audit Details", help="Timing and scope of interim audit work"
    )
    ...
    group_audit_strategy = fields.Html(
        string="Group Audit Strategy", help="Reference to P-11 group audit planning"
    )
    ...
    total_risks = fields.Integer(
        string="Total Risks", compute="_compute_risk_metrics", store=True
    )
    risks_with_responses = fields.Integer(
        string="Risks with Responses", compute="_compute_risk_metrics", store=True
    )
    ...
    significant_risk_count = fields.Integer(
        string="Significant Risks", compute="_compute_risk_metrics", store=True
    )
    ...
    programs_finalized = fields.Boolean(
        string="All Programs Finalized?", help="Confirm all audit programs are complete"
    )
    ...
    sampling_methodology = fields.Selection(
        ...
        string="Overall Sampling Methodology",
        tracking=True,
    )
    ...
    cash_flow_testing_details = fields.Html(string="Cash Flow Testing Details")
    ...
    subsequent_events_details = fields.Html(string="Subsequent Events Procedures")
    ...
    is_listed_entity = fields.Boolean(
        string="Listed Entity?", help="ISA 701 applies to listed entities"
    )
    ...
    kam_candidates_count = fields.Integer(
        string="KAM Candidates", compute="_compute_kam_metrics", store=True
    )
    ...
    planned_hours_partner = fields.Float(
        string="Planned Hours - Partner", help="Budgeted partner hours"
    )
    planned_hours_manager = fields.Float(
        string="Planned Hours - Manager", help="Budgeted manager hours"
    )
    planned_hours_senior = fields.Float(
        string="Planned Hours - Senior", help="Budgeted senior hours"
    )
    planned_hours_trainee = fields.Float(
        string="Planned Hours - Trainee/Assistant", help="Budgeted trainee hours"
    )
    total_planned_hours = fields.Float(
        string="Total Planned Hours", compute="_compute_total_hours", store=True
    )
    ...
    interim_audit_date = fields.Date(
        string="Interim Audit Date", help="Date of interim audit work"
    )
    ...
    draft_report_date = fields.Date(
        string="Draft Report Date", help="Target date for draft report"
    )
    final_report_date = fields.Date(
        string="Final Report Date", help="Target date for final report issuance"
    )
    ...
    review_checkpoints = fields.Html(
        string="Review & EQCR Checkpoints", help="Key review points throughout audit"
    )

    # Budget reconciliation
    budget_reconciliation = fields.Html(
        string="Budget Reconciliation", help="Reconciliation with P-5 Audit Budget"
    )
    ...
    prepared_by = fields.Many2one(
        "res.users", string="Prepared By", tracking=True, readonly=True
    )
    prepared_by_role = fields.Char(
        string="Role", compute="_compute_preparer_role", store=True
    )
    prepared_on = fields.Datetime(string="Prepared On", readonly=True)

    reviewed_by = fields.Many2one(
        "res.users", string="Reviewed By (Manager)", tracking=True, readonly=True
    )
    reviewed_on = fields.Datetime(string="Reviewed On", readonly=True)
    review_notes = fields.Html(
        string="Manager Review Notes", help="Manager must document review findings"
    )

    partner_approved = fields.Boolean(
        string="Partner Approval", tracking=True, readonly=True, copy=False
    )
    partner_approved_by = fields.Many2one(
        "res.users", string="Partner Approved By", tracking=True, readonly=True
    )
    ...
    reviewer_timestamps = fields.Text(string="Reviewer Timestamps", readonly=True)
        ...
        for rec in self:
            try:
                ...
                rec.significant_risk_count = len(
                    rec.risk_response_ids.filtered(
                        lambda r: r.risk_level == "significant"
                    )
                )
            except Exception as e:
                _logger.warning(
                    f"P-12 _compute_risk_metrics failed for record {rec.id}: {e}"
                )
                rec.total_risks = 0
                rec.risks_with_responses = 0
                rec.unaddressed_risks = 0
    ...
    def _compute_fs_area_coverage(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-12 _compute_fs_area_coverage failed for record {rec.id}: {e}"
                )
                rec.mandatory_fs_areas_covered = False
    ...
    def _compute_kam_metrics(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-12 _compute_kam_metrics failed for record {rec.id}: {e}"
                )
                rec.kam_candidates_count = 0
                rec.kam_from_significant_risks = False

    @api.depends(
        "planned_hours_partner",
        "planned_hours_manager",
        "planned_hours_senior",
        "planned_hours_trainee",
    )
        ...
        for rec in self:
            if rec.unaddressed_risks > 0:
                raise ValidationError(
                    _(
                        "Cannot approve P-12: %d risk(s) do not have documented audit responses. "
                        "All risks must be addressed per ISA 330."
                    )
                    % rec.unaddressed_risks
                )
    ...
    def create(self, vals_list):
        # Ensure default conclusion narrative for any missing values
    ...
    def write(self, vals):
        result = super(PlanningP12Strategy, self).write(vals)
    ...
    def _validate_mandatory_fields(self):
        ...
        if self.unaddressed_risks > 0:
            errors.append(
                f"Section B: {self.unaddressed_risks} risk(s) without responses"
            )
        ...
        if not self.journal_entry_testing_approach:
            errors.append(
                "Section G: Journal entry testing approach is required (ISA 240.32)"
            )
        if not self.management_override_procedures:
            errors.append(
                "Section G: Management override procedures are required (ISA 240.33)"
            )
    ...
    def action_mark_complete(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_("P-12 marked complete and submitted for manager review.")
            )
    ...
    def action_manager_review(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_("P-12 reviewed by manager and forwarded to partner.")
            )
    ...
    def action_partner_approve(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_(
                    "P-12 approved by partner. PLANNING PHASE LOCKED. Execution Phase is now unlocked."
                )
            )
    ...
    def action_partner_unlock(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_(
                    "P-12 unlocked by partner for amendment. WARNING: Planning phase unlocked."
                )
            )
    ...
    def _unlock_execution_phase(self):
        ...
        self.message_post(
            body=_(
                "Planning Phase complete. Execution Phase (Audit Fieldwork) is now accessible."
            )
        )
...
class PlanningP12RiskResponse(models.Model):
    ...
    risk_id = fields.Char(string="Risk ID", help="Reference to original risk ID")
    risk_description = fields.Html(string="Risk Description", required=True)
    fs_area = fields.Char(string="FS Area", help="Financial statement area affected")
    assertion = fields.Char(
        string="Assertion", help="Financial statement assertion affected"
    )
    ...
    response_documented = fields.Boolean(
        string="Response Documented?", help="Has the audit response been documented?"
    )
...
class PlanningP12FSAreaStrategy(models.Model):
    ...
    rmm_level = fields.Selection(
        ...
        string="RMM Level",
        required=True,
        help="Overall RMM for this area",
    )

    controls_reliance = fields.Boolean(
        string="Controls Reliance?", help="Will we rely on controls for this area?"
    )
    ...
    specialist_required = fields.Boolean(
        string="Specialist Required?", help="Does this area require a specialist?"
    )
    specialist_type = fields.Char(
        string="Specialist Type", help="E.g., IT, Valuation, Actuary"
    )
...
class PlanningP12AuditProgram(models.Model):
    ...
    nature = fields.Char(string="Nature", help="Nature of procedure (ISA 330)")
    timing = fields.Char(string="Timing", help="Timing of procedure (interim/year-end)")
    extent = fields.Char(
        string="Extent", help="Extent of procedure (sample size, coverage)"
    )

    is_finalized = fields.Boolean(
        string="Finalized?", help="Has this procedure been finalized?"
    )
...
class PlanningP12SamplingPlan(models.Model):
    ...
    fs_area = fields.Char(string="FS Area / Population", required=True)
    ...
    population_size = fields.Integer(
        string="Population Size", help="Total number of items in population"
    )
    population_value = fields.Monetary(
        string="Population Value", currency_field="currency_id"
    )
    currency_id = fields.Many2one("res.currency", related="p12_id.currency_id")

    sample_size = fields.Integer(
        string="Sample Size", required=True, help="Number of items to be tested"
    )
    ...
    sampling_unit = fields.Char(
        string="Sampling Unit", help="E.g., invoice, transaction, item"
    )
    expected_misstatement = fields.Monetary(
        string="Expected Misstatement", currency_field="currency_id"
    )
...
class PlanningP12KAMCandidate(models.Model):
    ...
    area = fields.Char(
        string="Area / Matter", required=True, help="Area giving rise to potential KAM"
    )
    ...
    risk_link = fields.Char(
        string="Risk Link", help="Reference to linked risk (P-6, P-7, etc.)"
    )
