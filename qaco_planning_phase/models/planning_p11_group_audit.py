...
class PlanningP11GroupAudit(models.Model):
    ...
    senior_signed_user_id = fields.Many2one(
        "res.users", string="Senior Signed By", related="prepared_by", readonly=True
    )
    senior_signed_on = fields.Datetime(
        string="Senior Signed On", related="prepared_on", readonly=True
    )
    manager_reviewed_user_id = fields.Many2one(
        "res.users", string="Manager Reviewed By", related="reviewed_by", readonly=True
    )
    manager_reviewed_on = fields.Datetime(
        string="Manager Reviewed On", related="reviewed_on", readonly=True
    )
    ...
    def _compute_component_metrics(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-11 _compute_component_metrics failed for record {rec.id}: {e}"
                )
                rec.total_component_count = 0
                rec.significant_component_count = 0
    ...
    @api.depends(
        "component_ids",
        "component_ids.is_significant",
        "component_ids.component_name",
        "component_ids.percentage_group_assets",
        "component_ids.percentage_group_revenue",
    )
        ...
        for rec in self:
            try:
                ...
                if not rec.component_ids:
                    ...
                    rec.coverage_assessment = (
                        "<p>Cannot assess coverage without component definitions.</p>"
                    )
                    continue

                # Significant Components
                significant_comps = rec.component_ids.filtered(
                    lambda c: c.is_significant
                )
                if significant_comps:
                    ...
                    rec.significant_components = (
                        "<p>No significant components identified.</p>"
                    )

                # Non-Significant Components
                non_sig_comps = rec.component_ids.filtered(
                    lambda c: not c.is_significant
                )
                if non_sig_comps:
                    ...
                    rec.non_significant_components = (
                        "<p>All components are significant.</p>"
                    )
                ...
                _logger.warning(
                    f"P-11 _compute_significance_assessment failed for record {rec.id}: {e}"
                )
                rec.significance_criteria = (
                    "<p>Error generating significance criteria.</p>"
                )
                rec.significant_components = (
                    "<p>Error generating significant components list.</p>"
                )
                rec.non_significant_components = (
                    "<p>Error generating non-significant components list.</p>"
                )
    ...
    def _compute_component_auditor_involvement(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-11 _compute_component_auditor_involvement failed for record {rec.id}: {e}"
                )
                rec.component_auditors_involved = False

    @api.depends(
        "component_auditor_ids.independence_confirmed",
        ...
        for rec in self:
            try:
                ...
                rec.escalation_required = (
                    not rec.independence_confirmed_all
                    or not rec.competence_confirmed_all
                )
            except Exception as e:
                _logger.warning(
                    f"P-11 _compute_auditor_confirmations failed for record {rec.id}: {e}"
                )
                rec.independence_confirmed_all = False
                rec.competence_confirmed_all = False
                rec.escalation_required = False
        ...
        for rec in self:
            if rec.is_group_audit and not rec.component_ids:
                raise ValidationError(
                    _(
                        "If this is a group audit, you must identify at least one component."
                    )
                )
    ...
    def _check_component_auditor_completeness(self):
        for rec in self:
            if rec.component_auditors_involved and not rec.component_auditor_ids:
                raise ValidationError(
                    _(
                        "Component auditors are involved but none have been registered. "
    ...
    def _check_escalation_before_approval(self):
        for rec in self:
            if rec.escalation_required and rec.state in ["partner", "locked"]:
                raise ValidationError(
                    _(
                        "Partner escalation is required due to component auditor independence "
    ...
    def _check_preconditions(self):
        ...
        if not p10 or p10.state != "locked":
            errors.append(
                "P-10 (Related Parties Planning) must be partner-approved and locked."
            )
        ...
        if not p2 or p2.state not in ["partner", "locked"]:
            errors.append(
                "P-2 (Understanding Entity & Group Structure) must be completed."
            )
    ...
    def _validate_mandatory_fields(self):
        ...
        if self.is_group_audit:
            ...
            if not self.component_risk_narrative:
                errors.append(
                    "Section C: Component risk assessment narrative is required"
                )
            ...
            if not self.instructions_documented:
                errors.append(
                    "Section G: Component auditor instructions must be documented"
                )

            # Section H
            if not self.quality_management_considerations:
                errors.append(
                    "Section H: Quality management considerations are required"
                )
            ...
            if not self.group_structure_attachments:
                errors.append(
                    "Section J: Group structure/organogram attachment is required"
                )
            if (
                self.component_auditors_involved
                and not self.component_instructions_attachments
            ):
                errors.append(
                    "Section J: Component auditor instructions must be uploaded"
                )
            ...
            if not self.basis_established_strategy:
                errors.append(
                    "Section K: Confirm basis for group strategy is established"
                )
    ...
    def action_mark_complete(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_("P-11 marked complete and submitted for manager review.")
            )
    ...
    def action_manager_review(self):
        ...
        for rec in self:
            ...
            rec.message_post(
                body=_("P-11 reviewed by manager and forwarded to partner.")
            )
    ...
    def action_partner_approve(self):
        ...
        for rec in self:
            ...
            if rec.escalation_required:
                raise UserError(
                    _(
                        "Cannot approve: Component auditor independence or competence "
            ...
            rec.message_post(
                body=_("P-11 approved by partner and locked. P-12 is now unlocked.")
            )
            rec._unlock_p12()
    ...
    def _unlock_p12(self):
        ...
        self.message_post(
            body=_(
                "P-11 locked successfully. P-12 (Audit Strategy & Plan) is now accessible."
            )
        )
    ...
    def _auto_unlock_p12(self):
        ...
        p12_record = P12.search(
            [("engagement_id", "=", self.engagement_id.id)], limit=1
        )
...
class PlanningP11Component(models.Model):
    ...
    component_name = fields.Char(string="Component Name", required=True, tracking=True)
    ...
    jurisdiction = fields.Char(string="Jurisdiction/Location", required=True)
    ...
    scope_basis = fields.Html(
        string="Basis for Scope Decision", help="Document why this scope was selected"
    )
    ...
    # Materiality
    currency_id = fields.Many2one(
        "res.currency", related="p11_id.currency_id", store=False
    )
    ...
    reporting_deadline = fields.Date(
        string="Reporting Deadline", help="Deadline for component work completion"
    )
    ...
    @api.depends(
        "percentage_group_assets",
        "percentage_group_revenue",
    ...
    def _compute_financial_significance(self):
        ...
        for comp in self:
            ...
            comp.is_significant = (
                comp.percentage_group_assets >= threshold
                or comp.percentage_group_revenue >= threshold
            )
    ...
    def _check_percentages(self):
        for comp in self:
            if comp.percentage_group_assets < 0 or comp.percentage_group_assets > 100:
                raise ValidationError(
                    _("Percentage of group assets must be between 0 and 100.")
                )
            if comp.percentage_group_revenue < 0 or comp.percentage_group_revenue > 100:
                raise ValidationError(
                    _("Percentage of group revenue must be between 0 and 100.")
                )
...
class PlanningP11ComponentRisk(models.Model):
    ...
    response_responsibility = fields.Selection(
        ...
        string="Response Responsibility",
        required=True,
    )
...
class PlanningP11ComponentAuditor(models.Model):
    ...
    auditor_name = fields.Char(
        string="Auditor Name / Firm", required=True, tracking=True
    )
    ...
    independence_notes = fields.Html(string="Independence Evaluation Notes")
    ...
    def _compute_escalation_flag(self):
        for auditor in self:
            auditor.escalation_flag = (
                not auditor.independence_confirmed or not auditor.competence_adequate
            )
        ...
        for auditor in self:
            if auditor.p11_id.state in ["partner", "locked"]:
                if not auditor.independence_confirmed:
                    raise ValidationError(
                        _(
                            'Component auditor "%s": Independence must be confirmed before approval.'
                        )
                ...
                if not auditor.competence_adequate:
                    raise ValidationError(
                        _(
                            'Component auditor "%s": Competence must be confirmed before approval.'
                        )
                        % auditor.auditor_name
                    )
