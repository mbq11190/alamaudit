...
class PlanningP6Risk(models.Model):
    ...
    audit_year = fields.Many2one(
        "audit.year", string="Audit Year", required=True, ondelete="cascade", index=True
    )
    partner_id = fields.Many2one(
        "res.users", string="Engagement Partner", required=True
    )
    ...
    def _compute_can_open(self):
        ...
        for rec in self:
            ...
            p5 = self.env["qaco.planning.p5.materiality"].search(
                [("audit_id", "=", rec.engagement_id.id)], limit=1
            )
    ...
    fs_level_impact_checklist = fields.Boolean(
        string="Impact on audit approach considered?"
    )
    # Section A: Risk Identification Sources (auto-display)
    sources_entity = fields.Boolean(
        string="Entity/Industry (P-2)", default=True, readonly=True
    )
    sources_controls = fields.Boolean(
        string="Internal Controls (P-3)", default=True, readonly=True
    )
    sources_analytics = fields.Boolean(
        string="Analytics (P-4)", default=True, readonly=True
    )
    sources_materiality = fields.Boolean(
        string="Materiality (P-5)", default=True, readonly=True
    )
    sources_prior_year = fields.Boolean(
        string="Prior-Year Issues", default=True, readonly=True
    )
    sources_fraud_brainstorm = fields.Boolean(
        string="Fraud Brainstorming", default=True, readonly=True
    )
    ...
    significant_risks_confirmed = fields.Boolean(
        string="All significant risks identified?"
    )
    rmm_assessed_confirmed = fields.Boolean(string="RMM appropriately assessed?")
    audit_response_basis_confirmed = fields.Boolean(
    ...
    total_risks_count = fields.Integer(
        string="Total Risks", compute="_compute_risk_dashboard_metrics", store=True
    )
    high_risk_count = fields.Integer(
        string="High Risk Count", compute="_compute_risk_dashboard_metrics", store=True
    )
    ...
    low_risk_count = fields.Integer(
        string="Low Risk Count", compute="_compute_risk_dashboard_metrics", store=True
    )
    ...
    @api.depends(
        "risk_line_ids",
        "risk_line_ids.risk_rating",
        "risk_line_ids.is_significant_risk",
        "risk_line_ids.isa_240_fraud_risk",
        "risk_line_ids.account_cycle",
        "risk_line_ids.assertion_type",
    )
    def _compute_risk_dashboard_metrics(self):
        ...
        for rec in self:
            ...
            rec.high_risk_count = len(lines.filtered(lambda line: line.risk_rating == "high"))
            rec.medium_risk_count = len(
                lines.filtered(lambda line: line.risk_rating == "medium")
            )
            rec.low_risk_count = len(lines.filtered(lambda line: line.risk_rating == "low"))
            rec.significant_risks_count = len(
                lines.filtered(lambda line: line.is_significant_risk)
            )
            rec.fraud_risks_count = len(lines.filtered(lambda line: line.isa_240_fraud_risk))
        ...
        if created_count > 0:
            ...
            return {
                ...
                "params": {
                    ...
                    "message": _(
                        f"{created_count} risks have been created from planning phase findings."
                    ),
                    "type": "success",
                    "sticky": False,
                },
            ...
            return {
                ...
                "params": {
                    ...
                    "message": _(
                        "No new risks were identified from P-2/P-3/P-4 findings."
                    ),
                    "type": "info",
                    "sticky": False,
                },
    ...
    def _create_risks_from_p2(self, p2_record):
        ...
        if (
            hasattr(p2_record, "entity_level_risk_factors")
            and p2_record.entity_level_risk_factors
        ):
                    ...
                    self.env["qaco.planning.p6.risk.line"].create(
                        {
                            ...
                            "control_risk": (
                                "high"
                                if deficiency.severity == "material_weakness"
                                else "medium"
                            ),
                            "is_significant_risk": deficiency.severity
                            == "material_weakness",
    ...
    def _create_risks_from_p4(self, p4_record):
        ...
        if hasattr(p4_record, "fs_line_ids"):
            for fs_line in p4_record.fs_line_ids:
                ...
                if fs_line.exceeds_threshold or (
                    hasattr(fs_line, "risk_indicator")
                    and fs_line.risk_indicator == "high"
                ):
                    ...
                    variance_pct = (
                        fs_line.variance_pct if hasattr(fs_line, "variance_pct") else 0
                    )
                    self.env["qaco.planning.p6.risk.line"].create(
                        {
                            "p6_risk_id": self.id,
                            ...
                            "inherent_risk": (
                                "high" if abs(variance_pct) > 20 else "medium"
                            ),
                            "control_risk": "medium",
    ...
    def _auto_unlock_p7(self):
        ...
        if "qaco.planning.p7.fraud" in self.env:
            p7 = self.env["qaco.planning.p7.fraud"].search(
                [("audit_id", "=", self.engagement_id.id)], limit=1
            )
            if p7 and p7.state == "not_started":
                _logger.info(
                    f"P-7 auto-unlock triggered by P-6 approval for audit {self.engagement_id.id}"
                )
        ...
        for rec in self:
            if not rec.attachment_ids:
                raise ValidationError(
                    "Mandatory risk assessment documents must be uploaded."
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            planning = self.env["qaco.planning.main"].browse(
                vals.get("planning_main_id")
            )
            if not planning or not planning.p5_partner_locked:
                raise UserError(
                    "P-6 cannot be started until P-5 is partner-approved and locked."
                )
            # Add checks for P-2, P-3, P-4 outputs
            if (
                not planning.p2_outputs_ready
                or not planning.p3_outputs_ready
                or not planning.p4_outputs_ready
            ):
...
class PlanningP6RiskLine(models.Model):
    ...
    account_cycle = fields.Selection(
        ACCOUNT_CYCLES, string="Account/Cycle", required=True
    )
    # XML view compatible alias
    account_area = fields.Selection(
        ACCOUNT_CYCLES, string="Account Area", related="account_cycle", readonly=False
    )
    risk_description = fields.Text(string="Risk Description", required=True)
    assertion_type = fields.Selection(
        ASSERTION_TYPES, string="Assertion", required=True
    )
    # XML view compatible alias
    assertion = fields.Selection(
        string="Assertion", related="assertion_type", readonly=False
    )
    fs_level_risk = fields.Boolean(
        string="FS-Level Risk", help="Is this a financial statement level risk?"
    )
    ...
    risk_rating = fields.Selection(
        RISK_RATING, string="Combined RMM", compute="_compute_risk_rating", store=True
    )
    # XML view compatible alias
    combined_rmm = fields.Selection(
        string="Combined RMM", related="risk_rating", store=False
    )
    is_significant_risk = fields.Boolean(string="Significant Risk", tracking=True)
    # XML view compatible alias
    significant_risk = fields.Boolean(
        string="Significant Risk", related="is_significant_risk", readonly=False
    )
    ...
    risk_factors = fields.Text(
        string="Risk Factors", help="Factors contributing to this risk"
    )
    ...
    fraud_type = fields.Selection(
        ...
        string="Fraud Risk Type",
        help="ISA 240 fraud risk classification",
    )
    ...
    planned_procedures = fields.Text(
        string="Planned Audit Procedures", help="Procedures to address this risk"
    )
    # XML view compatible alias
    planned_response = fields.Text(
        string="Planned Response", related="planned_procedures", readonly=False
    )
    ...
    def _compute_risk_rating(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-6 _compute_risk_rating failed for record {record.id}: {e}"
                )
    ...
    def _compute_senior_involvement(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-6 _compute_senior_involvement failed for record {record.id}: {e}"
                )
    ...
    def _compute_extended_procedures(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"P-6 _compute_extended_procedures failed for record {record.id}: {e}"
                )
