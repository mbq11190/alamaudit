...
class PlanningP10RelatedParties(models.Model):
    ...
    significant_rpt_identified = fields.Boolean(
        string="Significant RPT Identified", tracking=True
    )
    ...
    understanding_source = fields.Html(
        string="Sources of Information", help="Sources used to identify related parties"
    )
    # XML view compatible alias
    ...
    family_relationships = fields.Html(
        string="Family Relationships", help="Close family members of key management"
    )
    ...
    transaction_types = fields.Html(
        string="Transaction Types", help="Types of transactions with related parties"
    )
    ...
    rpt_risk_assessment = fields.Html(
        string="RPT Risk Assessment", related="rp_risk_assessment", readonly=False
    )
    ...
    risk_assessment_procedures = fields.Html(
        string="Risk Assessment Procedures", help="Procedures for assessing RPT risks"
    )
    ...
    substantive_procedures = fields.Html(
        string="Substantive Procedures", help="Substantive audit procedures for RPT"
    )
    ...
    disclosure_requirements = fields.Html(
        string="Disclosure Requirements", help="Applicable disclosure requirements"
    )
    ...
    ias24_compliance = fields.Html(
        string="IAS 24 Compliance", help="Assessment of IAS 24 compliance"
    )
    ...
    representations_obtained = fields.Html(
        string="Representations Obtained", help="Details of representations obtained"
    )
    ...
    rpt_conclusion = fields.Html(
        string="Related Parties Conclusion", related="rp_risk_summary", readonly=False
    )
    ...
    isa_reference = fields.Char(
        string="ISA Reference", default="ISA 550", readonly=True
    )
    ...
    _sql_constraints = [
        (
            "audit_unique",
            "UNIQUE(audit_id)",
            "Only one P-10 record per Audit Engagement is allowed.",
        )
    ]
...
"""
    ...
    def _validate_mandatory_fields(self):
        ...
        if self.completeness_procedures_count < 2:
            errors.append(
                "Section B: At least 2 completeness procedures must be performed per ISA 550.13"
            )
        if not self.completeness_results:
            errors.append(
                "Section B: Results of completeness procedures must be documented (MANDATORY)"
            )
        ...
        if not self.declarations_attachment_ids:
            errors.append(
                "Section I: Management declarations of interest must be uploaded (MANDATORY)"
            )
        if not self.board_minutes_attachment_ids:
            errors.append(
                "Section I: Board/audit committee minutes must be uploaded (MANDATORY)"
            )
        if not self.rpt_contracts_attachment_ids:
            errors.append(
                "Section I: RPT agreements/contracts must be uploaded (MANDATORY)"
            )
        if not self.prior_year_rpt_attachment_ids:
            errors.append(
                "Section I: Prior-year RPT schedules must be uploaded (MANDATORY)"
            )
    ...
    def action_approve(self):
        for record in self:
            ...
            record.message_post(
                body="P-10 Related Parties Planning approved by Partner."
            )
    ...
    @api.depends(
        "completeness_inquiry_mgmt",
        "completeness_board_minutes",
        "completeness_declarations",
        "completeness_shareholder_records",
    ...
    def _compute_significant_rpt_flags(self):
        ...
        for record in self:
            ...
            record.unusual_rpt_flagged = (
                record.significant_rpt_identified or record.rpt_fraud_risk_identified
            )

    @api.depends(
        "rpt_fraud_risk_identified",
        "management_dominance_indicators",
        "circular_transactions_identified",
        "rpt_critical_to_liquidity",
    )
...
class PlanningP10RelatedPartyLine(models.Model):
    ...
    relationship_details = fields.Text(string="Relationship Details")
    transaction_types = fields.Char(string="Types of Transactions")
    transaction_volume = fields.Monetary(
        string="Estimated Transaction Volume", currency_field="currency_id"
    )
    currency_id = fields.Many2one(
        "res.currency", related="p10_related_parties_id.currency_id"
    )
    outstanding_balances = fields.Monetary(
        string="Outstanding Balances", currency_field="currency_id"
    )
    ...
    arms_length = fields.Boolean(
        string="At Arm's Length", help="Are transactions at arm's length?"
    )
...
class RelatedPartyLine(models.Model):
    ...
    relationship_nature = fields.Char(string="Nature of Relationship")
...
class RptTransactionLine(models.Model):
    ...
    transaction_description = fields.Char(string="Description")
    period = fields.Selection(
        ...
        string="Period",
        help="Period when transaction occurred",
    )
    transaction_amount = fields.Monetary(string="Amount", currency_field="currency_id")
    currency_id = fields.Many2one(
        "res.currency", related="p10_related_parties_id.currency_id"
    )
    terms_conditions = fields.Text(string="Terms & Conditions")
    business_rationale = fields.Text(string="Business Rationale")
    arms_length = fields.Boolean(string="At Arm's Length")
