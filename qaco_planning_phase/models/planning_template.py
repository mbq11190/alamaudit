...
class PlanningTemplate(models.Model):
    ...
    description = fields.Text(
        string="Description", help="Template usage guidance and applicability"
    )
    ...
    def action_apply_to_planning(self, planning_main_id):
        ...
        if "p4_analytical_procedures" in data and planning.p4_analytical_id:
            self._apply_p4_procedures(
                planning.p4_analytical_id, data["p4_analytical_procedures"]
            )
    ...
    def _apply_p2_risks(self, p2_entity, risks_data):
        ...
        _logger.info(
            f"Applied {len(risks_data)} P-2 business risks from template to {p2_entity.audit_id.name}"
        )
    ...
    def _apply_p4_procedures(self, p4_analytical, procedures_data):
        ...
        _logger.info(
            f"Applied {len(procedures_data)} P-4 analytical procedures from template to {p4_analytical.audit_id.name}"
        )
...
class PlanningTemplateWizard(models.TransientModel):
    ...
    industry_type = fields.Selection(
        related="template_id.industry_type", string="Industry", readonly=True
    )
    description = fields.Text(
        related="template_id.description", string="Template Description", readonly=True
    )
