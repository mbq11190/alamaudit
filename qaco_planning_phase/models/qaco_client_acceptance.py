...
class ClientAcceptance(models.Model):
    ...
    # -------------------------------------------------------------------------
    # SECTION A – BASIC CLIENT & ENGAGEMENT INFORMATION
    # -------------------------------------------------------------------------
    ...
    engagement_type = fields.Selection(
        [
            ...
            ("special_purpose", "Special Purpose Engagement"),
    ...
    # A – checklist
    companies_act_compliance_checked = fields.Boolean(
        string="Companies Act 2017 Appointment/Removal Checked"
    )
    regulator_requirements_checked = fields.Boolean(
        string="Regulator Requirements (SECP/PSX/SBP/Insurance) Checked"
    )
    aob_registration_required = fields.Boolean(
        string="AOB Registration Required"
    )
    aob_registration_confirmed = fields.Boolean(
        string="AOB Registration Confirmed"
    )

    # -------------------------------------------------------------------------
    # SECTION B – NEW CLIENT REQUEST / LEAD RECORD
    # -------------------------------------------------------------------------
    ...
    initial_red_flags = fields.Text(string="Initial Red Flags / Concerns")
    ...
    # -------------------------------------------------------------------------
    # SECTION C – PRE-ACCEPTANCE RISK (INTEGRITY / AML / NOCLAR)
    # -------------------------------------------------------------------------
    ownership_structure_summary = fields.Text(string="Ownership Structure Summary")
    pep_sanction_check_done = fields.Boolean(string="PEP / Sanctions Check Done")
    pep_sanction_findings = fields.Text(string="PEP / Sanctions Findings")
    ...
    regulatory_history_summary = fields.Text(
        string="Regulatory History (SECP / PSX / SBP / FBR)"
    )
    ...
    cdd_completed = fields.Boolean(string="CDD / KYC Completed")
    ...
    noclar_action_plan = fields.Text(string="NOCLAR Action / Follow-Up")

    public_records_reviewed = fields.Boolean(string="Public Records Reviewed (SECP/PSX/FBR/SBP)")
    adverse_media_screen_performed = fields.Boolean(string="Adverse Media Screening Performed")
    aml_checklist_completed = fields.Boolean(string="AML Checklist Completed")
    noclar_evaluation_documented = fields.Boolean(string="NOCLAR Evaluation Documented")

    # -------------------------------------------------------------------------
    # SECTION D – INDEPENDENCE & CONFLICT-OF-INTEREST
    # -------------------------------------------------------------------------
    ...
    non_audit_services_summary = fields.Text(string="Non-Audit Services Summary")
    relationships_affecting_independence = fields.Text(
        string="Relationships Affecting Independence"
    )

    independence_safeguards = fields.Text(string="Safeguards Applied")
    ...
    conflict_client_consents = fields.Boolean(string="Client Consents to Conflict Handling")

    annual_independence_confirmation_obtained = fields.Boolean(
        string="Annual Independence Confirmation Obtained"
    )
    non_audit_services_evaluated = fields.Boolean(string="Non-Audit Services Evaluated")
    fee_dependency_evaluated = fields.Boolean(string="Fee Dependency Evaluated")
    eqcr_required_for_pie = fields.Boolean(string="EQCR Required for PIE")
    eqcr_assigned = fields.Boolean(string="EQCR Assigned")

    # -------------------------------------------------------------------------
    # SECTION E – PREVIOUS AUDITOR COMMUNICATION
    # -------------------------------------------------------------------------
    clearance_letter_sent = fields.Boolean(string="Professional Clearance Letter Sent")
    clearance_letter_date = fields.Date(string="Clearance Letter Date")
    ...
    clearance_response_received = fields.Boolean(string="Response Received")
    clearance_response_date = fields.Date(string="Response Date")
    clearance_response_summary = fields.Text(string="Response Summary")
    no_response_followup = fields.Text(string="No Response Follow-Up / Attempts")

    professional_clearance_letter_issued = fields.Boolean(
        string="Professional Clearance Letter Issued"
    )
    response_reviewed_by_partner = fields.Boolean(
        string="Response Reviewed by Partner"
    )
    issues_discussed_with_management = fields.Boolean(
        string="Issues Discussed with Management"
    )
    impact_on_acceptance_documented = fields.Boolean(
        string="Impact on Acceptance Documented"
    )

    # -------------------------------------------------------------------------
    # SECTION F – ENGAGEMENT RISK EVALUATION
    # -------------------------------------------------------------------------
    ...
    control_environment_risk = fields.Selection(
        [("low", "Low"), ("medium", "Medium"), ("high", "High")],
        string="Control / Compliance Environment Risk",
    )
    litigation_regulatory_risk = fields.Selection(
        [("low", "Low"), ("medium", "Medium"), ("high", "High")],
        string="Litigation / Regulatory Risk",
    )
    going_concern_indicators = fields.Boolean(string="Going Concern Indicators Present")
    going_concern_summary = fields.Text(string="Going Concern Summary")
    ...
    risk_assessment_reviewed_by_manager = fields.Boolean(
        string="Risk Assessment Reviewed by Manager"
    )
    high_risk_engagement_requires_eqcr = fields.Boolean(
        string="High-Risk Engagement Requires EQCR"
    )
    high_risk_escalated_to_quality_partner = fields.Boolean(
        string="High-Risk Escalated to Quality Partner"
    )

    # -------------------------------------------------------------------------
    # SECTION G – DECISION & APPROVALS
    # -------------------------------------------------------------------------
    engagement_status = fields.Selection(
        [
            ("new", "New Engagement"),
            ("continuing", "Continuing Engagement"),
    ...
    decision_conditions = fields.Text(string="Conditions / Reservations")
    ...
    quality_partner_review_required = fields.Boolean(string="Quality Partner Review Required")
    quality_partner_id = fields.Many2one(
        "res.users",
        string="Quality / ISQM Partner",
    )
    quality_partner_review_date = fields.Date(string="Quality Review Date")

    decision_documented = fields.Boolean(string="Decision Documented")
    engagement_letter_to_be_issued = fields.Boolean(
        string="Engagement Letter to be Issued"
    )
    isqm1_policies_followed = fields.Boolean(
        string="ISQM 1 Policies Followed (Acceptance/Continuance)"
    )
    for_pie_aob_requirements_checked = fields.Boolean(
        string="For PIE: AOB Requirements Checked"
    )

    # -------------------------------------------------------------------------
    # BUTTON ACTIONS / BUSINESS RULES
    # -------------------------------------------------------------------------
    def action_submit_review(self):
        """Move from draft to under review."""
        for rec in self:
            if rec.state == "draft":
                rec.state = "under_review"

    def action_approve(self):
        """Partner approval with basic validation."""
        for rec in self:
            if rec.independence_acceptable == "no":
                raise ValidationError(
                    _("Cannot approve: independence is marked as 'No'.")
                )
            if rec.decision not in ("accept", "continue"):
                raise ValidationError(
                    _("Decision must be 'Accept' or 'Continue' before approval.")
                )
            if not rec.engagement_partner_id:
                raise ValidationError(
                    _("Engagement partner must be set before approval.")
                )
            if not rec.engagement_partner_decision_date:
                rec.engagement_partner_decision_date = fields.Date.context_today(self)

            rec.state = "approved"

    def action_reject(self):
        """Set status to rejected / closed."""
        for rec in self:
            rec.state = "rejected"
    ...
    def _onchange_client_id(self):
        for rec in self:
            if rec.client_id and not rec.legal_name:
                rec.legal_name = rec.client_id.name
from odoo import api, fields, models, _
from odoo.exceptions import ValidationError
...
class ClientAcceptance(models.Model):
    ...
    # === A. Basic Client & Engagement Information ===
    ...
    # === C. Pre-Acceptance Risk Assessment (Integrity / AML / NOCLAR) ===
    ownership_structure_summary = fields.Text(string="Ownership Structure Summary")
    pep_sanction_check_done = fields.Boolean(string="PEP / Sanctions Check Done")
    pep_sanction_findings = fields.Text(string="PEP / Sanctions Findings")
    adverse_media_found = fields.Boolean(string="Adverse Media Found")
    adverse_media_summary = fields.Text(string="Adverse Media Summary")
    regulatory_history_summary = fields.Text(string="Regulatory History (SECP / PSX / SBP / FBR)")
    aml_risk_level = fields.Selection(
        [("low", "Low"), ("medium", "Medium"), ("high", "High")],
        string="AML Risk Level",
    )
    ...
    independence_safeguards = fields.Text(string="Independence Safeguards")
    independence_acceptable = fields.Selection(
        [
            ("yes", "Yes"),
            ("no", "No"),
    ...
    # === E. Previous Auditor Communication ===
    clearance_letter_sent = fields.Boolean(string="Professional Clearance Letter Sent")
    clearance_letter_date = fields.Date(string="Letter Date")
    clearance_letter_mode = fields.Selection(
        [
            ("email", "Email"),
            ("registered_post", "Registered Post"),
    ...
    engagement_partner_decision_date = fields.Date(string="Partner Decision Date")
    quality_partner_id = fields.Many2one("res.users", string="Quality / ISQM Partner")
    quality_partner_review_required = fields.Boolean(string="Quality Partner Review Required")
    quality_partner_review_date = fields.Date(string="Quality Partner Review Date")
    ...
    def action_approve(self):
        for rec in self:
            if rec.independence_acceptable == "no":
                raise ValidationError(
                    _("Cannot approve: independence is not acceptable.")
                )
            if not rec.decision or rec.decision not in ("accept", "continue"):
                raise ValidationError(
                    _("Decision must be Accept or Continue before approval.")
                )
            if not rec.engagement_partner_id:
            ...
            if not rec.engagement_partner_decision_date:
                rec.engagement_partner_decision_date = fields.Date.context_today(rec)
            rec.state = "approved"
