...
class PlanningPhase(models.Model):
    ...
    name = fields.Char(
        string="Planning Reference", compute="_compute_name", store=True, readonly=True
    )
    ...
    understanding_status = fields.Selection(
        ...
        default="red",
        tracking=True,
        string="Entity Understanding Status",
    )
    analytics_status = fields.Selection(
        SUBTAB_STATUS, default="red", tracking=True, string="Analytics Status"
    )
    materiality_status = fields.Selection(
        SUBTAB_STATUS, default="red", tracking=True, string="Materiality Status"
    )
    control_status = fields.Selection(
        SUBTAB_STATUS, default="red", tracking=True, string="Controls Status"
    )
    risk_status = fields.Selection(
        SUBTAB_STATUS, default="red", tracking=True, string="Risk Assessment Status"
    )
    completion_status = fields.Selection(
        SUBTAB_STATUS, default="red", tracking=True, string="Completion Status"
    )

    audit_type = fields.Selection(
        AUDIT_TYPES, string="Type of Audit", required=True, default="statutory"
    )
    ...
    audit_tenure_years = fields.Integer(
        string="Audit Tenure (Years)", compute="_compute_audit_tenure"
    )

    industry_id = fields.Many2one(
        "qaco.industry", string="Industry Classification", tracking=True
    )
    industry_type = fields.Selection(
        INDUSTRY_TYPES, string="Industry Sector", default="other"
    )
    business_model = fields.Html(string="Business Model & Revenue Sources")
    organizational_structure = fields.Html(
        string="Organizational Structure & Key Personnel"
    )
    ...
    applicable_regulators = fields.Many2many(
        "qaco.regulator", string="Applicable Regulators"
    )
    regulatory_filings_status = fields.Selection(
        ...
        string="Regulatory Filings Status",
        tracking=True,
    )
    regulatory_penalties = fields.Html(
        string="Regulatory Penalties/Litigations History"
    )
    ...
    significant_fluctuations = fields.Html(
        string="Significant Fluctuations Analysis (>10% or Materiality)"
    )
    ...
    materiality_benchmark = fields.Selection(
        MATERIALITY_BENCHMARKS, string="Materiality Benchmark", default="pbt"
    )
    materiality_percentage = fields.Float(
        string="Materiality Percentage (%)", default=5.0
    )
    materiality_justification = fields.Html(string="Benchmark Selection Justification")
    custom_benchmark_value = fields.Monetary(
        string="Custom Benchmark Value", currency_field="company_currency_id"
    )
    ...
    control_environment_rating = fields.Selection(
        CONTROL_RATING, string="Control Environment", default="none"
    )
    entity_level_controls_rating = fields.Selection(
        CONTROL_RATING, string="Entity-Level Controls", default="none"
    )
    risk_assessment_rating = fields.Selection(
        CONTROL_RATING, string="Risk Assessment Process", default="none"
    )
    itgc_rating = fields.Selection(
        CONTROL_RATING, string="IT General Controls", default="none"
    )
    control_activities_rating = fields.Selection(
        CONTROL_RATING, string="Control Activities", default="none"
    )
    monitoring_controls_rating = fields.Selection(
        CONTROL_RATING, string="Monitoring of Controls", default="none"
    )
    ...
    risk_register_line_ids = fields.One2many(
        "qaco.risk.register.line", "planning_phase_id", string="Risk Register"
    )
    ...
    component_auditors_involved = fields.Boolean(
        string="Component Auditors Involved (ISA 600)"
    )
    component_auditors_details = fields.Html(
        string="Component Auditors Coordination Plan"
    )
    ...
    significant_risks_identified = fields.Integer(
        string="Significant Risks Count", compute="_compute_significant_risks"
    )

    planning_memo_summary = fields.Html(string="Planning Memorandum Executive Summary")
    icap_compliance_check = fields.Boolean(
        string="ICAP Standards Compliance Verified", default=False
    )
    secp_requirements_met = fields.Boolean(
        string="SECP Requirements Addressed", default=False
    )
    aob_guidelines_considered = fields.Boolean(
        string="AOB Guidelines Considered", default=False
    )
    ...
    checklist_engagement_letter = fields.Boolean(
        string="✓ Engagement Letter Executed & Uploaded", tracking=True
    )
    checklist_entity_understanding = fields.Boolean(
        string="✓ Entity Understanding Complete (ISA 315)", tracking=True
    )
    checklist_fraud_brainstormed = fields.Boolean(
        string="✓ Fraud Brainstorming Documented (ISA 240)", tracking=True
    )
    checklist_analytics_performed = fields.Boolean(
        string="✓ Analytical Procedures Performed (ISA 520)", tracking=True
    )
    checklist_materiality_finalized = fields.Boolean(
        string="✓ Materiality Finalized & Justified (ISA 320)", tracking=True
    )
    checklist_controls_assessed = fields.Boolean(
        string="✓ Internal Controls Assessed (ISA 315)", tracking=True
    )
    checklist_risks_linked = fields.Boolean(
        string="✓ Risks Linked to Audit Programs", tracking=True
    )
    checklist_strategy_signed = fields.Boolean(
        string="✓ Audit Strategy Memo Approved", tracking=True
    )
    checklist_independence_confirmed = fields.Boolean(
        string="✓ Independence Confirmed (Code of Ethics)", tracking=True
    )
    checklist_partner_manager_review = fields.Boolean(
        string="✓ Partner/Manager Review Completed", tracking=True
    )
    checklist_icap_standards = fields.Boolean(
        string="✓ ICAP Standards Compliance Verified", tracking=True
    )
    checklist_secp_requirements = fields.Boolean(
        string="✓ SECP Requirements Addressed", tracking=True
    )
    checklist_aob_guidelines = fields.Boolean(
    ...
    planning_complete = fields.Boolean(
        string="Planning Phase Complete", default=False, tracking=True, copy=False
    )
    planning_completed_on = fields.Datetime(
        string="Planning Completed On", tracking=True, copy=False, readonly=True
    )
    can_finalize_planning = fields.Boolean(
        string="Can Finalize Planning", compute="_compute_can_finalize", store=True
    )
    missing_requirements_note = fields.Html(
        string="Outstanding Planning Requirements",
        compute="_compute_can_finalize",
        store=True,
    )
    ...
    def _compute_name(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"Planning Phase _compute_name failed for record {record.id}: {e}"
                )
                record.name = "Planning Phase - Draft"
    ...
    def _compute_audit_tenure(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"Planning Phase _compute_audit_tenure failed for record {record.id}: {e}"
                )
                record.audit_tenure_years = 0

    @api.depends(
        "control_environment_rating", "control_activities_rating", "itgc_rating"
    )
    def _compute_reliance_strategy(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning(
                    f"Planning Phase _compute_reliance_strategy failed for record {record.id}: {e}"
                )
    ...
    def _compute_significant_risks(self):
        ...
        for record in self:
            try:
                ...
                record.significant_risks_identified = len(
                    record.risk_register_line_ids.filtered(
                        lambda r: r.is_significant_risk
                    )
                )
            except Exception as e:
                _logger.warning(
                    f"Planning Phase _compute_significant_risks failed for record {record.id}: {e}"
                )
                record.significant_risks_identified = 0
    ...
    @api.depends(
        ...
        "checklist_icap_standards",
        "checklist_secp_requirements",
        "checklist_aob_guidelines",
    ...
    def _compute_can_finalize(self):
        ...
        for record in self:
            statuses_ready = all(
                getattr(record, field) == "green" for field in status_fields
            )
            attachments_ready = all(
                bool(getattr(record, field)) for field in attachment_fields
            )
    ...
    def _ensure_statuses_green(self):
        ...
        if incomplete:
            raise UserError(
                f"Complete these sub-tabs before sign-off: {', '.join(incomplete)}"
            )
    ...
    def _validate_materiality_parameters(self):
        ...
        if not self.materiality_justification:
            raise UserError(
                "Justification for benchmark selection is mandatory for audit documentation."
            )
        if not self.materiality_percentage or self.materiality_percentage <= 0:
            raise UserError(
                "Valid materiality percentage must be specified (typically 1-10%)."
            )
        if self.materiality_benchmark == "custom" and not self.custom_benchmark_value:
            raise UserError(
                "Custom benchmark value is required when using custom benchmark."
            )
    ...
    def _validate_risk_assessment(self):
        ...
        if not self.risk_register_line_ids:
            raise UserError(
                "Risk register must contain at least one identified risk as per ISA 315."
            )
        if not any(line.is_significant_risk for line in self.risk_register_line_ids):
            raise UserError(
                "At least one significant risk must be identified in the risk register."
            )
    ...
    def action_mark_planning_complete(self):
        ...
        if not self.can_finalize_planning:
            raise UserError(
                "Cannot complete planning - all prerequisites must be satisfied."
            )
        self._validate_materiality_parameters()
        self._validate_risk_assessment()
    ...
    def _check_materiality_percentage(self):
        for record in self:
            if record.materiality_percentage and not (
                0.1 <= record.materiality_percentage <= 20
            ):
                raise ValidationError(
                    "Materiality percentage should typically be between 0.1% and 20% as per ISA guidance."
                )
    ...
    def _check_planning_complete_guard(self):
        for record in self:
            if record.planning_complete and not record.can_finalize_planning:
                raise ValidationError(
                    "Planning cannot be marked complete until every prerequisite is satisfied for zero-deficiency compliance."
                )
...
class RiskRegisterLine(models.Model):
    ...
    assertion_type = fields.Selection(
        PlanningPhase.ASSERTION_TYPES, string="Assertion Level Risk"
    )
    risk_rating = fields.Selection(
        PlanningPhase.RISK_RATING, string="Risk Rating", required=True
    )
