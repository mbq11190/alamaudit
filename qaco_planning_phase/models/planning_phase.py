...
class PlanningPhase(models.Model):
	...
	audit_period_from = fields.Date(string='Audit Period From', required=True)
	audit_period_to = fields.Date(string='Audit Period To', required=True)
	financial_year_end = fields.Date(string='Financial Year End', required=True)
	...
	industry_type = fields.Selection(INDUSTRY_TYPES, string='Industry Sector', required=True)
	...
	materiality_benchmark = fields.Selection(MATERIALITY_BENCHMARKS, string='Materiality Benchmark', required=True)
	materiality_percentage = fields.Float(string='Materiality Percentage (%)', required=True, default=5.0)
	materiality_justification = fields.Html(string='Benchmark Selection Justification', required=True)
	...
	control_environment_rating = fields.Selection(CONTROL_RATING, string='Control Environment', required=True)
	entity_level_controls_rating = fields.Selection(CONTROL_RATING, string='Entity-Level Controls', required=True)
	risk_assessment_rating = fields.Selection(CONTROL_RATING, string='Risk Assessment Process', required=True)
	itgc_rating = fields.Selection(CONTROL_RATING, string='IT General Controls', required=True)
	control_activities_rating = fields.Selection(CONTROL_RATING, string='Control Activities', required=True)
	monitoring_controls_rating = fields.Selection(CONTROL_RATING, string='Monitoring of Controls', required=True)
	...
	audit_approach = fields.Html(string='Overall Audit Approach & Strategy', required=True)
