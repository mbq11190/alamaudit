...
class PlanningP7Fraud(models.Model):
    ...
    brainstorming_date = fields.Date(string="Brainstorming Date", tracking=True)
    ...
    brainstorm_fs_susceptibility = fields.Boolean(
        string="☐ Susceptibility of FS to fraud discussed", help="ISA 240.15(a)"
    )
    brainstorm_management_override = fields.Boolean(
        string="☐ Management override risk discussed", help="ISA 240.31"
    )
    brainstorm_revenue_recognition = fields.Boolean(
        string="☐ Revenue recognition risks discussed", help="ISA 240.26"
    )
    brainstorm_unpredictability = fields.Boolean(
        string="☐ Unpredictability incorporated", help="ISA 240.30"
    )
    ...
    management_inquiries_performed = fields.Boolean(
        string="Management Inquiries Performed?", default=False, tracking=True
    )
    ...
    tcwg_inquiries_performed = fields.Boolean(
        string="TCWG Inquiries Performed?", default=False, tracking=True
    )
    ...
    inquiry_documented = fields.Boolean(
        string="☐ Inquiries documented", help="All fraud inquiries properly documented"
    )
    ...
    pressure_factors = fields.Html(
        string="Pressure Factors", related="incentive_factors", readonly=False
    )
    ...
    rationalization_factors = fields.Html(
        string="Rationalization Factors", related="attitude_factors", readonly=False
    )
    ...
    fraud_risk_factors = fields.Html(
        string="Fraud Risk Factors", help="Documented fraud risk factors identified"
    )
    ...
    overall_fraud_response = fields.Html(
        string="Overall Fraud Response", related="fraud_responses", readonly=False
    )
    ...
    other_inquiries = fields.Html(
        string="Other Inquiries", help="Other fraud-related inquiries documented"
    )
    # XML view compatible aliases for inquiries
    management_inquiries = fields.Html(
        string="Management Inquiries", related="management_inquiry", readonly=False
    )
    tcwg_inquiries = fields.Html(
        string="TCWG Inquiries", related="tcwg_inquiry", readonly=False
    )
    ...
    fraud_conclusion = fields.Html(
        string="Fraud Conclusion", related="fraud_risk_summary", readonly=False
    )
    overall_fraud_risk = fields.Selection(
        ...
        string="Overall Fraud Risk Assessment",
        tracking=True,
    )
    ...
    isa_reference = fields.Char(
        string="ISA Reference", default="ISA 240", readonly=True
    )
    ...
    _sql_constraints = [
        (
            "audit_unique",
            "UNIQUE(audit_id)",
            "Only one P-7 record per Audit Engagement is allowed.",
        )
    ]
...
"""
    ...
    def _compute_fraud_risks_identified(self):
        ...
        for record in self:
            try:
                ...
                record.fraud_risks_identified = (
                    len(record.fraud_risk_line_ids) if record.fraud_risk_line_ids else 0
                )
            except Exception as e:
                _logger.warning(
                    f"P-7 _compute_fraud_risks_identified failed for record {record.id}: {e}"
                )
    ...
    def _auto_unlock_p8(self):
        ...
        p8_record = P8.search([("engagement_id", "=", self.engagement_id.id)], limit=1)
...
class PlanningP7FraudLine(models.Model):
    ...
    fraud_scenario = fields.Text(
        string="Fraud Scenario", help="Specific how/what/when fraud scenario"
    )
    ...
    fs_area = fields.Selection(
        ...
        string="FS Area/Account Cycle",
        help="Link to P-6 RMM",
    )
    ...
    source = fields.Selection(
        ...
        string="Source of Risk",
        help="Traceability to prior planning",
    )
    ...
    affected_area = fields.Char(string="Affected Area/Account")
