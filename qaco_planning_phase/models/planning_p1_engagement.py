    ...
    def _compute_total_hours(self):
        ...
        for rec in self:
            try:
                ...
                _logger.warning(
                    f"P-1 Budget _compute_total_hours failed for record {rec.id}: {e}"
                )
                rec.total_hours = 0
...
class PlanningP1Engagement(models.Model):
    ...
    reporting_framework = fields.Selection(
        ...
        string="Applicable Financial Reporting Framework",
        default="ifrs",
        tracking=True,
    )
    ...
    def _compute_team_summary(self):
        ...
        for rec in self:
            try:
                ...
                rec.total_assigned_hours = sum(
                    rec.team_member_ids.mapped("assigned_hours")
                )
            except Exception as e:
                _logger.warning(
                    f"P-1 _compute_team_summary failed for record {rec.id}: {e}"
                )
                rec.total_team_members = 0
                rec.total_assigned_hours = 0
    ...
    @api.depends(
        "time_budget_line_ids",
        "time_budget_line_ids.partner_hours",
        "time_budget_line_ids.manager_hours",
        "time_budget_line_ids.senior_hours",
    ...
    def _check_preconditions(self):
        ...
        if "qaco.client.onboarding" in self.env:
            ...
            if not onboarding:
                errors.append(
                    "Client onboarding must be completed before P-1 can begin."
                )
            elif (
                hasattr(onboarding, "partner_approved")
                and not onboarding.partner_approved
            ):
                errors.append(
                    "Client onboarding must be partner-approved before P-1 can begin."
                )
    ...
    def _validate_mandatory_fields(self):
        ...
        if self.eqcr_required and not self.eqcr_partner_id:
            errors.append(
                "EQCR Partner must be assigned when EQCR is required (Section B)"
            )
        ...
        if self.new_threats_identified and not self.safeguards_applied:
            errors.append(
                "Safeguards must be documented when threats identified (Section F)"
            )
        ...
        if self.eqcr_required and not self.eqcr_appointment_attachment_ids:
            errors.append(
                "EQCR Appointment must be uploaded when EQCR required (Section H)"
            )
    ...
    def action_complete(self):
        ...
        for rec in self:
            ...
            rec.state = "completed"
            rec.message_post(body=f"P-1 marked as completed by {self.env.user.name}.")
    ...
    def action_manager_review(self):
        ...
        for rec in self:
            ...
            rec.state = "reviewed"
            rec.message_post(body=f"P-1 reviewed by Manager: {self.env.user.name}.")
