...
class PlanningP2Entity(models.Model):
    ...
    # Section B Checklist
    check_b_control_environment = fields.Boolean(
        string="Control environment understood"
    )
    check_b_governance = fields.Boolean(string="Governance structure assessed")
    check_b_management_override = fields.Boolean(
        string="Risk of management override considered"
    )
    ...
    check_d_going_concern = fields.Boolean(
        string="Going-concern indicators flagged (if any)"
    )
    ...
    # Section I Checklist
    check_i_changes_identified = fields.Boolean(
        string="All significant changes identified"
    )
    ...
    def _compute_risk_counts(self):
        for rec in self:
            ...
            rec.high_risk_count = len(
                rec.business_risk_ids.filtered(lambda r: r.severity == "high")
            )
    ...
    def _check_preconditions(self):
        ...
        if "qaco.planning.p1.engagement" in self.env:
            ...
            if not p1:
                ...
                errors.append(
                    "P-1: Engagement Setup must be partner-approved before P-2 can begin."
                )
    ...
    def _validate_mandatory_fields(self):
        ...
        if not self.confirm_risks_identified:
            errors.append(
                "Risks must be confirmed as adequately identified (Section L)"
            )
    ...
    def action_complete(self):
        ...
        for rec in self:
            ...
            rec.state = "completed"
            rec.message_post(body=f"P-2 marked as completed by {self.env.user.name}.")
    ...
    def action_manager_review(self):
        ...
        for rec in self:
            ...
            rec.state = "reviewed"
            rec.message_post(body=f"P-2 reviewed by Manager: {self.env.user.name}.")
    ...
    def action_generate_entity_memo(self):
        ...
        self.ensure_one()
        self.message_post(
            body="Entity & Environment Understanding Memorandum generated."
        )
        return {
