...
class TestAIAuditWorkflows(TransactionCase):
    ...
    def test_reports_actions_exist(self):
        ...
        self.assertTrue(
            self.env.ref(
                "ai_audit_management.report_engagement_letter", raise_if_not_found=False
            )
        )
        self.assertTrue(
            self.env.ref(
                "ai_audit_management.report_management_rep_letter",
                raise_if_not_found=False,
            )
        )
        self.assertTrue(
            self.env.ref(
                "ai_audit_management.report_tcw_report", raise_if_not_found=False
            )
        )
        self.assertTrue(
            self.env.ref(
                "ai_audit_management.report_audit_final", raise_if_not_found=False
            )
        )
