...
class AIAuditController(http.Controller):
    ...
    def chatbot(self, prompt=None):
        ...
        engine = request.env['audit.ai.helper.mixin']
        # Use a sudoed transient helper to reuse _call_openai logic safely
        helper = request.env['audit.ai.helper.mixin'].sudo()
        try:
            reply = helper._call_openai(prompt, model=request.env['audit.ai.settings'].sudo().get_active_settings().model_version)
    ...
    def vision_ocr(self, document_id=None):
        ...
        doc.action_run_ocr()
        return {'status': 'processed', 'text': doc.extracted_text, 'summary': doc.ai_summary}
