...
class AuditAIHelperMixin(models.AbstractModel):
    ...
    def _call_openai(self, prompt, temperature=None, model=None):
        ...
        if not settings or not settings.openai_api_key:
            raise UserError(
                _(
                    "Configure an OpenAI API key in the AI settings panel before using automation."
                )
            )
        if openai is None:
            raise UserError(
                _("The python-openai library is not installed on this server.")
            )
...
class AuditCollaborationMixin(models.AbstractModel):
    ...
    def schedule_review_activity(
        self, summary, activity_type_xmlid="mail.mail_activity_data_todo"
    ):
        for record in self:
