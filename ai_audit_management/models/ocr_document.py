...
class AuditOCRDocument(models.Model):
    ...
    attachment_id = fields.Many2one('ir.attachment', string='Source File', required=True)
    ...
    def action_run_ocr(self):
        ...
        for record in self:
            ...
            base_info = _('File name: %s, type: %s') % (record.attachment_id.name, record.document_type)
            ...
            record.ai_sampling_suggestion = record._call_openai('Suggest a sampling approach for this document type: %s' % record.document_type)
        return True
