...
class AuditDashboardMetrics(models.Model):
    ...
    control_deficiency_heat = fields.Integer(string='Control Deficiency Heat', tracking=True)
    ...
    def update_from_ai(self):
        ...
        if not record:
            record = self.create({})
        prompt = "Generate concise KPI insights for audit portfolio: high risk clients, control issues, exceptions, forecast." \
                 " Return bullets only."
