...
field_assign_re = lambda field: re.compile(r"^\s*%s\s*=\s*" % re.escape(field), re.M)
    ...
    for m in one2many_re.finditer(text):
        ...
        for mf in model_files:
            ...
            if field_assign_re(inverse).search(mtext):
                ...
                break
        if not ok:
            issues.append((str(path), comodel, inverse, 'MISSING_FIELD', [str(x) for x in model_files]))
...
for it in issues:
    if it[3] == 'MISSING_MODEL':
        ...
        models = ', '.join(it[4])
        print(f"- {it[0]} -> comodel '{it[1]}' (inverse '{it[2]}'): Missing field on model files: {models}")
