...
TARGET_FILES = list(ROOT.glob("qaco_planning_phase/models/planning_p*.py")) + list(
    ROOT.glob("qaco_client_onboarding/models/*.py")
)
one2many_re = re.compile(
    r"fields\.One2many\(\s*['\"]([^'\"]+)['\"]\s*,\s*['\"]([^'\"]+)['\"]", re.I
)
name_re = re.compile(r"_name\s*=\s*['\"]([^'\"]+)['\"]")
def inv_assign_re_template(inv):
    return re.compile(r"^\s*%s\s*=\s*" % re.escape(inv), re.M)
    ...
    for m in one2many_re.finditer(txt):
        ...
        for g in ROOT.rglob("*.py"):
            try:
                ...
                continue
            if name_re.search(gtxt) and any(
                name_re.findall(gtxt)[i] == comodel
                for i in range(len(name_re.findall(gtxt)))
            ):
        ...
        for g, gtxt in found_files:
            if inv_assign_re_template(inverse).search(gtxt) or re.search(
                r"%s\s*=\s*fields\.Many2one\(" % re.escape(inverse), gtxt
            ):
                ok = True
                break
        if not ok:
            issues.append(
                (
                    ...
                    comodel,
                    inverse,
                    "MISSING_INVERSE_FIELD",
                    [str(x[0]) for x in found_files],
                )
            )
...
if not issues:
    ...
    for it in issues:
        if it[3] == "MISSING_COMODEL":
            print(
                f"- {it[0]} -> comodel '{it[1]}' inverse '{it[2]}': comodel not found"
            )
        else:
            print(
                f"- {it[0]} -> comodel '{it[1]}' inverse '{it[2]}': missing inverse field in files: {', '.join(it[4])}"
            )
