...
def run_preview(db_name, config_path=None, output_path=None, print_summary=False, export_csv_dir=None):
    if config_path:
    ...
    with registry.cursor() as cr:
        ...
        for v in all_views:
            ...
            fields = [seg.strip().lstrip('+-').split()[0] for seg in expr.split(',') if seg.strip()]
            # Get field names for this view.model
            model_fields = set(Field.search([('model', '=', v.model)]).mapped('name')) if v.model else set()
        ...
        for v in all_views:
            if v.model and v.model not in model_names:
                report['views_with_missing_model'].append({'view_id': v.id, 'name': v.name, 'model': v.model, 'module': v.module})
    ...
    if print_summary:
        ...
        print(f"Models: {s['total_models']}, Fields: {s['total_fields']}, Views: {s['total_views']}")
        print(f"Bad fields: {s['bad_fields_count']}, Bad views: {s['bad_views_count']}, Views with missing model: {s['views_with_missing_model_count']}")
        if report['bad_fields']:
            print('\nSample bad fields (first 10):')
            for bf in report['bad_fields'][:10]:
                print(f"  id={bf['id']}, name={bf['name']}, model={bf['model']}, module={bf.get('module')}")
        if report['bad_views']:
            print('\nSample views with missing default_order fields (first 10):')
            for bv in report['bad_views'][:10]:
                print(f"  view id={bv['view_id']}, name={bv['name']}, model={bv['model']}, missing_fields={bv['missing_fields']}")
        if report['views_with_missing_model']:
            ...
            for vm in report['views_with_missing_model'][:10]:
                print(f"  view id={vm['view_id']}, name={vm['name']}, model={vm['model']}")
    ...
    if export_csv_dir:
        ...
        with open(bf_path, 'w', newline='', encoding='utf-8') as bf_csv:
            writer = csv.DictWriter(bf_csv, fieldnames=['id', 'name', 'model', 'module', 'model_id'])
            writer.writeheader()
            for row in report['bad_fields']:
                writer.writerow({k: row.get(k) for k in ['id', 'name', 'model', 'module', 'model_id']})
        # bad_views
        bv_path = os.path.join(export_csv_dir, 'bad_views.csv')
        with open(bv_path, 'w', newline='', encoding='utf-8') as bv_csv:
            writer = csv.DictWriter(bv_csv, fieldnames=['view_id', 'name', 'model', 'module', 'default_order', 'missing_fields'])
            writer.writeheader()
        ...
        with open(vm_path, 'w', newline='', encoding='utf-8') as vm_csv:
            writer = csv.DictWriter(vm_csv, fieldnames=['view_id', 'name', 'model', 'module'])
            writer.writeheader()
            for row in report['views_with_missing_model']:
                writer.writerow({k: row.get(k) for k in ['view_id', 'name', 'model', 'module']})
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Safe migration preview for qaco_client_onboarding (read-only)')
    ...
    parser.add_argument('--summary', action='store_true', help='Print a human-readable summary to the console in addition to the JSON report')
    parser.add_argument('--export-csv', dest='export_csv', help='Write CSV exports for affected rows into this directory (optional)')
    args = parser.parse_args()

    print('\n*** SAFE PREVIEW - NO CHANGES WILL BE MADE TO THE DATABASE ***\n')
    run_preview(args.db, args.config, args.output, print_summary=args.summary, export_csv_dir=args.export_csv)
