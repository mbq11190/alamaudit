...
# Print results
if not issues:
    print('OK: All One2many(..., "onboarding_id") references correspond to models defining onboarding_id Many2one.')
else:
    print('Found issues:')
    for it in issues:
        if it[2] == 'MISSING_MODEL':
            print(f"- {it[0]} references comodel '{it[1]}' but no model with _name='{it[1]}' was found in repo")
        else:
            files = ', '.join(str(x) for x in it[3])
            print(f"- {it[0]} references comodel '{it[1]}' but no 'onboarding_id = fields.Many2one' was found in: {files}")
