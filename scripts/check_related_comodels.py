                        ...
                        for target in node.targets:
                            if isinstance(target, ast.Name):
                                ...
                                for kw in node.value.keywords:
                                    if kw.arg in ('comodel_name','relation','comodel') and isinstance(kw.value, ast.Constant) and isinstance(kw.value.value, str):
...
for model, info in ModelInfo.items():
    ...
    for fname, meta in fields.items():
        ...
        m = re.search(r"\b"+re.escape(fname)+r"\s*=\s*fields\.[A-Za-z0-9_]+\([^\)]*\)", text, re.S)
        if not m:
            continue
        ...
        if comodel not in ModelInfo:
            problems.append((model, fname, related, fpath, f'comodel_missing:{comodel}'))
