...
class Deliverables(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_audit",
            "unique(audit_id)",
            "Deliverables already exist for this audit.",
        ),
    ]
    ...
    finalisation_phase_id = fields.Many2one(
        ...
        compute="_compute_related_phase",
        store=True,
        readonly=True,
    )

    central_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_section_statuses", store=True, tracking=True
    )
    audit_report_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_section_statuses", store=True, tracking=True
    )
    management_letter_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_section_statuses", store=True, tracking=True
    )
    tcwg_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_section_statuses", store=True, tracking=True
    )
    other_info_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_section_statuses", store=True, tracking=True
    )
    ...
    ready_for_dispatch = fields.Boolean(
        string="Ready for Dispatch", compute="_compute_ready", store=True
    )
    deliverables_locked = fields.Boolean(
        string="Deliverables Locked", tracking=True, copy=False
    )
    outstanding_items_log = fields.Html(string="Pre-Issuance Findings", copy=False)

    final_partner_id = fields.Many2one(
        "res.users",
        string="Final Sign-off Partner",
        tracking=True,
        domain=[("share", "=", False)],
    )
    final_manager_id = fields.Many2one(
        "res.users",
        string="Final Sign-off Manager",
        tracking=True,
        domain=[("share", "=", False)],
    )
    review_notes_cleared = fields.Boolean(
        string="All review notes cleared", tracking=True
    )
    final_fs_received = fields.Boolean(
        string="Final FS received & locked", tracking=True
    )
    final_fs_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_fs_rel",
        "deliverables_id",
        "attachment_id",
        string="Final FS PDFs",
    )
    ...
    independence_confirmed = fields.Boolean(
        string="Independence confirmed for team", tracking=True
    )
    independence_note = fields.Text(string="Independence confirmation notes")
    completion_checklist_signed = fields.Boolean(
        string="Completion checklist signed", tracking=True
    )
    audit_completion_scan_passed = fields.Boolean(
        string="Audit completion checklist 100% signed off", tracking=True
    )
    cold_review_completed = fields.Boolean(
        string="Cold Review Completed", tracking=True
    )
    technical_review_completed = fields.Boolean(
        string="Technical Review Completed", tracking=True
    )

    audit_report_ids = fields.One2many(
        "qaco.deliverable.audit.report", "deliverables_id", string="Auditor Reports"
    )

    deficiency_ids = fields.One2many(
        "qaco.deliverable.deficiency", "deliverables_id", string="Deficiency Log"
    )
    management_letter_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_ml_rel",
        "deliverables_id",
        "attachment_id",
        string="Final Management Letter",
    )
    management_response_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_ml_resp_rel",
        "deliverables_id",
        "attachment_id",
        string="Management Response Form",
    )
    management_letter_sent_date = fields.Date(string="Management Letter Sent Date")
    management_response_received_date = fields.Date(
        string="Management Response Received Date"
    )

    tcwg_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_tcwgl_rel",
        "deliverables_id",
        "attachment_id",
        string="TCWG Deck / Letter",
    )
    tcwg_ack_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_tcwga_rel",
        "deliverables_id",
        "attachment_id",
        string="Acknowledged Minutes",
    )
    ...
    tcwg_topic_responsibilities = fields.Boolean(
        string="Auditor responsibility explained", tracking=True
    )
    tcwg_topic_significant_findings = fields.Boolean(
        string="Significant findings communicated", tracking=True
    )
    tcwg_topic_uncorrected = fields.Boolean(
        string="Uncorrected misstatements shared", tracking=True
    )
    tcwg_topic_scope = fields.Boolean(string="Scope & timing discussed", tracking=True)
    tcwg_topic_independence = fields.Boolean(
        string="Independence confirmation given", tracking=True
    )
    tcwg_topic_qualitative = fields.Boolean(
        string="Qualitative aspects discussed", tracking=True
    )
    tcwg_topic_going_concern = fields.Boolean(
        string="Going concern assessment discussed", tracking=True
    )
    tcwg_topic_related_parties = fields.Boolean(
        string="Related party transactions discussed", tracking=True
    )
    ...
    other_info_line_ids = fields.One2many(
        "qaco.deliverable.other.info",
        "deliverables_id",
        string="Other Information Register",
    )
    other_info_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_other_rel",
        "deliverables_id",
        "attachment_id",
        string="Other Information Evidence",
    )
    other_info_review_completed = fields.Boolean(
        string="Other Information Review Completed", tracking=True
    )
    other_info_misstatement_count = fields.Integer(
        string="Material Misstatements Found",
        compute="_compute_misstatement_count",
        store=True,
    )

    dispatch_log_ids = fields.One2many(
        "qaco.deliverable.dispatch", "deliverables_id", string="Dispatch Log"
    )
    deliverable_pack_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_pack_rel",
        "deliverables_id",
        "attachment_id",
        string="Final Deliverables Pack",
    )
    archive_confirmed = fields.Boolean(
        string="Archive confirmation logged", tracking=True
    )
    archive_date = fields.Datetime(string="Archive Date")
    archive_notes = fields.Text(string="Archive Notes")

    fs_consistency_check = fields.Boolean(
        string="FS Consistency Check Passed", tracking=True
    )
    regulatory_filing_required = fields.Boolean(
        string="Regulatory Filing Required", tracking=True
    )
    regulatory_filing_completed = fields.Boolean(
        string="Regulatory Filing Completed", tracking=True
    )
    zakat_deduction_verified = fields.Boolean(
        string="Zakat Deduction Verified", tracking=True
    )
    ...
    def _compute_related_phase(self):
        ...
        for record in self:
            finalisation = final_model.search(
    ...
    def _compute_misstatement_count(self):
        for record in self:
            record.other_info_misstatement_count = len(
                record.other_info_line_ids.filtered(
                    lambda line: line.status == "misstatement"
                )
            )
    ...
    @api.depends(
        ...
        "final_fs_received",
        "final_fs_attachment_ids",
        "eqcr_required",
        "eqcr_completed",
        "eqcr_reviewer_id",
        "independence_confirmed",
        "completion_checklist_signed",
        "audit_completion_scan_passed",
        "cold_review_completed",
        "technical_review_completed",
        "final_partner_id",
        "final_manager_id",
        "audit_report_ids",
        "audit_report_ids.readiness_flag",
        "deficiency_ids",
        "management_letter_attachment_ids",
        "management_response_attachment_ids",
        "management_letter_sent_date",
        "management_response_received_date",
        "tcwg_topic_responsibilities",
        "tcwg_topic_significant_findings",
        "tcwg_topic_uncorrected",
        "tcwg_topic_scope",
        "tcwg_topic_independence",
        "tcwg_topic_qualitative",
        "tcwg_topic_going_concern",
        "tcwg_topic_related_parties",
        "tcwg_attachment_ids",
        "tcwg_ack_attachment_ids",
        "tcwg_meeting_date",
        "other_info_line_ids.status",
        "other_info_line_ids.escalated",
        "other_info_review_completed",
        "dispatch_log_ids.status",
        "deliverable_pack_attachment_ids",
        "archive_confirmed",
        "fs_consistency_check",
        "regulatory_filing_required",
        "regulatory_filing_completed",
    )
        ...
        for record in self:
            ...
            central_complete = all(
                [
                    ...
                    (not record.eqcr_required)
                    or (record.eqcr_completed and record.eqcr_reviewer_id),
                ]
            )
            record.central_status = record._determine_status(
                central_complete, central_started
            )

            audit_started = bool(record.audit_report_ids)
            audit_complete = bool(record.audit_report_ids.filtered("readiness_flag"))
            record.audit_report_status = record._determine_status(
                audit_complete, audit_started
            )

            mgmt_started = bool(record.deficiency_ids) or bool(
                record.management_letter_attachment_ids
            )
            mgmt_complete = all(
                [
                    bool(record.deficiency_ids),
                    all(
                        line.recommendation and line.action_owner
                        for line in record.deficiency_ids
                    ),
                    bool(record.management_letter_attachment_ids),
                    record.management_letter_sent_date,
                ]
            )
            record.management_letter_status = record._determine_status(
                mgmt_complete, mgmt_started
            )
            ...
            other_started = (
                bool(record.other_info_line_ids) or record.other_info_review_completed
            )
            other_complete = all(
                [
                    bool(record.other_info_line_ids),
                    all(
                        line.status in ("cleared", "misstatement")
                        for line in record.other_info_line_ids
                    ),
                    not record.other_info_line_ids.filtered(
                        lambda line: line.status == "misstatement"
                        and not line.escalated
                    ),
                    record.other_info_review_completed,
                ]
            )
            record.other_info_status = record._determine_status(
                other_complete, other_started
            )
            ...
            dispatch_complete = all(
                [
                    bool(
                        record.dispatch_log_ids.filtered(
                            lambda line: line.status in ("dispatched", "acknowledged")
                        )
                    ),
                    bool(record.deliverable_pack_attachment_ids),
                    record.archive_confirmed,
                    record.fs_consistency_check,
                    (not record.regulatory_filing_required)
                    or record.regulatory_filing_completed,
                ]
            )
            record.dispatch_status = record._determine_status(
                dispatch_complete, dispatch_started
            )

    @api.depends(
        "central_status",
        "audit_report_status",
        "management_letter_status",
        "tcwg_status",
        "other_info_status",
        "dispatch_status",
    )
    ...
    def _collect_gateway_findings(self) -> List[str]:
        ...
        if self.eqcr_required and (
            not self.eqcr_completed or not self.eqcr_reviewer_id
        ):
            findings.append(_("EQCR sign-off pending."))
        if (
            self.finalisation_phase_id
            and self.finalisation_phase_id.completion_status != "green"
        ):
        ...
        if self.deficiency_ids.filtered(
            lambda line: not line.recommendation or not line.action_owner
        ):
            findings.append(_("Deficiencies missing recommendations/action owners."))
        if (
            not self.management_letter_attachment_ids
            or not self.management_letter_sent_date
        ):
        ...
        if self.other_info_line_ids.filtered(
            lambda line: line.status not in ("cleared", "misstatement")
        ):
            findings.append(_("Other information register has pending rows."))
        if self.other_info_line_ids.filtered(
            lambda line: line.status == "misstatement" and not line.escalated
        ):
    ...
    def action_run_gateway_check(self):
        for record in self:
            ...
            record.outstanding_items_log = _(
                '<p style="color:green;">Gateway cleared.</p>'
            )
            record.message_post(
                body=_("Gateway checklist cleared. All prerequisites satisfied.")
            )
    ...
    def action_validate_deliverables(self):
        for record in self:
            ...
            if blocking:
                raise UserError("\n".join(blocking))
            record.message_post(
                body=_("All deliverables validated. Ready for dispatch.")
            )
    ...
    def action_mark_archive_confirmed(self):
        for record in self:
            ...
            if not record.deliverable_pack_attachment_ids:
                raise UserError(
                    _("Deliverables pack attachment is required to archive.")
                )
    ...
    def action_generate_deliverables_pack(self):
        ...
        if not self.deliverable_pack_attachment_ids:
            raise UserError(
                _(
                    "Attach the deliverables pack (signed report, management letter, TCWG minutes) before dispatch."
                )
            )
...
class DeliverableAuditReport(models.Model):
    ...
    deliverables_id = fields.Many2one(
        "qaco.deliverables", string="Deliverables", required=True, ondelete="cascade"
    )
    client_id = fields.Many2one(
        related="deliverables_id.client_id", store=False, readonly=True
    )
    ...
    opinion_type = fields.Selection(
        OPINION_TYPES, string="Opinion Type", required=True, default="unmodified"
    )
    entity_type = fields.Selection(
        ENTITY_TYPES, string="Entity Type", required=True, default="non_listed"
    )
    report_template_type = fields.Selection(
        REPORT_TEMPLATE_TYPES,
        string="Template Type",
        required=True,
        default="isa_700_non_listed",
    )
    ...
    regulatory_submission_required = fields.Boolean(
        string="Regulatory Submission Required"
    )
    ...
    signing_partner_id = fields.Many2one(
        "res.users", string="Signing Partner", domain=[("share", "=", False)]
    )
    signing_manager_id = fields.Many2one(
        "res.users", string="Signing Director/Manager", domain=[("share", "=", False)]
    )
    ...
    basis_template = fields.Html(
        string="Basis Template", compute="_compute_basis_template", sanitize=False
    )
    template_recommendation = fields.Char(
        string="Suggested Template", compute="_compute_template", store=True
    )
    draft_report_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_report_draft_rel",
        "report_id",
        "attachment_id",
        string="Draft Reports w/Comments",
    )
    final_report_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_report_signed_rel",
        "report_id",
        "attachment_id",
        string="Final Signed Reports",
    )
    support_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_report_support_rel",
        "report_id",
        "attachment_id",
        string="Supporting Memos",
    )
    kam_line_ids = fields.One2many(
        "qaco.deliverable.kam", "audit_report_id", string="KAMs"
    )
    eom_line_ids = fields.One2many(
        "qaco.deliverable.eom", "audit_report_id", string="EOM/OM"
    )
    readiness_flag = fields.Boolean(
        string="Ready for Dispatch", compute="_compute_readiness", store=True
    )
    ...
    def _compute_template(self):
        ...
        for record in self:
            suggested_key = (
                mapping.get((record.entity_type, record.framework))
                or record.report_template_type
                or ""
            )
            fallback_key = record.report_template_type or ""
            template_value = template_dict.get(suggested_key) or template_dict.get(
                fallback_key, ""
            )
    ...
    def _compute_basis_template(self):
        template_map = {
            "unmodified": _(
                "<p>We conducted our audit in accordance with International Standards on Auditing...</p>"
            ),
        ...
        for record in self:
            if record.kam_required and not record.kam_line_ids:
                raise ValidationError(
                    _(
                        "At least one KAM entry is required when KAMs are flagged as required."
                    )
                )
    ...
    def _check_eom_requirements(self):
        for record in self:
            if record.eom_required and not record.eom_line_ids:
                raise ValidationError(
                    _("Provide at least one EOM/OM paragraph when marked as required.")
                )
    ...
    def action_mark_ready(self):
        for record in self:
            if not record.final_report_attachment_ids:
                raise ValidationError(
                    _("Attach the signed report before marking it ready.")
                )
    ...
    def _check_lengths(self):
        for record in self:
            if len((record.description or "").strip()) < 30:
                raise ValidationError(
                    _("Provide sufficient detail in the KAM description.")
                )
            if len((record.audit_response or "").strip()) < 30:
                raise ValidationError(
                    _("Provide sufficient detail for the audit response.")
                )
...
class DeliverableDeficiency(models.Model):
    ...
    recommendation = fields.Text(
        string="Recommendation / Remediation Plan", required=True
    )
    ...
    def create(self, vals):
        if not vals.get("reference") and vals.get("deliverables_id"):
            ...
            vals["reference"] = "%s-DEF-%02d" % (
                deliverable.audit_id.name or "AUD",
                count,
            )
...
class DeliverableOtherInformation(models.Model):
    ...
    status = fields.Selection(
        OTHER_INFO_STATUS, default="not_received", string="Status", tracking=True
    )
    ...
    misstatement_communicated = fields.Boolean(
        string="Misstatement Communicated to Management"
    )
    resolution_reference = fields.Char(string="Resolution Reference / Minutes")
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_otherinfo_rel",
        "otherinfo_id",
        "attachment_id",
        string="Evidence",
    )
...
class DeliverableDispatch(models.Model):
    ...
    dispatch_date = fields.Datetime(
        string="Dispatch Date/Time", required=True, default=fields.Datetime.now
    )
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_dispatch_rel",
        "dispatch_id",
        "attachment_id",
        string="Proof of Dispatch",
    )
    acknowledgment_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_deliverables_dispatch_ack_rel",
        "dispatch_id",
        "attachment_id",
        string="Acknowledgment Proof",
    )
    ...
    def _check_acknowledgment(self):
        for record in self:
            if record.status == "acknowledged" and not record.acknowledged_date:
                raise ValidationError(
                    _(
                        "Capture the acknowledgment date/time when status is acknowledged."
                    )
                )
