...
def migrate(cr, version):
    ...
    try:
        ...
        if not cr.fetchone():
            _logger.info("No firm_name_old column found - migration not needed or already completed")
            return
        ...
        for old_value, xml_id in firm_mapping.items():
            ...
            if not firm_id_result:
                _logger.warning("Could not find firm record for '%s' (xml_id: %s) - skipping", 
                              old_value, xml_id)
                continue
