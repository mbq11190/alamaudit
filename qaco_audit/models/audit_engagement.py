...
class QacoBranchOffice(models.Model):
    ...
    engagement_id = fields.Many2one(
        "qaco.audit.engagement", string="Engagement", ondelete="cascade"
    )
...
class QacoAuditChecklist(models.Model):
    ...
    engagement_id = fields.Many2one(
        "qaco.audit.engagement", string="Engagement", required=True, ondelete="cascade"
    )
...
class QacoAuditEngagement(models.Model):
    ...
    branch_office_ids = fields.One2many(
        "qaco.branch.office", "engagement_id", string="Branch Offices"
    )
    ubo_ids = fields.One2many(
        "qaco.ubo", "engagement_id", string="Ultimate Beneficial Owners"
    )
    ...
    regulatory_inspection_ids = fields.One2many(
        "qaco.regulatory.inspection", "engagement_id", string="Regulatory Inspections"
    )
    ...
    prohibited_services_evaluated = fields.Boolean(
        string="Prohibited Services Evaluated"
    )
    fee_dependency_percentage = fields.Float(string="Fee Dependency (%)")
    fee_dependency_acceptable = fields.Boolean(
        string="Fee Dependency Acceptable",
        compute="_compute_fee_dependency_acceptable",
        store=True,
    )
    safeguards_documentation_required = fields.Boolean(
        string="Safeguard Documentation Required",
        compute="_compute_safeguards_required",
        store=True,
    )
    partner_review_required = fields.Boolean(
        string="Partner Review Required",
        compute="_compute_partner_review_required",
        store=True,
    )
    ...
    material_misstatements_previous = fields.Text(
        string="Material Misstatements from Previous Auditor"
    )

    preconditions_met = fields.Boolean(string="Pre-conditions Met")
    management_responsibilities_acknowledged = fields.Boolean(
    ...
    doc_certificate_incorporation = fields.Boolean(
    ...
    checklist_ids = fields.One2many(
        "qaco.audit.checklist", "engagement_id", string="Checklists"
    )
    acceptance_checklist_complete = fields.Boolean(
        string="Acceptance Checklist Complete",
        compute="_compute_checklist_completion",
        store=True,
    )
    independence_checklist_complete = fields.Boolean(
        string="Independence Checklist Complete",
        compute="_compute_checklist_completion",
        store=True,
    )
    isqm_checklist_complete = fields.Boolean(
        string="ISQM Checklist Complete",
        compute="_compute_checklist_completion",
        store=True,
    )
    ...
    def _compute_fee_dependency_acceptable(self):
        for record in self:
            record.fee_dependency_acceptable = bool(
                record.fee_dependency_percentage <= 15
            )
    ...
    def _compute_eqcr_required(self):
        for record in self:
            record.eqcr_required = (
                record.aml_ctf_risk == "high"
                or record.entity_classification in ("pic", "lsc")
            )
    ...
    @api.depends(
        ...
        "fraud_knowledge_previous",
        "material_misstatements_previous",
        "outstanding_fees",
        "aml_ctf_risk",
    )
    def _compute_partner_review_required(self):
        for record in self:
            ...
            independence_issue = (
                record.independence_threats and record.independence_threats != "none"
            )
            record.partner_review_required = bool(
                issues
                or independence_issue
                or record.aml_ctf_risk == "high"
    ...
    def _compute_safeguards_required(self):
        for record in self:
            record.safeguards_documentation_required = bool(
                record.independence_threats and record.independence_threats != "none"
            )
    ...
    def _compute_checklist_completion(self):
        for record in self:
            ...
            record.acceptance_checklist_complete = (
                all(item.status == "done" for item in types["acceptance"])
                if types["acceptance"]
                else False
            )
            record.independence_checklist_complete = (
                all(item.status == "done" for item in types["independence"])
                if types["independence"]
                else False
            )
            record.isqm_checklist_complete = (
                all(item.status == "done" for item in types["isqm"])
                if types["isqm"]
                else False
            )
    ...
    def action_accept_engagement(self):
        for record in self:
            if (
                not record.preconditions_met
                or not record.management_responsibilities_acknowledged
            ):
                raise ValidationError(
                    _(
                        "Preconditions and management responsibilities must be acknowledged before accepting."
                    )
                )
            if record.partner_review_required and not record.partner_review_completed:
                raise ValidationError(
                    _("Partner review is required before acceptance.")
                )
        ...
        for record in self:
            if record.partner_review_required and not record.partner_review_completed:
                record.activity_schedule(
                    ...
                    note=_("Engagement partner review required."),
                )
