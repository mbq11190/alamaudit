...
class auditAttachment(models.Model):
    ...
    file_type = fields.Selection([('pdf','PDF'),('docx','DOCX'),('xlsx','XLSX'),('img','Image'),('eml','Email')], string='File Type')
    ...
    confidentiality = fields.Selection([('public','Public'),('restricted','Restricted')], string='Confidentiality', default='public')
    ...
    def create(self, vals):
        ...
        if not vals.get('document_index'):
            vals['document_index'] = self.env['ir.sequence'].next_by_code('audit.document.index') or 'DOC-NEW'
        if not vals.get('evidence_ref'):
            vals['evidence_ref'] = self.env['ir.sequence'].next_by_code('audit.evidence.ref') or 'EVID-NEW'
        rec = super().create(vals)
        # Log attachment creation on the audit record
        if rec.audit_id:
            rec.audit_id.message_post(body=f'Attachment {rec.name} uploaded ({rec.document_index}) by {rec.create_uid.name}')
        return rec
