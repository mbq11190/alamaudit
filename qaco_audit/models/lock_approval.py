...
class AuditLockApproval(models.Model):
    ...
    audit_id = fields.Many2one('qaco.audit', string='Engagement', required=True, ondelete='cascade', index=True)
    requestor_id = fields.Many2one('res.users', string='Requested by', default=lambda self: self.env.user, readonly=True)
    ...
    status = fields.Selection([('pending', 'Pending'), ('approved', 'Approved'), ('rejected', 'Rejected')], string='Status', default='pending', tracking=True)
    ...
    def action_approve(self, note=None):
        ...
        for rec in self:
            if not self.env.user.has_group('qaco_audit.group_audit_manager'):
                raise exceptions.AccessError(_('Only managers can approve lock/unlock requests.'))
            ...
            if rec.audit_id and rec.audit_id.engagement_status == 'locked' and not rec.applied:
                rec.audit_id.action_unlock_engagement(reason=f"Override approved: {rec.reason}")
                rec.applied = True

    def action_reject(self, note=None):
        for rec in self:
            if not self.env.user.has_group('qaco_audit.group_audit_manager'):
                raise exceptions.AccessError(_('Only managers can reject lock/unlock requests.'))
            rec.write({
                'status': 'rejected',
                'approver_id': self.env.user.id,
                'approved_on': fields.Datetime.now(),
