...
class TestLockApproval(TransactionCase):
    ...
    def test_request_and_approve_unlock(self):
        audit = self.Audit.create({'client_id': self.env.ref('base.res_partner_1').id, 'engagement_status': 'locked'})
        # request as normal user
        approval = self.Lock.create({'audit_id': audit.id, 'reason': 'Emergency unlock required for file error'})
        self.assertEqual(approval.status, 'pending')
        # approve as manager user
        manager_user = self.env['res.users'].search([('groups_id', 'in', self.manager.id)], limit=1)
