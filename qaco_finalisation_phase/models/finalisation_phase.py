...
class FinalisationPhase(models.Model):
    ...
    audit_id = fields.Many2one(
        "qaco.audit", string="Audit Engagement", required=True, ondelete="cascade"
    )
    ...
    planning_phase_id = fields.Many2one(
        ...
        compute="_compute_related_phases",
        store=True,
        readonly=True,
    )
    ...
    misstatement_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    subsequent_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    going_concern_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    related_party_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    representation_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    analytics_status = fields.Selection(
        SUBTAB_STATUS, compute="_compute_submodule_statuses", store=True, tracking=True
    )
    ...
    signoff_unlocked = fields.Boolean(
        string="Sign-off Submodule Available",
        compute="_compute_signoff_unlock",
        store=True,
    )
    compliance_scan_passed = fields.Boolean(
        string="Compliance Scan Passed", tracking=True, copy=False
    )
    ...
    regulatory_news = fields.Html(
        string="Regulatory Update Feed",
        compute="_compute_regulatory_feed",
        sanitize=False,
        copy=False,
    )

    overall_materiality_ref = fields.Monetary(
        string="Overall Materiality Ref",
        currency_field="company_currency_id",
        compute="_compute_materiality_refs",
        store=True,
        readonly=True,
    )
    performance_materiality_ref = fields.Monetary(
        string="Performance Materiality Ref",
        currency_field="company_currency_id",
        compute="_compute_materiality_refs",
        store=True,
        readonly=True,
    ...
    misstatement_line_ids = fields.One2many(
        "qaco.final.misstatement", "finalisation_phase_id", string="Misstatements"
    )
    misstatement_summary_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_misstatement_summary_rel",
        "phase_id",
        "attachment_id",
        string="Summary of Uncorrected Misstatements",
        help="Upload the final ISA 450 template.",
    )
    misstatement_support_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_misstatement_support_rel",
        "phase_id",
        "attachment_id",
    ...
    misstatement_correspondence_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_misstatement_correspondence_rel",
        "phase_id",
        "attachment_id",
        string="Management Correspondence",
    )
    uncorrected_total = fields.Monetary(
        string="Uncorrected Misstatements",
        currency_field="company_currency_id",
        compute="_compute_misstatement_totals",
        store=True,
    )
    corrected_total = fields.Monetary(
        string="Corrected Misstatements",
        currency_field="company_currency_id",
        compute="_compute_misstatement_totals",
        store=True,
    )
    misstatement_threshold_percent = fields.Float(
        string="Threshold % of Materiality", default=70.0
    )
    threshold_breached = fields.Boolean(
        string="Threshold Breached", compute="_compute_threshold_breach", store=True
    )
    ...
    checklist_misstatement_evaluated = fields.Boolean(
        string="All misstatements evaluated", tracking=True
    )
    checklist_cumulative_below_materiality = fields.Boolean(
        string="Cumulative impact below materiality", tracking=True
    )
    checklist_qualitative_considered = fields.Boolean(
        string="Qualitative impact considered", tracking=True
    )
    checklist_disclosures_reassessed = fields.Boolean(
        string="Disclosures reassessed", tracking=True
    )

    subsequent_procedure_ids = fields.One2many(
        "qaco.final.subsequent.procedure",
        "finalisation_phase_id",
        string="Subsequent Event Procedures",
    )
    subsequent_event_ids = fields.One2many(
        "qaco.final.subsequent.event",
        "finalisation_phase_id",
        string="Identified Events",
    )
    subsequent_minutes_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_subsequent_minutes_rel",
        "phase_id",
        "attachment_id",
    ...
    subsequent_legal_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_subsequent_legal_rel",
        "phase_id",
        "attachment_id",
    ...
    subsequent_checklist_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_subsequent_checklist_rel",
        "phase_id",
        "attachment_id",
        string="Subsequent Events Checklist",
    )
    checklist_sub_events_period_covered = fields.Boolean(
        string="Procedures cover up to report date", tracking=True
    )
    checklist_sub_events_adjusting = fields.Boolean(
        string="Adjusting events recorded", tracking=True
    )
    checklist_sub_events_disclosure = fields.Boolean(
        string="Non-adjusting events disclosed", tracking=True
    )

    going_concern_scenario_ids = fields.One2many(
        "qaco.final.gc.scenario", "finalisation_phase_id", string="Scenario Analysis"
    )
    going_concern_covenant_ids = fields.One2many(
        "qaco.final.gc.covenant", "finalisation_phase_id", string="Covenant Calculator"
    )
    going_concern_conclusion = fields.Selection(
        GOING_CONCERN_CONCLUSION,
        string="Final Going Concern Conclusion",
        required=True,
        default="no_uncertainty",
    )
    emphasis_matter_required = fields.Boolean(
        string="Emphasis of Matter Needed",
        compute="_compute_emphasis_of_matter",
        store=True,
    )
    going_concern_assessment_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_gc_assessment_rel",
        "phase_id",
        "attachment_id",
        string="Management Assessment",
    )
    going_concern_forecast_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_gc_forecast_rel",
        "phase_id",
        "attachment_id",
        string="Forecasts (12 months)",
    )
    going_concern_covenant_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_gc_covenant_rel",
        "phase_id",
        "attachment_id",
        string="Covenant Certificates",
    )
    going_concern_memo_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_gc_memo_rel",
        "phase_id",
        "attachment_id",
        string="Going Concern Memo",
    )
    checklist_gc_assessment_challenged = fields.Boolean(
        string="Management assessment challenged", tracking=True
    )
    checklist_gc_future_info_considered = fields.Boolean(
        string="Future info considered", tracking=True
    )
    checklist_gc_basis_appropriate = fields.Boolean(
        string="Basis of prep appropriate", tracking=True
    )
    checklist_gc_reporting_impact = fields.Boolean(
        string="Reporting impact evaluated", tracking=True
    )

    related_party_line_ids = fields.One2many(
        "qaco.final.related.party",
        "finalisation_phase_id",
        string="Related Party Register",
    )
    non_compliance_log_ids = fields.One2many(
        "qaco.final.noncompliance", "finalisation_phase_id", string="Non-Compliance Log"
    )
    related_party_register_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_related_register_rel",
        "phase_id",
        "attachment_id",
        string="Final RPT Register",
    )
    related_party_minutes_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_related_minutes_rel",
        "phase_id",
        "attachment_id",
        string="Board Approvals",
    )
    noncompliance_summary_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_noncompliance_summary_rel",
        "phase_id",
        "attachment_id",
        string="Non-compliance Summary",
    )
    checklist_related_complete = fields.Boolean(
        string="Register complete & accurate", tracking=True
    )
    checklist_related_authorised = fields.Boolean(
        string="Transactions authorised", tracking=True
    )
    checklist_related_disclosures_ok = fields.Boolean(
        string="Disclosures comply with IFRS", tracking=True
    )
    checklist_noncompliance_communicated = fields.Boolean(
        string="Non-compliance communicated to TCWG", tracking=True
    )

    representation_letter_template = fields.Html(
        string="Representation Letter Template",
        compute="_compute_representation_template",
        sanitize=False,
        store=True,
    )
    representation_letter_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_rep_letter_rel",
        "phase_id",
        "attachment_id",
        string="Signed Representation Letter",
    )
    representation_letter_signed_on = fields.Date(string="Representation Letter Dated")
    checklist_rep_obtained = fields.Boolean(
        string="Signed letter obtained", tracking=True
    )
    checklist_rep_content_complete = fields.Boolean(
        string="Letter covers all representations", tracking=True
    )
    checklist_rep_date_valid = fields.Boolean(
        string="Letter appropriately dated", tracking=True
    )

    analytics_ratio_ids = fields.One2many(
        "qaco.final.analytics.ratio", "finalisation_phase_id", string="Analytics"
    )
    analytics_variance_ids = fields.One2many(
        "qaco.final.analytics.variance",
        "finalisation_phase_id",
        string="Variance Flags",
    )
    analytics_workbook_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_analytics_workbook_rel",
        "phase_id",
        "attachment_id",
        string="Analytical Review Workbook",
    )
    analytics_conclusion_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_analytics_conclusion_rel",
        "phase_id",
        "attachment_id",
        string="Overall Conclusion Memo",
    )
    checklist_analytics_corrob = fields.Boolean(
        string="Analytics corroborate conclusions", tracking=True
    )
    checklist_variances_resolved = fields.Boolean(
        string="Variance explanations documented", tracking=True
    )
    checklist_fs_compliant = fields.Boolean(
        string="FS form/content compliant", tracking=True
    )

    review_note_ids = fields.One2many(
        "qaco.final.review.note", "finalisation_phase_id", string="Review Notes"
    )
    regulatory_alert_ids = fields.One2many(
        "qaco.final.regulatory.alert",
        "finalisation_phase_id",
        string="Regulatory Alerts",
    )
    checklist_review_notes_cleared = fields.Boolean(
        string="Review notes cleared", tracking=True
    )
    checklist_compliance_scan = fields.Boolean(
        string="Compliance scan passed", tracking=True
    )
    checklist_partner_approval = fields.Boolean(
        string="Partner final approval obtained", tracking=True
    )
    checklist_archive_ready = fields.Boolean(
        string="File ready for archiving", tracking=True
    )
    completion_certificate_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_completion_certificate_rel",
        "phase_id",
        "attachment_id",
        string="File Completion Checklist",
    )

    audit_report_date = fields.Date(
        string="Audit Report Date",
        required=True,
    ...
    final_locked_on = fields.Datetime(
        string="File Locked On", tracking=True, copy=False, readonly=True
    )
    ...
    def _compute_name(self):
        for record in self:
            if record.client_id:
                record.name = (
                    f"FINAL-{record.client_id.name}"
                    if record.audit_id
                    else f"Finalisation - {record.client_id.name}"
                )
            else:
    ...
    def _compute_related_phases(self):
        ...
        for record in self:
            ...
            if record.audit_id:
                ...
                execution = execution_model.search(
                    [("audit_id", "=", record.audit_id.id)], limit=1
                )
    ...
    @api.depends(
        "planning_phase_id.overall_materiality",
        "planning_phase_id.performance_materiality",
    )
    def _compute_materiality_refs(self):
        for record in self:
            record.overall_materiality_ref = (
                record.planning_phase_id.overall_materiality
                if record.planning_phase_id
                else 0.0
            )
            record.performance_materiality_ref = (
                record.planning_phase_id.performance_materiality
                if record.planning_phase_id
                else 0.0
            )
    ...
    def _build_dashboard_summary(self) -> str:
        ...
        def human(code):
            return status_labels.get(code, _("n/a"))

        items = "".join(
            f"<li><b>{label}:</b> {human(state)}</li>" for label, state in status_rows
        )
        regulatory_snippet = self.regulatory_news or _(
            "<p>No regulator updates recorded.</p>"
        )
        return "".join(
            [
    ...
    def _compute_submodule_statuses(self):
        for record in self:
            misstatement_started = bool(
                record.misstatement_line_ids
                or record.misstatement_summary_attachment_ids
                or record.misstatement_support_attachment_ids
            )
            misstatement_complete = (
                bool(record.misstatement_summary_attachment_ids)
                and bool(record.misstatement_support_attachment_ids)
                and all(
                    [
            ...
            mis_status = record._determine_status(
                misstatement_complete, misstatement_started
            )

            events_all_resolved = (
                all(record.subsequent_event_ids.mapped("resolved"))
                if record.subsequent_event_ids
                else True
            )
            subseq_started = bool(
                record.subsequent_procedure_ids
                or record.subsequent_event_ids
                or record.subsequent_checklist_attachment_ids
            )
            subseq_complete = (
                bool(record.subsequent_checklist_attachment_ids)
                and events_all_resolved
                and all(
                    [
            ...
            gc_started = bool(
                record.going_concern_scenario_ids
                or record.going_concern_covenant_ids
                or record.going_concern_assessment_attachment_ids
                or record.going_concern_memo_attachment_ids
            )
            gc_complete = (
                bool(record.going_concern_assessment_attachment_ids)
                and bool(record.going_concern_memo_attachment_ids)
                and all(
                    [
            ...
            related_started = bool(
                record.related_party_line_ids
                or record.non_compliance_log_ids
                or record.related_party_register_attachment_ids
            )
            related_complete = bool(
                record.related_party_register_attachment_ids
            ) and all(
                [
            ...
            rep_started = bool(
                record.representation_letter_attachment_ids
                or record.representation_letter_signed_on
            )
            ...
            analytics_started = bool(
                record.analytics_ratio_ids
                or record.analytics_variance_ids
                or record.analytics_workbook_attachment_ids
            )
            analytics_complete = (
                bool(record.analytics_workbook_attachment_ids)
                and bool(record.analytics_conclusion_attachment_ids)
                and all(
                    [
            ...
            analytics_status = record._determine_status(
                analytics_complete, analytics_started
            )

            pending_review_notes = record.review_note_ids.filtered(
                lambda n: n.state != "resolved"
            )
            ...
            completion_status = record._determine_status(
                completion_complete, completion_started
            )
    ...
    def _compute_misstatement_totals(self):
        for record in self:
            corrected = sum(
                line.amount
                for line in record.misstatement_line_ids.filtered(
                    lambda line: line.status == "corrected"
                )
            )
            uncorrected = sum(
                line.amount
                for line in record.misstatement_line_ids.filtered(
                    lambda line: line.status == "uncorrected"
                )
            )
    ...
    @api.depends(
        "uncorrected_total", "overall_materiality_ref", "misstatement_threshold_percent"
    )
    def _compute_threshold_breach(self):
        for record in self:
            threshold = (record.overall_materiality_ref or 0.0) * (
                record.misstatement_threshold_percent / 100.0
            )
            record.threshold_breached = bool(
                threshold and record.uncorrected_total > threshold
            )
    ...
    def _compute_regulatory_feed(self):
        ...
        for record in self:
            alerts = record.regulatory_alert_ids.sorted(
                lambda a: a.create_date or fields.Datetime.now(), reverse=True
            )[:5]
            if not alerts:
            ...
            for alert in alerts:
                ...
                date_label = alert.effective_date or (
                    alert.create_date.date() if alert.create_date else False
                )
                ...
                link = (
                    f" <a href='{alert.link_url}' target='_blank'>{_('read')}</a>"
                    if alert.link_url
                    else ""
                )
                rows.append(
    ...
    def _compute_representation_template(self):
        for record in self:
            ...
            conclusion_map = {
                "no_uncertainty": _(
                    "We confirm there are no events or conditions that may cast significant doubt on the entity's ability to continue as a going concern."
                ),
                "material_uncertainty": _(
                    "We have disclosed all material uncertainties related to events or conditions that may cast significant doubt on the entity's ability to continue as a going concern."
                ),
                "not_going_concern": _(
                    "We have prepared the financial statements on an alternative basis as the entity is not a going concern."
                ),
            }
            going_concern_clause = conclusion_map.get(
                record.going_concern_conclusion, conclusion_map["no_uncertainty"]
            )

            alert_highlights = record.regulatory_alert_ids.sorted(
                lambda a: a.create_date or fields.Datetime.now(), reverse=True
            )[:3]
            regulator_section = ""
            if alert_highlights:
                bullet_items = "".join(
                    f"<li>{alert.reference_code or _('Update')}: {alert.summary}</li>"
                    for alert in alert_highlights
                )
                regulator_section = (
                    _(
                        "<p>We confirm consideration of the following regulator communications:</p><ul>%s</ul>"
                    )
                    % bullet_items
                )
    ...
    def _compute_emphasis_of_matter(self):
        for record in self:
            record.emphasis_matter_required = record.going_concern_conclusion in (
    ...
    def _ensure_threshold_controls(self):
        for record in self:
            if record.threshold_breached:
                ...
                if missing:
                    raise UserError(
                        _("Threshold exceeded. Provide: %s") % ", ".join(missing)
                    )
    ...
    def action_run_compliance_scan(self):
        for record in self:
            ...
            record.compliance_scan_log = _(
                '<p style="color:green;">All compliance gates passed.</p>'
            )
    ...
    def action_sync_misstatements(self):
        for record in self:
            ...
            procedures = record.execution_phase_id.substantive_area_ids.mapped(
                "procedure_ids"
            )
            existing_lines = {
                (line.source_reference, line.source_model): line
                for line in record.misstatement_line_ids
                if line.source_reference and line.source_model
            }
            created = 0
            updated = 0
            for procedure in procedures.filtered(lambda p: p.result == "exception"):
                ...
                rationale = procedure.exception_note or _(
                    "Exception noted in substantive procedure."
                )
            ...
            if created or updated:
                ...
                record.message_post(
                    body=_("Misstatement register refreshed (%s).") % ", ".join(parts)
                )
            else:
                record.message_post(
                    body=_("No new execution exceptions found to sync.")
                )
    ...
    def action_manager_sign(self):
        ...
        self.message_post(
            body=_("Manager sign-off recorded by %s.") % self.env.user.name
        )
    ...
    def action_lock_final_file(self):
        ...
        self.archive_deadline_date = (
            self.audit_report_date + timedelta(days=60)
            if self.audit_report_date
            else False
        )
        self.checklist_archive_ready = True
        self.message_post(
            body=_("File locked and archive timer started. Deadline: %s")
            % (self.archive_deadline_date or _("N/A"))
        )
    ...
    def action_trigger_archive_alert(self):
        ...
        if days_left < 0:
            raise UserError(
                _("Archive deadline already passed. Escalate to administrator.")
            )
...
class FinalMisstatement(models.Model):
    ...
    finalisation_phase_id = fields.Many2one(
    ...
    misstatement_type = fields.Selection(
        MISSTATEMENT_TYPES, string="Type", required=True
    )
    status = fields.Selection(
        MISSTATEMENT_STATUS, string="Status", required=True, default="uncorrected"
    )
    amount = fields.Monetary(
        string="Amount", currency_field="company_currency_id", required=True
    )
    rationale = fields.Text(
        string="Rationale / Non-correction Justification", required=True
    )
    ...
    attachments_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_misstatement_line_rel",
        "misstatement_id",
        "attachment_id",
        ...
        for record in self:
            if record.amount <= 0:
                raise ValidationError(
                    _("Misstatement amount must be greater than zero.")
                )
...
class FinalSubsequentProcedure(models.Model):
    ...
    finalisation_phase_id = fields.Many2one(
        "qaco.finalisation.phase", required=True, ondelete="cascade"
    )
    procedure_code = fields.Selection(
        PROCEDURE_CODES, string="Procedure", required=True
    )
...
class FinalGoingConcernCovenant(models.Model):
    ...
    def _compute_result(self):
        for record in self:
            ...
            record.result = (
                "compliant"
                if record.ratio_actual >= record.ratio_required
                else "breach"
            )
...
class FinalRelatedPartyLine(models.Model):
    ...
    balance_amount = fields.Monetary(
        string="Balance Amount", currency_field="company_currency_id"
    )
    ...
    documentation_attachment_ids = fields.Many2many(
        "ir.attachment",
        "qaco_final_related_line_rel",
        "line_id",
        "attachment_id",
        string="Supporting Docs",
    )
    company_currency_id = fields.Many2one(
        "res.currency", default=lambda self: self.env.company.currency_id
    )
...
class FinalReviewNote(models.Model):
    ...
    state = fields.Selection(
        REVIEW_NOTE_STATES, string="Status", default="open", tracking=True
    )
